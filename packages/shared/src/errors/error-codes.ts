/**
 * Stable error code hierarchy for the Barghsa platform.
 *
 * Format: `DOMAIN:SUBDOMAIN:ERROR_NAME`
 * Codes are stable across releases — never renumber or repurpose a code.
 * Deprecate by adding NEW codes, never rename existing ones.
 */

/** Error severity levels for logging */
export type ErrorSeverity = 'debug' | 'info' | 'warn' | 'error' | 'critical';

/** A single stable error code definition */
export interface ErrorCodeDef {
  /** Machine-readable code, e.g. 'VALIDATION:INPUT:INVALID_EMAIL' */
  code: string;
  /** Default HTTP status for this error type */
  httpStatus: number;
  /** Safe English title for tooling; client messages use messageKey. */
  title: string;
  /** A later attempt may succeed without changing input. Respect Retry-After and operation idempotency. */
  retryable: boolean;
  /** i18n key for the localized message */
  messageKey: string;
  /** Default severity for logging */
  severity: ErrorSeverity;
}

/** Error code definitions */
export const ErrorCodes = {
  // ── Validation ──────────────────────────────────────────
  VALIDATION_PAYLOAD_TOO_LARGE: {
    code: 'VALIDATION:INPUT:PAYLOAD_TOO_LARGE',
    httpStatus: 413,
    title: 'The request is too large',
    retryable: false,
    messageKey: 'error.validation.payload_too_large',
    severity: 'debug' as ErrorSeverity,
  },
  VALIDATION_INPUT_INVALID: {
    code: 'VALIDATION:INPUT:INVALID',
    httpStatus: 400,
    title: 'Invalid input value',
    retryable: false,
    messageKey: 'error.validation.input.invalid',
    severity: 'debug' as ErrorSeverity,
  },
  VALIDATION_INPUT_MISSING: {
    code: 'VALIDATION:INPUT:MISSING',
    httpStatus: 400,
    title: 'Required field is missing',
    retryable: false,
    messageKey: 'error.validation.input.missing',
    severity: 'debug' as ErrorSeverity,
  },
  VALIDATION_PARSE_ZOD: {
    code: 'VALIDATION:PARSE:ZOD_ERROR',
    httpStatus: 400,
    title: 'Submitted data is not valid',
    retryable: false,
    messageKey: 'error.validation.parse.zod',
    severity: 'debug' as ErrorSeverity,
  },
  VALIDATION_PARSE_JSON: {
    code: 'VALIDATION:PARSE:JSON_ERROR',
    httpStatus: 400,
    title: 'Invalid JSON request format',
    retryable: false,
    messageKey: 'error.validation.parse.json',
    severity: 'debug' as ErrorSeverity,
  },

  // ── Authentication ──────────────────────────────────────
  AUTH_UNAUTHENTICATED: {
    code: 'AUTH:UNAUTHENTICATED',
    httpStatus: 401,
    title: 'Authentication required',
    retryable: false,
    messageKey: 'error.auth.unauthenticated',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_TOKEN_EXPIRED: {
    code: 'AUTH:TOKEN_EXPIRED',
    httpStatus: 401,
    title: 'Your session has expired',
    retryable: false,
    messageKey: 'error.auth.token.expired',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_TOKEN_INVALID: {
    code: 'AUTH:TOKEN_INVALID',
    httpStatus: 401,
    title: 'Invalid session token',
    retryable: false,
    messageKey: 'error.auth.token.invalid',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_SESSION_REVOKED: {
    code: 'AUTH:SESSION_REVOKED',
    httpStatus: 401,
    title: 'Your session has been revoked',
    retryable: false,
    messageKey: 'error.auth.session.revoked',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_MFA_REQUIRED: {
    code: 'AUTH:MFA_REQUIRED',
    httpStatus: 401,
    title: 'Multi-factor authentication is required',
    retryable: false,
    messageKey: 'error.auth.mfa.required',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_MFA_INVALID: {
    code: 'AUTH:MFA_INVALID',
    httpStatus: 401,
    title: 'Invalid authentication code',
    retryable: false,
    messageKey: 'error.auth.mfa.invalid',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_REGISTER_USERNAME_TAKEN: {
    code: 'AUTH:REGISTER:USERNAME_TAKEN',
    httpStatus: 409,
    title: 'This username is already registered',
    retryable: false,
    messageKey: 'auth.register.error.usernameTaken',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_REGISTER_INVALID_USERNAME: {
    code: 'AUTH:REGISTER:INVALID_USERNAME',
    httpStatus: 400,
    title: 'Invalid username',
    retryable: false,
    messageKey: 'auth.register.error.invalidUsername',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_REGISTER_WEAK_PASSWORD: {
    code: 'AUTH:REGISTER:WEAK_PASSWORD',
    httpStatus: 422,
    title: 'Password does not meet security requirements',
    retryable: false,
    messageKey: 'auth.register.error.weakPassword',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_REGISTER_TOS_NOT_ACCEPTED: {
    code: 'AUTH:REGISTER:TOS_NOT_ACCEPTED',
    httpStatus: 400,
    title: 'You must accept the terms of service',
    retryable: false,
    messageKey: 'auth.register.error.tosNotAccepted',
    severity: 'debug' as ErrorSeverity,
  },

  VERIFICATION_PROVIDER_UNAVAILABLE: {
    code: 'VERIFICATION:PROVIDER:UNAVAILABLE',
    httpStatus: 503,
    title: 'Automatic identity verification is currently unavailable.',
    retryable: false,
    messageKey: 'profile.verification.error.providerUnavailable',
    severity: 'error' as ErrorSeverity,
  },

  // ── OTP ────────────────────────────────────────────
  AUTH_DELIVERY_UNAVAILABLE: {
    code: 'AUTH:DELIVERY:UNAVAILABLE',
    httpStatus: 503,
    title: 'Verification code delivery is unavailable. Please try again shortly.',
    retryable: true,
    messageKey: 'auth.otp.error.deliveryUnavailable',
    severity: 'error' as ErrorSeverity,
  },
  AUTH_OTP_INVALID: {
    code: 'AUTH:OTP:INVALID',
    httpStatus: 401,
    title: 'Invalid verification code',
    retryable: false,
    messageKey: 'auth.otp.error.invalid',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_OTP_EXPIRED: {
    code: 'AUTH:OTP:EXPIRED',
    httpStatus: 401,
    title: 'Verification code has expired',
    retryable: false,
    messageKey: 'auth.otp.error.expired',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_OTP_MAX_ATTEMPTS: {
    code: 'AUTH:OTP:MAX_ATTEMPTS',
    httpStatus: 401,
    title: 'Too many failed attempts',
    retryable: false,
    messageKey: 'auth.otp.error.maxAttempts',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_OTP_CONSUMED: {
    code: 'AUTH:OTP:ALREADY_CONSUMED',
    httpStatus: 409,
    title: 'This verification code has already been used. Request a new code.',
    retryable: false,
    messageKey: 'auth.otp.error.alreadyConsumed',
    severity: 'info' as ErrorSeverity,
  },

  // ── Authorization ───────────────────────────────────────
  AUTHZ_FORBIDDEN: {
    code: 'AUTHZ:FORBIDDEN',
    httpStatus: 403,
    title: 'Access denied',
    retryable: false,
    messageKey: 'error.authz.forbidden',
    severity: 'info' as ErrorSeverity,
  },
  AUTHZ_INSUFFICIENT_ROLE: {
    code: 'AUTHZ:INSUFFICIENT_ROLE',
    httpStatus: 403,
    title: 'Your role does not have permission for this action',
    retryable: false,
    messageKey: 'error.authz.insufficient.role',
    severity: 'info' as ErrorSeverity,
  },
  AUTHZ_RESOURCE_OWNER: {
    code: 'AUTHZ:NOT_RESOURCE_OWNER',
    httpStatus: 403,
    title: 'You are not the owner of this resource',
    retryable: false,
    messageKey: 'error.authz.not.resource.owner',
    severity: 'info' as ErrorSeverity,
  },

  // ── Not Found ───────────────────────────────────────────
  NOT_FOUND_RESOURCE: {
    code: 'NOT_FOUND:RESOURCE',
    httpStatus: 404,
    title: 'Requested resource was not found',
    retryable: false,
    messageKey: 'error.not_found.resource',
    severity: 'debug' as ErrorSeverity,
  },
  NOT_FOUND_ROUTE: {
    code: 'NOT_FOUND:ROUTE',
    httpStatus: 404,
    title: 'Requested route was not found',
    retryable: false,
    messageKey: 'error.not_found.route',
    severity: 'debug' as ErrorSeverity,
  },

  // ── Conflict ────────────────────────────────────────────
  CONFLICT_DUPLICATE: {
    code: 'CONFLICT:DUPLICATE_ENTRY',
    httpStatus: 409,
    title: 'This entry already exists',
    retryable: false,
    messageKey: 'error.conflict.duplicate',
    severity: 'debug' as ErrorSeverity,
  },
  CONFLICT_STATE: {
    code: 'CONFLICT:INVALID_STATE',
    httpStatus: 409,
    title: 'Current state does not allow this operation',
    retryable: false,
    messageKey: 'error.conflict.state',
    severity: 'debug' as ErrorSeverity,
  },
  CONFLICT_VERSION: {
    code: 'CONFLICT:VERSION_CONFLICT',
    httpStatus: 409,
    title: 'Concurrent modification conflict detected',
    retryable: false,
    messageKey: 'error.conflict.version',
    severity: 'debug' as ErrorSeverity,
  },

  // ── CRM / Business Constraints ──────────────────────────
  CRM_PROFILE_DELETION_BLOCKED: {
    code: 'CRM:PROFILE:DELETION_BLOCKED',
    httpStatus: 409,
    title: 'This profile has linked records that prevent deletion.',
    retryable: false,
    messageKey: 'crm.profile.deletion.blocked',
    severity: 'info' as ErrorSeverity,
  },
  CRM_PROFILE_ALREADY_ARCHIVED: {
    code: 'CRM:PROFILE:ALREADY_ARCHIVED',
    httpStatus: 409,
    title: 'This profile is already archived.',
    retryable: false,
    messageKey: 'crm.profile.already_archived',
    severity: 'info' as ErrorSeverity,
  },
  CRM_PROFILE_LAST_OWNER: {
    code: 'CRM:PROFILE:LAST_OWNER',
    httpStatus: 409,
    title: 'The last profile owner cannot be removed.',
    retryable: false,
    messageKey: 'crm.profile.last_owner',
    severity: 'info' as ErrorSeverity,
  },

  // ── CSRF ─────────────────────────────────────────────────
  AUTHZ_CSRF_INVALID: {
    code: 'AUTHZ:CSRF_TOKEN_INVALID',
    httpStatus: 403,
    title: 'The security token is invalid. Refresh the page and try again.',
    retryable: false,
    messageKey: 'error.authz.csrf.invalid',
    severity: 'warn' as ErrorSeverity,
  },
  AUTHZ_STEP_UP_REQUIRED: {
    code: 'AUTHZ:STEP_UP_REQUIRED',
    httpStatus: 403,
    title: 'Re-verify your identity to continue',
    retryable: false,
    messageKey: 'error.authz.step_up.required',
    severity: 'debug' as ErrorSeverity,
  },
  AUTHZ_PROFILE_NOT_VERIFIED: {
    code: 'AUTHZ:PROFILE_NOT_VERIFIED',
    httpStatus: 403,
    title: 'Your profile is not verified. Please verify your profile before placing an order',
    retryable: false,
    messageKey: 'error.authz.profile.not_verified',
    severity: 'info' as ErrorSeverity,
  },

  // ── Rate Limit ──────────────────────────────────────────
  RATE_LIMIT_EXCEEDED: {
    code: 'RATE_LIMIT:EXCEEDED',
    httpStatus: 429,
    title: 'Too many requests – please try again later',
    retryable: true,
    messageKey: 'error.rate_limit.exceeded',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_OTP_RATE_LIMITED: {
    code: 'AUTH:OTP:RATE_LIMITED',
    httpStatus: 429,
    title: 'Too many code requests. Please wait before trying again.',
    retryable: true,
    messageKey: 'auth.otp.error.rateLimited',
    severity: 'info' as ErrorSeverity,
  },

  // ── Provider / External Service ─────────────────────────
  PROVIDER_DOWNSTREAM: {
    code: 'PROVIDER:DOWNSTREAM_ERROR',
    httpStatus: 502,
    title: 'External service error',
    retryable: true,
    messageKey: 'error.provider.downstream',
    severity: 'error' as ErrorSeverity,
  },
  PROVIDER_TIMEOUT: {
    code: 'PROVIDER:TIMEOUT',
    httpStatus: 504,
    title: 'External service timed out',
    retryable: true,
    messageKey: 'error.provider.timeout',
    severity: 'error' as ErrorSeverity,
  },
  PROVIDER_RATE_LIMITED: {
    code: 'PROVIDER:RATE_LIMITED',
    httpStatus: 429,
    title: 'External service is rate-limited',
    retryable: true,
    messageKey: 'error.provider.rate_limited',
    severity: 'warn' as ErrorSeverity,
  },
  PROVIDER_CALLBACK_INVALID: {
    code: 'PROVIDER:CALLBACK:INVALID',
    httpStatus: 401,
    title: 'Payment provider callback is invalid',
    retryable: false,
    messageKey: 'error.provider.callback.invalid',
    severity: 'warn' as ErrorSeverity,
  },
  PROVIDER_CALLBACK_REPLAYED: {
    code: 'PROVIDER:CALLBACK:REPLAYED',
    httpStatus: 401,
    title: 'Payment provider callback is expired or replayed',
    retryable: false,
    messageKey: 'error.provider.callback.replayed',
    severity: 'warn' as ErrorSeverity,
  },
  PROVIDER_CALLBACK_UNCONFIGURED: {
    code: 'PROVIDER:CALLBACK:UNCONFIGURED',
    httpStatus: 503,
    title: 'Payment provider callback signing secret is not configured',
    retryable: false,
    messageKey: 'error.provider.callback.unconfigured',
    severity: 'error' as ErrorSeverity,
  },

  // ── Internal ────────────────────────────────────────────
  INTERNAL_SERVER: {
    code: 'INTERNAL:SERVER_ERROR',
    httpStatus: 500,
    title: 'Internal server error',
    retryable: false,
    messageKey: 'error.internal.server',
    severity: 'error' as ErrorSeverity,
  },
  INTERNAL_DATABASE: {
    code: 'INTERNAL:DATABASE_ERROR',
    httpStatus: 500,
    title: 'Database error',
    retryable: false,
    messageKey: 'error.internal.database',
    severity: 'error' as ErrorSeverity,
  },
  INTERNAL_UNEXPECTED: {
    code: 'INTERNAL:UNEXPECTED',
    httpStatus: 500,
    title: 'An unexpected error occurred',
    retryable: false,
    messageKey: 'error.internal.unexpected',
    severity: 'critical' as ErrorSeverity,
  },
  AUTH_REGISTER_FAILED: {
    code: 'AUTH:REGISTER:FAILED',
    httpStatus: 500,
    title: 'An error occurred. Please try again',
    retryable: false,
    messageKey: 'auth.register.error.generic',
    severity: 'error' as ErrorSeverity,
  },

  // ── Login ─────────────────────────────────────
  AUTH_LOGIN_INVALID_CREDENTIALS: {
    code: 'AUTH:LOGIN:INVALID_CREDENTIALS',
    httpStatus: 401,
    title: 'Invalid username or password',
    retryable: false,
    messageKey: 'auth.login.error.invalidCredentials',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_LOGIN_MUST_CHANGE_PASSWORD: {
    code: 'AUTH:LOGIN:MUST_CHANGE_PASSWORD',
    httpStatus: 400,
    title: 'Password change is required',
    retryable: false,
    messageKey: 'auth.login.error.mustChangePassword',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_ACCOUNT_DISABLED: {
    code: 'AUTH:ACCOUNT:DISABLED',
    httpStatus: 403,
    title: 'This account has been disabled',
    retryable: false,
    messageKey: 'auth.login.error.accountDisabled',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_LOGIN_PASSWORD_REUSED: {
    code: 'AUTH:LOGIN:PASSWORD_REUSED',
    httpStatus: 422,
    title: 'This password was used before. Please choose a different password',
    retryable: false,
    messageKey: 'auth.login.error.passwordReused',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_LOGIN_FAILED: {
    code: 'AUTH:LOGIN:FAILED',
    httpStatus: 500,
    title: 'An error occurred. Please try again',
    retryable: false,
    messageKey: 'auth.login.error.generic',
    severity: 'error' as ErrorSeverity,
  },

  // ── Username / Contact Change ──────────────────────────
  AUTH_CHANGE_USERNAME_INVALID: {
    code: 'AUTH:CHANGE_USERNAME:INVALID',
    httpStatus: 400,
    title: 'Invalid username change request',
    retryable: false,
    messageKey: 'auth.changeUsername.error.invalid',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_SAME: {
    code: 'AUTH:CHANGE_USERNAME:SAME',
    httpStatus: 400,
    title: 'New username is the same as current',
    retryable: false,
    messageKey: 'auth.changeUsername.error.same',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_TAKEN: {
    code: 'AUTH:CHANGE_USERNAME:TAKEN',
    httpStatus: 409,
    title: 'This username is already taken',
    retryable: false,
    messageKey: 'auth.changeUsername.error.taken',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_NO_EMAIL: {
    code: 'AUTH:CHANGE_USERNAME:NO_EMAIL',
    httpStatus: 400,
    title: 'No email registered for this account',
    retryable: false,
    messageKey: 'auth.changeUsername.error.noEmail',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_NO_MOBILE: {
    code: 'AUTH:CHANGE_USERNAME:NO_MOBILE',
    httpStatus: 400,
    title: 'No mobile number registered for this account',
    retryable: false,
    messageKey: 'auth.changeUsername.error.noMobile',
    severity: 'info' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_ALREADY_HAS_EMAIL: {
    code: 'AUTH:CHANGE_USERNAME:ALREADY_HAS_EMAIL',
    httpStatus: 409,
    title: 'This account already has an email',
    retryable: false,
    messageKey: 'auth.changeUsername.error.alreadyHasEmail',
    severity: 'debug' as ErrorSeverity,
  },
  AUTH_CHANGE_USERNAME_ALREADY_HAS_MOBILE: {
    code: 'AUTH:CHANGE_USERNAME:ALREADY_HAS_MOBILE',
    httpStatus: 409,
    title: 'This account already has a mobile number',
    retryable: false,
    messageKey: 'auth.changeUsername.error.alreadyHasMobile',
    severity: 'debug' as ErrorSeverity,
  },
} as const satisfies Record<string, ErrorCodeDef>;

/** Union type of all error code strings */
export type ErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes]['code'];

/** Get an error code definition by its HTTP status – useful as a fallback */
export function errorCodeForHttpStatus(status: number): ErrorCodeDef {
  const matched = Object.values(ErrorCodes).find((def) => def.httpStatus === status);
  return matched ?? ErrorCodes.INTERNAL_UNEXPECTED;
}

/** Get a default error code for a given HTTP status */
export function defaultErrorCode(status: number): string {
  return errorCodeForHttpStatus(status).code;
}
