...
describe('AiModelTester (T-09.11.01)', () => {
  describe('wire protocol shaping', () => {
    it('shapes an OpenAI-compatible chat completion ping', async () => {
      const client = okClient(200, JSON.stringify({ choices: [{ message: { content: 'pong' } }] }));
      const tester = new AiModelTester(client);
  ...
  describe('response preview extraction', () => {
    it('falls back to trimmed raw text when the body is not JSON', async () => {
      const tester = new AiModelTester(okClient(200, '  plain text reply  '));
      const result = await tester.test(input());
      expect(result.ok).toBe(true);
    ...
    it('caps oversized bodies to the preview limit', async () => {
      ...
      const tester = new AiModelTester(okClient(200, long));
      const result = await tester.test(input());
  ...
  describe('failure handling', () => {
    ...
    it('redacts the submitted token from thrown-client errors', async () => {
      ...
      const tester = new AiModelTester(failing);
    ...
    it('redacts the submitted token from successful response previews', async () => {
      const client = okClient(
        200,
        JSON.stringify({ choices: [{ message: { content: 'echo sk-preview-secret' } }] })
      );
      const tester = new AiModelTester(client);
  ...
  describe('structural validation', () => {
    it('rejects a missing base URL', async () => {
      const tester = new AiModelTester(okClient(200, '{}'));
  ...
  describe('SSRF guard', () => {
    ...
    it('honors the deployment allow-list for local endpoints', async () => {
      ...
      try {
        const tester = new AiModelTester(okClient(200, JSON.stringify({ ok: true })));
...
describe('connection-test response limits and token boundaries', () => {
  it.each(['preview', 'error'] as const)('redacts a token spanning the %s cutoff', async (kind) => {
    ...
    const tester = new AiModelTester(
      okClient(kind === 'preview' ? 200 : 401, JSON.stringify(body))
    );
  ...
  ])('rejects an ambiguous base URL before sending credentials: %s', async (baseUrl) => {
    const client = okClient(200, 'pong');
    const result = await new AiModelTester(client).test(input({ baseUrl }));
    expect(result.ok).toBe(false);
