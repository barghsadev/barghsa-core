...
const message = { destination: 'staff@example.test', subject: 'Invoice', text: '5000', idempotencyKey: 'same-occurrence' }
it('pins the checked address, requires TLS and keeps stable message IDs across retries', async () => {
  ...
  expect(await send(message)).toBe('smtp-receipt')
  expect(createTransport).toHaveBeenCalledWith(expect.objectContaining({ host: '8.8.8.8', requireTLS: true,
    tls: { servername: 'mail.example.test', minVersion: 'TLSv1.2' }, disableFileAccess: true, disableUrlAccess: true }))
...
it('requires recipient acceptance and closes the connection after failure', async () => {
  sendMail.mockResolvedValue({ accepted: [], rejected: ['staff@example.test'], messageId: 'not-delivered' })
