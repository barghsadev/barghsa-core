...
export class EmailCircuitBreaker {
  constructor(private readonly pool: DeliveryPool, private readonly config: EmailBreakerConfig = DEFAULT_EMAIL_BREAKER_CONFIG,
  ...
  async readState(providerId: string): Promise<EmailBreakerState> {
    const row = (await this.pool.query(`SELECT ${COLUMNS} FROM email_provider_configs WHERE id=$1`, [providerId])).rows[0]
  ...
  async decision(providerId: string): Promise<EmailBreakerDecision> {
    ...
    const claimed = await this.pool.query(`UPDATE email_provider_configs SET cooldown_until=$3
      WHERE id=$1 AND degraded=true AND (cooldown_until IS NULL OR cooldown_until <= $2)
      RETURNING ${COLUMNS}`, [providerId, now, deadline])
    if (claimed.rows[0]) {
    ...
    return { allow: false, kind: 'open', state, degradedReason: state.degradedReason ?? 'Email provider degraded',
  ...
  async recordOutcome(providerId: string, outcome: EmailBreakerOutcome): Promise<EmailBreakerState> {
    ...
    if (outcome.ok) {
      await this.pool.query(`UPDATE email_provider_configs SET degraded=false,degraded_reason=NULL,
        consecutive_failures=0,window_failures=0,window_started_at=NULL,last_failure_at=NULL,opened_at=NULL,cooldown_until=NULL
        WHERE id=$1 AND ${owned}`, [providerId, now, token])
    } else {
      ...
      await this.pool.query(`UPDATE email_provider_configs SET
        ...
        WHERE id=$1 AND ${owned}`, [providerId, now, token, this.config.windowMs, this.config.threshold, this.config.cooldownMs])
    }
