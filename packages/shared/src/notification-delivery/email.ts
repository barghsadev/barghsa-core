...
import { z } from 'zod'
import { decryptProviderSecret, ResendConfigSchema, SmtpConfigSchema, SmtpNetworkGuard } from '../auth-delivery/index.js'
...
export function createEmailSender(pool: DeliveryPool, request: typeof fetch = fetch) {
  return async (message: EmailMessage): Promise<string> => {
    if (!z.string().email().safeParse(message.destination).success || /[\r\n]/.test(message.subject) || !message.idempotencyKey || (!message.html && !message.text)) {
      throw new Error('Invalid email message')
    }
    message.signal?.throwIfAborted()
    const suppressed = await pool.query('SELECT id FROM email_suppressions WHERE lower(address)=lower($1) LIMIT 1', [message.destination])
    if (suppressed.rows.length) throw new Error('Email address suppressed')
    const providers = await pool.query("SELECT id,transport,config FROM email_provider_configs WHERE status='active' AND last_test_status='passed'")
    ...
    const send = async (): Promise<string> => {
      ...
      if (provider.transport === 'resend') {
        ...
        const response = await request('https://api.resend.com/emails', {
          ...
          headers: { Authorization: `Bearer ${decryptProviderSecret(config.api_key)}`, 'Content-Type': 'application/json', 'Idempotency-Key': message.idempotencyKey },
          body: JSON.stringify({ from: config.from_email, to: [message.destination], subject: message.subject,
            ...(message.html ? { html: message.html } : {}), ...(message.text ? { text: message.text } : {}), reply_to: config.reply_to }),
        })
        if (!response.ok) throw new Error('Email provider rejected message')
        const body = await response.json() as { id?: unknown }
        if (typeof body.id !== 'string' || !body.id.trim()) throw new Error('Email provider returned no receipt')
        return body.id
      }
      ...
      const transport = nodemailer.createTransport({
        ...
        connectionTimeout: Math.min(config.connection_timeout * 1000, 10_000), greetingTimeout: 10_000,
        socketTimeout: Math.min(config.command_timeout * 1000, 15_000), disableFileAccess: true, disableUrlAccess: true,
      })
      let abort!: () => void
      const cancelled = new Promise<never>((_, reject) => {
        abort = () => { transport.close(); reject(new Error('Email delivery cancelled')) }
        signal.addEventListener('abort', abort, { once: true })
        if (signal.aborted) abort()
      })
      try {
        signal.throwIfAborted()
        const result = await Promise.race([cancelled, transport.sendMail({
          from: { name: config.from_name ?? 'Barghsa', address: config.from_email }, to: message.destination,
          replyTo: config.reply_to, subject: message.subject, ...(message.html ? { html: message.html } : {}),
          ...(message.text ? { text: message.text } : {}),
          messageId: `<${createHash('sha256').update(message.idempotencyKey).digest('hex')}@${config.from_email.split('@')[1]}>`,
        })])
        if (result.accepted?.length !== 1 || result.rejected?.length || !result.messageId) throw new Error('SMTP recipient rejected')
    ...
    try { receipt = await send() }
      ...
      await breaker.recordOutcome(provider.id, { ok: false, ...(decision.probeToken ? { probeToken: decision.probeToken } : {}) })
      throw error
    }
    await breaker.recordOutcome(provider.id, { ok: true, ...(decision.probeToken ? { probeToken: decision.probeToken } : {}) })
    return receipt
