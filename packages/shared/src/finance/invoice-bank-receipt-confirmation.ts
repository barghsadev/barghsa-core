...
export const INVOICE_BANK_RECEIPT_CONFIRMED_EVENT = 'invoice.bank_receipt.confirmed' as const;

/** Canonical audit event when staff reject an invoice bank receipt. */
export const INVOICE_BANK_RECEIPT_REJECTED_EVENT = 'invoice.bank_receipt.rejected' as const;
...
export const INVOICE_BANK_RECEIPT_REJECT_CUSTOMER_ROUTE_PREFIX = '/invoices/' as const;
...
export const INVOICE_BANK_RECEIPT_CONFIRMABLE_STATES = ['Submitted', 'UnderReview'] as const;
...
export function isInvoiceBankReceiptConfirmableState(
  ...
  return (INVOICE_BANK_RECEIPT_CONFIRMABLE_STATES as readonly string[]).includes(state);
}
...
export function invoiceBankReceiptRejectedNotificationIdempotencyKey(receiptId: string): string {
 ...
 */
export function invoiceBankReceiptOverpaymentCreditIdempotencyKey(receiptId: string): string {
