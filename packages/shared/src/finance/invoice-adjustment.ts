...
export const CUSTOMER_PAYABLE_STATES = ['Unpaid', 'Overdue', 'PartiallyFunded'] as const;
...
export function isCreditAdjustmentKind(value: string | null | undefined): boolean {
  return value === 'credit';
}
...
export function isCustomerPaymentTransition(
  ...
  return (CUSTOMER_PAYMENT_TRANSITIONS as readonly string[]).includes(value ?? '');
}
