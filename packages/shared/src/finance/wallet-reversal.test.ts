...
describe('wallet reversal helpers (T-04.2.04.01)', () => {
  it('names the reversal ledger type, posted state, and unique index', () => {
    ...
    expect(WALLET_TX_REVERSAL_ORIGINAL_CONSTRAINT).toBe('chk_wallet_tx_reversal_original');
    expect(REVERSIBLE_WALLET_LEDGER_TYPES).toEqual(['topup', 'payment', 'refund', 'compensating']);
  });
  ...
  it('only treats Completed as reversible', () => {
    ...
    for (const state of ['Pending', 'Reserved', 'Failed', 'Rejected', 'Released', 'Reversed']) {
  ...
  it('accepts a matching Completed reversal replay and rejects collisions', () => {
    ...
    expect(isMatchingReversalReplay({ ...matching, amount: -100n }, expected)).toBe(false);
    expect(isMatchingReversalReplay({ ...matching, type: 'compensating' }, expected)).toBe(false);
    expect(isMatchingReversalReplay({ ...matching, description: 'other' }, expected)).toBe(false);
    expect(
      isMatchingReversalReplay({ ...matching, reversesTransactionId: WALLET_ID }, expected)
    ).toBe(false);
  });

  it('formats operator-facing error messages', () => {
    expect(WALLET_REVERSAL_ERRORS.ORIGINAL_ID_REQUIRED()).toMatch(/UUID/);
    expect(WALLET_REVERSAL_ERRORS.ALREADY_REVERSED(ORIGINAL_ID)).toContain(ORIGINAL_ID);
    expect(WALLET_REVERSAL_ERRORS.INSUFFICIENT_BALANCE(1n, 2n)).toBe(
      'Insufficient balance: available=1, required=2'
    );
    expect(WALLET_REVERSAL_ERRORS.USE_REVERSE_TRANSACTION()).toMatch(/reverseTransaction/);
  });
});
