 ...
 */
export const ONLINE_TOPUP_CREDIT_IDEMPOTENCY_PREFIX = 'wallet-online-topup-credit:' as const;
...
export const WALLET_CHARGEBACK_EVENT_CONSTRAINT = 'uq_wallet_chargeback_event_id' as const;

/** Provider event types that mean money already credited was taken back. */
export const CHARGEBACK_NOTIFICATION_TYPES = ['chargeback', 'reversed', 'reversal'] as const;

export type ChargebackNotificationType = (typeof CHARGEBACK_NOTIFICATION_TYPES)[number];
...
export type WalletChargebackEventStatus = (typeof WALLET_CHARGEBACK_EVENT_STATUSES)[number];

export const WALLET_CHARGEBACK_UNRESOLVED_STATUSES = ['unmatched', 'unresolved'] as const;

export type WalletChargebackUnresolvedStatus =
  (typeof WALLET_CHARGEBACK_UNRESOLVED_STATUSES)[number];

export function isChargebackNotificationType(raw: string): raw is ChargebackNotificationType {
  return (CHARGEBACK_NOTIFICATION_TYPES as readonly string[]).includes(raw);
}

export function isChargebackMatchMethod(raw: string): raw is ChargebackMatchMethod {
  return (CHARGEBACK_MATCH_METHODS as readonly string[]).includes(raw);
}

export function isWalletChargebackEventStatus(raw: string): raw is WalletChargebackEventStatus {
  return (WALLET_CHARGEBACK_EVENT_STATUSES as readonly string[]).includes(raw);
}

export function isUnresolvedChargebackStatus(raw: string): raw is WalletChargebackUnresolvedStatus {
  return (WALLET_CHARGEBACK_UNRESOLVED_STATUSES as readonly string[]).includes(raw);
}

/** Credit idempotency key written when the Pending top-up was confirmed. */
export function chargebackCreditIdempotencyKey(pendingTransactionId: string): string {
 ...
 */
export function parseChargebackNotification(raw: unknown): ParseChargebackResult {
...
export function parseChargebackNotificationJson(rawBody: string): ParseChargebackResult {
  let parsed: unknown;
  try {
...
export function hasChargebackLocator(notification: ParsedChargebackNotification): boolean {
  return Boolean(
    notification.merchantOrderId || notification.providerRefId || notification.authority
  );
}
...
export function topUpPendingTransactionId(candidate: ChargebackTopUpCandidate): string | null {
  if (!candidate.metadata || typeof candidate.metadata !== 'object') return null;
  const pending = (candidate.metadata as { pendingTransactionId?: unknown }).pendingTransactionId;
  return typeof pending === 'string' && pending.length > 0 ? pending : null;
}

export function topUpAuthority(candidate: ChargebackTopUpCandidate): string | null {
  if (candidate.metadata && typeof candidate.metadata === 'object') {
    ...
    if (typeof meta.gateway?.authority === 'string' && meta.gateway.authority.length > 0) {
