...
describe('isEligibleForOnlineTopUpExpiry', () => {
  ...
  it('refuses bank-receipt Pendings even when old', () => {
    expect(isEligibleForOnlineTopUpExpiry({ ...eligible, channel: 'bank_receipt' }, NOW, TTL)).toBe(
      false
    );
  });
  ...
  it('refuses every non-Pending or non-topup row even when past TTL', () => {
    ...
    expect(isEligibleForOnlineTopUpExpiry({ ...eligible, type: 'payment' }, NOW, TTL)).toBe(false);
  });
});
