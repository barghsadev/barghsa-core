...
describe('parseBankReceiptAttachmentKey (T-04.2.02.03)', () => {
  it('accepts document and image keys with permitted extensions', () => {
    ...
    expect(
      parseBankReceiptAttachmentKey('uploads/image/bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb.jpg')
  ...
  it('rejects traversal, other categories, and disallowed extensions', () => {
    ...
    expect(
      parseBankReceiptAttachmentKey('uploads/video/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa.mp4')
    ).toBeNull();
    expect(
      parseBankReceiptAttachmentKey('uploads/document/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa.exe')
    ).toBeNull();
...
describe('parseBankReceiptTopUpSubmission (T-04.2.02.03)', () => {
  it('accepts a complete receipt payload and does not cap the amount', () => {
    const parsed = parseBankReceiptTopUpSubmission(validBody({ amount: 3_000_000_000 }), TODAY);
  ...
  it('fails closed on each required field', () => {
    ...
    expect(parseBankReceiptTopUpSubmission(validBody({ payerReference: '' }), TODAY)).toMatchObject(
      { ok: false, field: 'payerReference' }
    );
    expect(
...
describe('bank receipt metadata helpers (T-04.2.02.03)', () => {
  it('round-trips receipt details under the bank_receipt channel', () => {
    ...
    expect(receiptDetailsMatch(metadata, { ...receipt, payerReference: 'other' })).toBe(false);
...
describe('evaluateBankReceiptStorageMetadata (T-04.2.02.03)', () => {
  ...
  it('accepts a verified receipt bound to the accessible profile', () => {
    expect(
      evaluateBankReceiptStorageMetadata({ ...valid, uploadedBy: 'other-user' }, actorId, profileId)
    ).toEqual({ ok: true });
  });
