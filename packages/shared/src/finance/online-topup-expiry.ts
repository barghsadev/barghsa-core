...
export type OnlineTopUpCallbackOpenState = (typeof ONLINE_TOPUP_CALLBACK_OPEN_STATES)[number];

/**
 * Intent rows that a paid callback may advance to `Released` after credit.
 * `Released` is already terminal for the intent.
 */
export const ONLINE_TOPUP_INTENT_RELEASE_STATES = ['Pending', 'Failed', 'Rejected'] as const;

export type OnlineTopUpIntentReleaseState = (typeof ONLINE_TOPUP_INTENT_RELEASE_STATES)[number];

/** Parse a created/updated instant from pg (`Date`) or an ISO string. */
export function parseOnlineTopUpCreatedAt(value: Date | string | null | undefined): Date | null {
