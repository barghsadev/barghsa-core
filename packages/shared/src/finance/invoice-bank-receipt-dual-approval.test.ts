...
describe('invoice bank receipt dual-approval contract (T-04.3.01.05)', () => {
  it('reuses the admin dual-approval threshold config key and bank-payment action', () => {
    ...
    expect(INVOICE_BANK_RECEIPT_DUAL_APPROVAL_ACTION_TYPE).toBe('bank_payment_confirmation');
    expect(INVOICE_BANK_RECEIPT_DUAL_APPROVAL_REQUESTED_EVENT).toBe(
      'invoice.bank_receipt.dual_approval_requested'
    );
    expect(INVOICE_BANK_RECEIPT_DUAL_APPROVAL_REASON).toBe('Invoice bank receipt confirmation');
  });
  ...
  it('names fail-closed errors for same-staff completion and corrupt config', () => {
    ...
    expect(INVOICE_BANK_RECEIPT_DUAL_APPROVAL_ERRORS.CONFIG_CORRUPT()).toContain('invalid');
  ...
  it('stores a stable details payload keyed by receiptId', () => {
    ...
    expect(receiptIdFromInvoiceBankReceiptDualApprovalDetails(details)).toBe(RECEIPT_ID);
    expect(receiptIdFromInvoiceBankReceiptDualApprovalDetails(null)).toBeNull();
    expect(
      receiptIdFromInvoiceBankReceiptDualApprovalDetails({ invoiceId: INVOICE_ID })
    ).toBeNull();
  });
});
...
describe('readInvoiceBankReceiptDualApprovalThreshold (T-04.3.01.05)', () => {
  ...
  it('accepts a positive safe-integer threshold', () => {
    expect(readInvoiceBankReceiptDualApprovalThreshold({ threshold_irr: THRESHOLD })).toEqual({
      status: 'enabled',
      thresholdIrR: THRESHOLD,
    });
  });
...
describe('invoiceBankReceiptRequiresDualApproval (T-04.3.01.05)', () => {
  it('never requires approval when disabled', () => {
    expect(
      invoiceBankReceiptRequiresDualApproval({ status: 'disabled', thresholdIrR: 0 }, 999_999_999n)
    ).toBe(false);
  });

  it('requires approval at and above the threshold in both helpers', () => {
    const read = { status: 'enabled' as const, thresholdIrR: THRESHOLD };
    expect(invoiceBankReceiptRequiresDualApproval(read, BigInt(THRESHOLD - 1))).toBe(false);
  ...
  it('does not require approval for a non-positive amount or a corrupt read', () => {
    ...
    expect(invoiceBankReceiptRequiresDualApproval({ status: 'corrupt' }, BigInt(THRESHOLD))).toBe(
      false
    );
  });
});
