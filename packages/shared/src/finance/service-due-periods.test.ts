...
describe('service due-period shared contract (T-04.1.03.01)', () => {
  ...
  describe('window status', () => {
    ...
    it('marks a future window as scheduled', () => {
      expect(serviceDuePeriodWindowStatus('2026-07-01T00:00:00.000Z', null, at)).toBe('scheduled');
    });

    it('marks an open window that already started as current', () => {
      expect(serviceDuePeriodWindowStatus('2026-01-01T00:00:00.000Z', null, at)).toBe('current');
    });

    it('marks a closed window whose exclusive end has arrived as expired', () => {
      expect(
        serviceDuePeriodWindowStatus('2026-01-01T00:00:00.000Z', '2026-06-15T00:00:00.000Z', at)
      ).toBe('expired');
    });

    it('keeps a window current until the exclusive until instant', () => {
      expect(
        serviceDuePeriodWindowStatus('2026-01-01T00:00:00.000Z', '2026-06-16T00:00:00.000Z', at)
      ).toBe('current');
    });
  });
});
