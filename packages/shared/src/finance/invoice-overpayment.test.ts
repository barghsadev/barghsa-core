...
describe('invoice overpayment allocation (T-04.2.02.05)', () => {
  ...
  describe('allocateReceiptAgainstInvoice', () => {
    ...
    it('credits the entire receipt to wallet when remaining is zero', () => {
      expect(allocateReceiptAgainstInvoice({ receiptAmount: 250_000n, remaining: 0n })).toEqual({
    ...
    it('rejects non-positive receipt amounts', () => {
      expect(() => allocateReceiptAgainstInvoice({ receiptAmount: 0n, remaining: 100n })).toThrow(
        BANK_RECEIPT_OVERPAYMENT_ERRORS.BAD_RECEIPT_AMOUNT()
      );
      expect(() => allocateReceiptAgainstInvoice({ receiptAmount: -1n, remaining: 100n })).toThrow(
        BANK_RECEIPT_OVERPAYMENT_ERRORS.BAD_RECEIPT_AMOUNT()
      );
    });
  });
