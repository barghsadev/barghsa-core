...
describe('reminder offset toggles contract (T-04.1.04.05)', () => {
  ...
  it('defaults every canonical offset on for every service type', () => {
    ...
    expect(defaults).toHaveLength(
      SERVICE_DUE_PERIOD_TYPES.length * INVOICE_REMINDER_OFFSETS.length
    );
    expect(defaults.every((row) => row.enabled)).toBe(true);
    expect(
      defaults.filter((row) => row.serviceType === 'electricity').map((row) => row.offset)
    ).toEqual([...INVOICE_REMINDER_OFFSETS]);
  });
  ...
  it('overlays stored disabled rows without dropping missing pairs', () => {
    ...
    expect(
      merged.find((row) => row.serviceType === 'electricity' && row.offset === -7)?.enabled
    ).toBe(false);
    expect(
      merged.find((row) => row.serviceType === 'electricity' && row.offset === -3)?.enabled
    ).toBe(true);
    expect(merged.find((row) => row.serviceType === 'manual' && row.offset === 0)?.enabled).toBe(
      false
    );
    expect(
      merged.find((row) => row.serviceType === 'saving_plan' && row.offset === -7)?.enabled
    ).toBe(true);
  });
  ...
  it('returns remaining offsets for a service type and the full set when type is unknown', () => {
    ...
    expect(enabledOffsetsForServiceType(toggles, 'saving_plan')).toEqual([
      ...INVOICE_REMINDER_OFFSETS,
    ]);
    expect(enabledOffsetsForServiceType(toggles, null)).toEqual([...INVOICE_REMINDER_OFFSETS]);
    expect(enabledOffsetsForServiceType(toggles, 'hardware')).toEqual([
      ...INVOICE_REMINDER_OFFSETS,
    ]);
  });
