...
describe('invoice adjustment accounting (T-04.1.05.03)', () => {
  ...
  it('never treats a credit note as a customer payable', () => {
    expect(isCustomerPayableInvoice({ state: 'Unpaid', adjustmentKind: 'credit' })).toBe(false);
    expect(isCustomerPayableInvoice({ state: 'Overdue', adjustmentKind: 'credit' })).toBe(false);
    expect(
      isCustomerPayableInvoice({
  ...
  it('treats open charge and ordinary invoices as payables', () => {
    for (const state of CUSTOMER_PAYABLE_STATES) {
      ...
      expect(isCustomerPayableInvoice({ state, adjustmentKind: 'charge' })).toBe(true);
    }
  ...
  it('unpaid outstanding SQL excludes credit notes', () => {
    expect(UNPAID_CUSTOMER_INVOICE_PREDICATE).toContain("state IN ('Unpaid', 'Overdue')");
