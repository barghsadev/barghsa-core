...
describe('complete production schema baseline', () => {
  it('creates the declared schema in an empty database without test-only prerequisite tables', async () => {
    ...
    expect(await runMigrations(options)).toEqual({ ok: true, applied: ['0080_complete_schema', '0081_restore_domain_constraints', '0082_restore_foundation_constraints', '0083_staff_identity', '0084_staff_capabilities', '0085_otp_purpose_binding', '0086_auth_delivery_outbox', '0087_authentication_version', '0088_staff_activation_delivery', '0089_pending_profile_verification'] })
    ...
    try {
      ...
      await pool.query('DROP TABLE auth_delivery_outbox; DROP INDEX users_activation_token_idx')
