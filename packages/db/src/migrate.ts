...
function journal(folder: string): JournalEntry[] {
  ...
  for (const entry of entries) {
    if (
      ...
      throw new Error(
        'Migration journal requires unique tags and strictly increasing positive timestamps'
      );
    }
    tags.add(entry.tag);
    previous = entry.when;
...
function verifyHistory(
  ...
  const latest = applied.reduce(
    (max, row) => (BigInt(row.created_at) > max ? BigInt(row.created_at) : max),
    0n
  );
  if (latest > BigInt(entries.at(-1)!.when))
    throw new Error('Database migration history is newer than this journal');
  for (const entry of entries) {
  ...
  try {
    ...
    const applied = after
      ...
      .map((row) => {
        ...
        if (!entry)
          throw new Error(`Migration timestamp ${row.created_at} is absent from the journal`);
        return entry.tag;
      });
...
export async function verifyMigrationVersion(
  ...
  const folder = options.migrationsFolder ?? MIGRATIONS_FOLDER;
  const entry = journal(folder).find(
    (entry) =>
      entry.tag === expectedMigrationId ||
