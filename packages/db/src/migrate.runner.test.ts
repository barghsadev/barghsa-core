...
describe('production migration runner', () => {
  ...
  it('blocks stale generated timestamps before applying any SQL', async () => {
    ...
    expect(await runMigrations(options)).toMatchObject({ ok: false, error: expect.stringContaining('strictly increasing') })
  ...
  it('rejects changed applied SQL and metadata gaps instead of reporting success', async () => {
    ...
    expect((await runMigrations(options)).ok).toBe(true)
    writeFileSync(join(options.migrationsFolder, '0000_foundation.sql'), 'SELECT 1;')
    expect(await runMigrations(options)).toMatchObject({ ok: false, error: expect.stringContaining('checksum/history mismatch') })
    ...
    try {
      ...
      journal.entries.unshift({ idx: -1, version: '7', when: 900, tag: 'missing_prior', breakpoints: true })
      writeFileSync(journalPath, JSON.stringify(journal))
      writeFileSync(join(options.migrationsFolder, 'missing_prior.sql'), 'SELECT 1;')
      expect(await runMigrations(options)).toMatchObject({ ok: false, error: expect.stringContaining('missing behind') })
      writeFileSync(journalPath, originalJournal)
      await pool.query(`UPDATE "${options.migrationsSchema}".__drizzle_migrations SET created_at=1001`)
      expect(await runMigrations(options)).toMatchObject({ ok: false, error: expect.stringContaining('newer than this journal') })
      await pool.query(`UPDATE "${options.migrationsSchema}".__drizzle_migrations SET created_at=1000`)
      expect(await runMigrations(options)).toEqual({ ok: true, applied: [] })
