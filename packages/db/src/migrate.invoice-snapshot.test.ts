...
describe('drizzle migrate() applies invoice_calculation_snapshot (T-04.1.02.08)', () => {
  ...
  beforeAll(async () => {
    ...
    `);
    // 0059 (service_due_periods), 0060 (invoice_reminder_schedule),
    // 0061 (reminder-schedule unique index), 0062 (offset toggles),
    // and 0063 (cancel on stop state) are journaled after 0058;
    // migrate() will also apply them and needs the users FK target
    // (invoices already exist from 0052 above).
    await ctx.db.execute(sql`
