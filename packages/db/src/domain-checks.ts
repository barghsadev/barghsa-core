...
const definitions: Record<string, Array<[string,string]>> = {
  ...
  "notification_delivery_log": [
    ...
    [
      "chk_ndl_error_category",
      "(error_category IN ('transient', 'permanent', 'provider'))"
    ]
  ],
  "notification_dead_letter": [
    ...
    [
      "chk_ndl_status",
      "(status IN ('open', 'retried', 'resolved', 'dismissed'))"
    ]
  ],
  "approval_requests": [
    [
  ...
  "reconciliation_exceptions": [
    [
      "chk_rex_type",
      "(exception_type IN ('wallet_mismatch', 'payment_mismatch'))"
    ],
    [
      "chk_rex_severity",
  ...
  "background_jobs": [
    [
      "chk_bj_status",
      "(status IN ('failed', 'retrying', 'dead_letter', 'resolved'))"
    ],
    [
      "chk_bj_error_category",
      "(error_category IN ('transient', 'permanent', 'provider'))"
    ],
    [
      "chk_bj_attempts_ge_1",
      "(attempts >= 1)"
    ],
    [
      "chk_bj_max_attempts_ge_1",
      "(max_attempts >= 1)"
    ]
  ],
  "contract_templates": [
    [
      "chk_contract_templates_status",
      "(status IN ('active', 'inactive'))"
    ],
    [
      "chk_contract_templates_name",
      "(length(btrim(name)) > 0)"
    ]
  ],
  "contract_template_versions": [
    [
      "chk_contract_template_versions_version_number",
      "(version_number > 0)"
    ]
  ],
  "upload_policies": [
    [
      "chk_upload_policies_category",
      "(category IN ('document', 'image', 'video'))"
    ],
    [
      "chk_upload_policies_extensions",
      "(barghsa_valid_upload_extensions(allowed_extensions))"
    ],
    [
      "chk_upload_policies_max_size",
      "(max_size_bytes BETWEEN 1 AND 104857600)"
    ],
    [
  ...
  "service_due_periods": [
    ...
    [
      "chk_service_due_periods_default_days",
      "(default_days BETWEEN 1 AND 365)"
    ],
    [
  ...
  "invoice_reminder_schedule": [
    [
      "chk_invoice_reminder_schedule_offset",
      "(\"offset\" IN (-7, -3, -1, 0, 1, 7))"
    ],
    [
      "chk_invoice_reminder_schedule_channel",
      "(channel IN ('in_app', 'email', 'sms'))"
    ],
    [
      "chk_invoice_reminder_schedule_status",
  ...
  "invoice_reminder_offset_toggles": [
    ...
    [
      "chk_invoice_reminder_offset_toggles_offset",
      "(\"offset\" IN (-7, -3, -1, 0, 1, 7))"
    ]
  ],
  "wallet_topup_callback_events": [
    [
      ...
      "(status IN ('credited', 'unpaid', 'duplicate'))"
    ]
  ],
...
export function domainChecks(tableName:string) {
  return (definitions[tableName] ?? []).map(([name,expression])=>check(name,sql.raw(expression)))
}

export function domainCheckEntries(tableName:string) {
  return Object.fromEntries(domainChecks(tableName).map((constraint,index)=>[`domainCheck${index}`,constraint]))
}
