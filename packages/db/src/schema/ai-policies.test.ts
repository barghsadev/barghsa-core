...
const TABLES = ['ai_policies', 'ai_policy_groups', 'ai_policy_group_members'] as const;
...
describe('AI policy schema (T-09.11.03)', () => {
  ...
  it('drizzle schema declares UUID FK columns matching migration 0044', () => {
    ...
    const columns = [aiPolicyGroupMembers['groupId'], aiPolicyGroupMembers['policyId']] as Array<{
      getSQLType: () => string;
    }>;
  ...
  it('migration 0044 keeps the membership FK cascade on policy/group delete', () => {
    expect(MIGRATION).toMatch(
      /group_id\s+UUID NOT NULL REFERENCES ai_policy_groups\(id\) ON DELETE CASCADE/
    );
    expect(MIGRATION).toMatch(
      /policy_id\s+UUID NOT NULL REFERENCES ai_policies\(id\) ON DELETE CASCADE/
    );
  });
  ...
  it('migration 0044 keeps the created_by owner FK (TEXT users.user_id, RESTRICT)', () => {
    ...
    expect(MIGRATION).toMatch(
      /created_by\s+TEXT NOT NULL REFERENCES users\(user_id\) ON DELETE RESTRICT/
    );
