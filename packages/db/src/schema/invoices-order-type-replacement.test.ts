...
/**
 * Real-PostgreSQL enforcement tests for the correction-safe unique index
 * (T-04.1.05.02).
 *
 * Runs 0057 + 0064 + 0065 against an isolated schema and proves:
 *   - two ordinary invoices of the same type still collide per order;
 *   - a replacement row (replaces_invoice_id set) of type 'manual' does
 *     not collide with an order-linked original of type 'manual';
 *   - a replacement does not collide with a sibling ordinary manual
 *     invoice on the same order as an auto original;
 *   - cancelled originals still occupy the ordinary uniqueness slot;
...
describe('invoice order-type unique index excludes replacements (T-04.1.05.02)', () => {
  ...
  it('rewrites uq_invoices_order_id_type as a partial unique index', async () => {
    ...
    expect(def.rows[0]!.indexdef).toMatch(/WHERE \(replaces_invoice_id IS NULL\)/i)
  })
  ...
  it('allows a replacement of type manual next to an order-linked original of type manual', async () => {
    ...
    const original = await ctx.db.execute<{ id: string }>(sql`
      ...
      VALUES (uuid_generate_v7(), ${profile}, ${orderId}, 'manual', 100000, 'Cancelled')
      RETURNING id
    `)
    ...
    await expect(
      ctx.db.execute(sql`
        INSERT INTO invoices (
          id, profile_id, order_id, type, total_amount, replaces_invoice_id
        ) VALUES (
          uuid_generate_v7(), ${profile}, ${orderId}, 'manual', 200000, ${originalId}
        )
      `),
    ).resolves.toBeDefined()
  })

  it('allows a replacement next to a sibling ordinary manual invoice on the same order', async () => {
    const orderId = await freshOrder()
    const profile = await profileId()

    const autoOriginal = await ctx.db.execute<{ id: string }>(sql`
      INSERT INTO invoices (id, profile_id, order_id, type, total_amount, state)
      VALUES (uuid_generate_v7(), ${profile}, ${orderId}, 'auto', 100000, 'Cancelled')
      RETURNING id
    `)
    ...
    await expect(
      ctx.db.execute(sql`
        INSERT INTO invoices (id, profile_id, order_id, type, total_amount)
        VALUES (uuid_generate_v7(), ${profile}, ${orderId}, 'manual', 150000)
      `),
    ).resolves.toBeDefined()
    ...
    await expect(
      ctx.db.execute(sql`
        INSERT INTO invoices (
          ...
          uuid_generate_v7(), ${profile}, ${orderId}, 'manual', 200000,
          ${autoOriginal.rows[0]!.id}
        )
      `),
    ).resolves.toBeDefined()
  })

  it('cancelled ordinary originals still block a second ordinary invoice of the same type', async () => {
    const orderId = await freshOrder()
    const profile = await profileId()
  ...
  it('migration 0065 is idempotent — re-running is a no-op', async () => {
    const migrationSql = readFileSync(REPLACEMENT_INDEX_MIGRATION, 'utf-8').trim()
