...
describe('Invoice amount constraints schema (T-04.1.01.04)', () => {
  ...
  it('migration 0052 still declares both named CHECK constraints', () => {
    ...
    expect(MIGRATION).toMatch(
      /ADD CONSTRAINT ck_paid_not_exceeds_total[\s\S]*CHECK \(paid_amount <= total_amount\)/
    );
    expect(MIGRATION).toMatch(
      /ADD CONSTRAINT ck_refund_not_exceeds_paid[\s\S]*CHECK \(refunded_amount <= paid_amount\)/
    );
  });

  it('migration 0052 backfills the non-negative column CHECKs for legacy tables', () => {
    expect(MIGRATION).toMatch(
      /ADD CONSTRAINT ck_invoices_total_amount_nonneg[\s\S]*CHECK \(total_amount >= 0\)/
    );
    expect(MIGRATION).toMatch(
      /ADD CONSTRAINT ck_invoices_paid_amount_nonneg[\s\S]*CHECK \(paid_amount >= 0\)/
    );
    expect(MIGRATION).toMatch(
      /ADD CONSTRAINT ck_invoices_refunded_amount_nonneg[\s\S]*CHECK \(refunded_amount >= 0\)/
    );
  });
...
describe('Invoice correction self-references schema (T-04.1.05.01)', () => {
  ...
  it('migration 0064 adds both columns as nullable UUID self-FKs', () => {
    expect(CORRECTION_MIGRATION).toContain('ADD COLUMN IF NOT EXISTS replaces_invoice_id UUID');
...
const ORDER_TYPE_REPLACEMENT_MIGRATION = readFileSync(
  resolve(__dirname, '../../drizzle/0065_invoice_order_type_unique_exclude_replacements.sql'),
  'utf8'
);
...
describe('Invoice order-type unique index excludes replacements (T-04.1.05.02)', () => {
  it('Drizzle unique index is partial on replaces_invoice_id IS NULL', () => {
    ...
    const chunks = (orderType!.config.where as { queryChunks?: unknown[] } | undefined)
      ?.queryChunks;
  ...
  it('createInvoicesTable SQL declares uq_invoices_id_profile_id', () => {
    const schemaSql = readFileSync(resolve(__dirname, './invoices.ts'), 'utf8');
    expect(schemaSql).toContain('CONSTRAINT uq_invoices_id_profile_id UNIQUE (id, profile_id)');
  });
...
const ORDER_TYPE_ADJUSTMENT_MIGRATION = readFileSync(
  resolve(__dirname, '../../drizzle/0066_invoice_order_type_unique_exclude_adjustments.sql'),
  'utf8'
);
...
const ADJUSTMENT_KIND_MIGRATION = readFileSync(
  resolve(__dirname, '../../drizzle/0067_invoice_adjustment_kind_accounting_amount.sql'),
  'utf8'
);
...
describe('Invoice adjustment kind and accounting amount (T-04.1.05.03)', () => {
  ...
  it('migration 0067 adds generated accounting_amount and kind CHECK', () => {
    ...
    expect(ADJUSTMENT_KIND_MIGRATION).toContain('ck_invoices_adjustment_kind_matches_link');
    expect(ADJUSTMENT_KIND_MIGRATION).toContain('NOT VALID');
    expect(ADJUSTMENT_KIND_MIGRATION).toContain('WHERE adjustment_for_invoice_id IS NOT NULL');
    expect(ADJUSTMENT_KIND_MIGRATION).toContain("WHEN metadata->>'kind' IN ('charge', 'credit')");
  });
