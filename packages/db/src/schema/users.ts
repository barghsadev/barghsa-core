...
import { pgTable, text, boolean, timestamp, integer, uuid, check } from 'drizzle-orm/pg-core';
...
export const users = pgTable(
  ...
  (table) => [
    check(
      'users_theme_mode_check',
      sql`${table.themeMode} IS NULL OR ${table.themeMode} IN ('light', 'dark')`
    ),
  ]
);
