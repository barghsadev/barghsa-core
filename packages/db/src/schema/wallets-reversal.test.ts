...
const TABLE_MIGRATION = resolve(__dirname, '../../drizzle/0068_create_wallet_transactions.sql');
...
describe('wallet_transactions reverses_transaction_id schema (T-04.2.04.01)', () => {
  ...
  it('migration 0074 is registered in the Drizzle journal so migrate() applies it', () => {
    ...
    const entry = journal.entries.find((row) => row.tag === '0074_wallet_tx_reverses_transaction');
    expect(entry).toBeDefined();
    expect(entry!.idx).toBe(74);
    const prior = journal.entries.find((row) => row.tag === '0073_create_idempotency_keys');
  ...
  it('migration 0077 still declares the reversal-original CHECK as NOT VALID', () => {
    ...
    expect(CHECK_MIGRATION).toMatch(/type = 'reversal' AND reverses_transaction_id IS NOT NULL/);
    expect(CHECK_MIGRATION).toMatch(/type <> 'reversal' AND reverses_transaction_id IS NULL/);
  ...
  async function insertLedger(opts: {
    ...
    return ctx.pool.query<{ id: string }>(
      `INSERT INTO wallet_transactions
       ...
       RETURNING id`,
      [walletId, opts.type, opts.amount, opts.idempotencyKey, opts.reversesTransactionId ?? null]
    );
  }
