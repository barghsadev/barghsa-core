...
export const authDeliveryOutbox = pgTable('auth_delivery_outbox', {
  ...
  check('auth_delivery_binding_check', sql`(${table.kind}='otp' AND ${table.challengeId} IS NOT NULL AND ${table.userId} IS NULL) OR (${table.kind}='staff_activation' AND ${table.challengeId} IS NULL AND ${table.userId} IS NOT NULL)`),
