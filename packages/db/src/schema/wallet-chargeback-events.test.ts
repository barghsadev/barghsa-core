import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import { sql } from 'drizzle-orm';
import { getTableConfig } from 'drizzle-orm/pg-core';
import { readFileSync } from 'node:fs';
import { resolve } from 'node:path';
import { createIsolatedTestDb, dropTestSchema } from '../test/testDb';
import type { IsolatedTestDb } from '../test/testDb';
import { walletChargebackEvents } from './wallet-chargeback-events.js';
import { walletTransactions, wallets } from './wallets.js';

const UUIDV7_MIGRATION = resolve(__dirname, '../../drizzle/0000_init_uuidv7_function.sql');
const WALLET_TX_MIGRATION = resolve(__dirname, '../../drizzle/0068_create_wallet_transactions.sql');
const MIGRATION_PATH = resolve(__dirname, '../../drizzle/0075_create_wallet_chargeback_events.sql');
const JOURNAL_PATH = resolve(__dirname, '../../drizzle/meta/_journal.json');
const MIGRATION = readFileSync(MIGRATION_PATH, 'utf8');

describe('wallet_chargeback_events schema (T-04.2.04.02)', () => {
  it('Drizzle table has the event-id unique index and nullable FKs', () => {
    const config = getTableConfig(walletChargebackEvents);
    expect(config.name).toBe('wallet_chargeback_events');
    expect(config.columns.map((column) => column.name)).toEqual([
      'id',
      'event_id',
      'original_transaction_id',
      'reversal_transaction_id',
      'wallet_id',
      'status',
      'match_method',
      'raw',
      'created_at',
    ]);
    const unique = config.indexes.find(
      (idx) => idx.config.name === 'uq_wallet_chargeback_event_id'
    );
    expect(unique).toBeDefined();
    expect(unique!.config.unique).toBe(true);
    const originalFk = config.foreignKeys.find((fk) => {
      const ref = fk.reference();
      return (
        ref.foreignTable === walletTransactions &&
        ref.columns[0]?.name === 'original_transaction_id'
      );
    });
    const walletFk = config.foreignKeys.find((fk) => fk.reference().foreignTable === wallets);
    expect(originalFk?.onDelete).toBe('restrict');
    expect(walletFk?.onDelete).toBe('restrict');
    const byName = Object.fromEntries(config.columns.map((column) => [column.name, column]));
    expect(byName.original_transaction_id?.notNull).toBe(false);
    expect(byName.wallet_id?.notNull).toBe(false);
  });

  it('migration 0075 is idempotent and registered in the journal', () => {
    expect(MIGRATION).toContain('CREATE TABLE IF NOT EXISTS wallet_chargeback_events');
    expect(MIGRATION).toContain('CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_chargeback_event_id');
    expect(MIGRATION).toContain(
      "CHECK (status IN ('processing', 'reversed', 'unmatched', 'unresolved', 'duplicate'))"
    );
    const journal = JSON.parse(readFileSync(JOURNAL_PATH, 'utf8')) as {
      entries: Array<{ tag: string; idx: number; when: number }>;
    };
    const entry = journal.entries.find((row) => row.tag === '0075_create_wallet_chargeback_events');
    expect(entry).toBeDefined();
    expect(entry!.idx).toBe(75);
    const prior = journal.entries.find((row) => row.tag === '0074_wallet_tx_reverses_transaction');
    expect(prior).toBeDefined();
    expect(entry!.when).toBeGreaterThan(prior!.when);
  });
});

describe('wallet_chargeback_events PostgreSQL enforcement (T-04.2.04.02)', () => {
  let ctx: IsolatedTestDb;
  const profileId = 'aaaaaaaa-aaaa-7aaa-8aaa-aaaaaaaaaaaa';

  beforeAll(async () => {
    ctx = await createIsolatedTestDb();
    await ctx.pool.query(readFileSync(UUIDV7_MIGRATION, 'utf-8').trim());
    await ctx.pool.query(`
      CREATE TABLE IF NOT EXISTS profiles (
        id UUID PRIMARY KEY DEFAULT uuid_generate_v7()
      )
    `);
    await ctx.pool.query(readFileSync(WALLET_TX_MIGRATION, 'utf-8').trim());
    await ctx.pool.query(readFileSync(MIGRATION_PATH, 'utf-8').trim());
    await ctx.pool.query(`INSERT INTO profiles (id) VALUES ($1)`, [profileId]);
    await ctx.pool.query(`INSERT INTO wallets (profile_id) VALUES ($1)`, [profileId]);
  }, 60_000);

  afterAll(async () => {
    await ctx.pool.end();
    await dropTestSchema(ctx.schemaName);
  });

  it('rejects a duplicate event_id', async () => {
    await ctx.db.execute(sql`
      INSERT INTO wallet_chargeback_events (event_id, status, raw)
      VALUES ('evt-dup', 'unmatched', '{}'::jsonb)
    `);
    await expect(
      ctx.db.execute(sql`
        INSERT INTO wallet_chargeback_events (event_id, status, raw)
        VALUES ('evt-dup', 'unmatched', '{}'::jsonb)
      `)
    ).rejects.toMatchObject({ cause: { code: '23505' } });
  });

  it('accepts an unmatched event with null original and wallet FKs', async () => {
    await expect(
      ctx.db.execute(sql`
        INSERT INTO wallet_chargeback_events (event_id, status)
        VALUES ('evt-unmatched', 'unmatched')
      `)
    ).resolves.toBeTruthy();
  });

  it('rejects an unknown status or match_method', async () => {
    await expect(
      ctx.db.execute(sql`
        INSERT INTO wallet_chargeback_events (event_id, status)
        VALUES ('evt-bad-status', 'credited')
      `)
    ).rejects.toMatchObject({ cause: { code: '23514' } });
    await expect(
      ctx.db.execute(sql`
        INSERT INTO wallet_chargeback_events (event_id, status, match_method)
        VALUES ('evt-bad-method', 'processing', 'guess')
      `)
    ).rejects.toMatchObject({ cause: { code: '23514' } });
  });

  it('stores a mapped processing claim against a Completed top-up', async () => {
    const tx = await ctx.pool.query<{ id: string }>(
      `INSERT INTO wallet_transactions
         (wallet_id, type, amount, state, idempotency_key)
       VALUES ($1, 'topup', 1000, 'Completed', 'cb-schema-mapped')
       RETURNING id`,
      [profileId]
    );
    await expect(
      ctx.db.execute(sql`
        INSERT INTO wallet_chargeback_events
          (event_id, original_transaction_id, wallet_id, status, match_method)
        VALUES (
          'evt-mapped',
          ${tx.rows[0]!.id}::uuid,
          ${profileId}::uuid,
          'processing',
          'merchant_order_id'
        )
      `)
    ).resolves.toBeTruthy();
  });
});
