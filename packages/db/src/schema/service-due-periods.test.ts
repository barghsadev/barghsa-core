...
const MIGRATION_PATH = resolve(__dirname, '../../drizzle/0059_create_service_due_periods.sql');
...
describe('service_due_periods schema (T-04.1.03.01)', () => {
  ...
  it('migration 0059 is idempotent (matching sibling migrations)', () => {
    ...
    expect(MIGRATION).toContain('DROP TRIGGER IF EXISTS trg_service_due_periods_updated_at');
  ...
  it('migration 0059 is registered in the Drizzle journal so migrate() applies it', () => {
    ...
    const entry = journal.entries.find((row) => row.tag === '0059_create_service_due_periods');
