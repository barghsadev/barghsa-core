...
describe('integration test harness', () => {
  ...
  it('connects to a real PostgreSQL server', async () => {
    const result = await ctx.db.execute<{ version: string }>(sql`SELECT version() AS version`);
  ...
  it('enforces PostgreSQL constraints (unique / not-null)', async () => {
    ...
    await expect(
      ctx.db.execute(sql`INSERT INTO _constraint_probe (id, email) VALUES (2, 'a@example.test')`)
    ).rejects.toThrow();
  ...
  it('uses row-level locking available in real PostgreSQL', async () => {
    ...
    await ctx.db.execute(sql`CREATE TABLE _lock_probe (id int PRIMARY KEY, v int)`);
    await ctx.db.execute(sql`INSERT INTO _lock_probe (id, v) VALUES (1, 100)`);

    const locked = await ctx.db.execute(sql`SELECT id, v FROM _lock_probe WHERE id = 1 FOR UPDATE`);
...
describe('schema isolation across workers', () => {
  it('creates a unique schema each call', async () => {
    ...
    try {
      ...
      await a.db.execute(sql`CREATE TABLE _iso_probe (id int PRIMARY KEY, v text)`);
