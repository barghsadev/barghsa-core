...
afterEach(async()=>{vi.restoreAllMocks();vi.unstubAllEnvs();await fixture?.close()})
...
it('requires explicit complete credentials and a valid password before creating the first admin',async()=>{
  ...
  for(const key of ['ADMIN_BOOTSTRAP_KEY','ADMIN_BOOTSTRAP_SECRET','ADMIN_BOOTSTRAP_EMAIL','ADMIN_BOOTSTRAP_PASSWORD'])vi.stubEnv(key,'')
...
it('creates one admin across concurrent identities, hashes credentials, audits once and emits no secret',async()=>{
  ...
  const admins=(await fixture.pool.query('SELECT username,password_hash,is_admin,is_staff,must_change_password FROM users')).rows
  ...
  const audit=(await fixture.pool.query("SELECT metadata FROM audit_log WHERE event='admin_bootstrapped'")).rows
  expect(audit).toHaveLength(1)
  const output=JSON.stringify([stderr.mock.calls,stdout.mock.calls,results,audit])
  for(const secret of [password,'fixture-secret','fixture-key'])expect(output).not.toContain(secret)
})

it('never replaces a disabled administrator or turns an existing customer into an admin',async()=>{
  await fixture.pool.query("INSERT INTO users(user_id,username,password_hash) VALUES ('customer','first@example.test','test')")
  expect((await seedAdmin(fixture.db,false)).errors).toHaveLength(1)
  expect((await fixture.pool.query("SELECT is_admin FROM users WHERE user_id='customer'")).rows[0].is_admin).toBe(false)
  await fixture.pool.query("INSERT INTO users(user_id,username,password_hash,is_admin,disabled_at) VALUES ('existing','prior@example.test','test',true,NOW())")
  vi.stubEnv('ADMIN_BOOTSTRAP_EMAIL','replacement@example.test')
  expect(await seedAdmin(fixture.db,false)).toMatchObject({created:0,skipped:1,errors:[]})
  expect((await fixture.pool.query('SELECT count(*)::int AS count FROM users WHERE is_admin')).rows[0].count).toBe(1)
})

it('rolls back account creation on audit failure and permits a clean retry',async()=>{
  await fixture.pool.query(`CREATE FUNCTION fail_bootstrap_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected audit failure'; END $$;
  ...
  expect(result.created).toBe(0);expect(result.errors).toHaveLength(1)
  expect((await fixture.pool.query('SELECT user_id FROM users')).rows).toEqual([])
  await fixture.pool.query('DROP TRIGGER fail_bootstrap_audit ON audit_log; DROP FUNCTION fail_bootstrap_audit()')
