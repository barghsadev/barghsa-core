...
export const NOTIFICATION_TEMPLATE_SEED: SeedEventDefinition[] = [
  ...
  },
  {
    ...
    faSubject: null,
    enSubject: null,
    faBody: 'آیتم «{{item_id}}» ({{service_type}}) بیش از {{target_hours}} ساعت بدون پاسخ مانده است. لطفاً بررسی کنید.',
    enBody: 'Item {{item_id}} ({{service_type}}) has been awaiting response for over {{target_hours}} hours. Please review.',
    channels: ['in_app'],
    variables: [
      { name: 'service_type', description: 'نوع خدمت / Service type' },
