...
try:
 ...
 time.sleep(2)
 result=subprocess.run(['docker','run','--rm','--name',runner,'--network',network,'--user','postgres','-e','BACKUP_S3_ENDPOINT=http://storage:9000','-e','BACKUP_ALLOW_HTTP=true','-e','BACKUP_S3_BUCKET=audit-backups','-e','BACKUP_S3_ACCESS_KEY=audit-access','-e','BACKUP_S3_SECRET_KEY=audit-secret-local-only','-v',str(Path(__file__).resolve().parent)+':/opt/backup:ro','-w','/opt/backup','barghsa-backup-audit:local','python3','-B','-m','unittest','-v',*(sys.argv[1:] or ['test_physical_backup', 'test_verify_restore'])], timeout=600)
