...
add_result_field() {
  local key="$1"
  local value="$2"
  RESULT_JSON=$(echo "$RESULT_JSON" | jq --arg k "$key" --arg v "$value" '. + {($k): $v}' 2>/dev/null || echo '{}')
}
...
find_pg_bin() {
  ...
  if command -v pg_config &>/dev/null; then
    pg_config --bindir 2>/dev/null || echo ""
    return 0
  fi
...
compute_rpo() {
  ...
  now=$(date -u +%s)
  local backup_ts
  backup_ts=$(date -u -d "$BACKUP_TIMESTAMP" +%s 2>/dev/null || echo "0")
...
step_restore() {
  ...
  RESTORE_DIR="$WORK_DIR" "${RESTORE_SCRIPT}" --target-dir "$WORK_DIR" --latest

  # Capture the backup label from the restore script's output
  BACKUP_LABEL=$(grep -oP 'Backup:\s+\K.+' <<< "$RESTORE_OUTPUT" 2>/dev/null || echo "unknown")
...
step_start_postgres() {
  ...
  # Configure minimal postgresql.conf for temporary instance
  local conf_file="${WORK_DIR}/data/postgresql.conf"
  {
    ...
    # Enable recovery if WAL is present and recovery.signal exists
    if [[ -f "${WORK_DIR}/data/recovery.signal" ]]; then
      echo "# Recovery mode enabled (recovery.signal present)"
    fi
  } >> "$conf_file"
...
step_measure_performance() {
  ...
  # Record results
  add_result_field "rto_seconds" "$rto"
  add_result_field "rto_status" "$rto_status"
  add_result_field "rpo_seconds" "$rpo"
