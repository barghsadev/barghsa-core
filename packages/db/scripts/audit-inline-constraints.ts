...
async function main() {
  ...
  try {
    const result=await runMigrations({connection:{pgdirectUrl:process.env.TEST_DATABASE_URL}})
    ...
    const indexes=(await pool.query(`SELECT t.relname AS "table",i.relname AS name,pg_get_indexdef(i.oid) AS definition
      ...
      JOIN pg_namespace n ON n.oid=t.relnamespace WHERE n.nspname='public' AND x.indisunique`)).rows
    const normalize=(value:string)=>value.replace(/[\s"]/g,'').toLowerCase()
    const report=expected.map(item=>{
      const match=actual.find(row=>row.table===item.table&&(row.name===item.name||normalize(row.definition)===normalize(item.definition)))
      const index=item.definition.startsWith('UNIQUE')?indexes.find(row=>row.table===item.table&&row.name===item.name):undefined
      return {...item,actual:match?.definition??index?.definition??null,actualName:match?.name??index?.name??null}
    })
    writeFileSync(path,JSON.stringify(report,null,2)+'\n')
    console.log(JSON.stringify({checked:report.length,missing:report.filter(item=>!item.actual)},null,2))
  } finally {await pool.end();await teardown()}
}
