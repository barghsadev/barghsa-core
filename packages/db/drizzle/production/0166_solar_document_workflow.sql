CREATE TABLE "solar_document_requests" (
	"id" uuid PRIMARY KEY DEFAULT uuid_generate_v7() NOT NULL,
	"request_id" uuid NOT NULL,
	"description" text NOT NULL,
	"requested_by" text NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "solar_document_requests_description" CHECK (length(trim("solar_document_requests"."description")) BETWEEN 1 AND 2000)
);
--> statement-breakpoint
ALTER TABLE "solar_document_requests" ADD CONSTRAINT "solar_document_requests_request_id_solar_construction_requests_id_fk" FOREIGN KEY ("request_id") REFERENCES "public"."solar_construction_requests"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "solar_document_requests" ADD CONSTRAINT "solar_document_requests_requested_by_users_user_id_fk" FOREIGN KEY ("requested_by") REFERENCES "public"."users"("user_id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "solar_document_requests_request_idx" ON "solar_document_requests" USING btree ("request_id","created_at","id");
--> statement-breakpoint
  ...
  IF TG_OP='INSERT' THEN
    ...
    IF NEW.supersedes_document_id IS NOT NULL THEN
      ...
      IF NOT FOUND OR predecessor.profile_id<>NEW.profile_id
        ...
        OR (predecessor.state NOT IN ('Available','Approved','Rejected') AND NOT (NEW.business_record_type='solar_request' AND predecessor.state='SubmittedForReview')) THEN
...
END $$;
--> statement-breakpoint
CREATE FUNCTION guard_solar_document_link() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP='DELETE' THEN
    RAISE EXCEPTION 'Solar document links are retained for audit' USING ERRCODE='23514';
  END IF;
  IF TG_OP='INSERT' AND NOT EXISTS (
    SELECT 1 FROM documents d WHERE d.id=NEW.document_id
      AND d.business_record_type='solar_request' AND d.business_record_id=NEW.request_id
      AND d.original_name=NEW.file_name AND d.uploaded_by=NEW.uploaded_by
  ) THEN
    RAISE EXCEPTION 'Solar document must match its request and upload' USING ERRCODE='23514';
  END IF;
  IF TG_OP='UPDATE' AND ROW(NEW.id,NEW.request_id,NEW.document_id,NEW.file_name,NEW.uploaded_by,NEW.uploaded_at)
    IS DISTINCT FROM ROW(OLD.id,OLD.request_id,OLD.document_id,OLD.file_name,OLD.uploaded_by,OLD.uploaded_at) THEN
    RAISE EXCEPTION 'Solar document identity is immutable' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;
--> statement-breakpoint
CREATE TRIGGER solar_document_link_guard BEFORE INSERT OR UPDATE OR DELETE ON solar_construction_documents
  FOR EACH ROW EXECUTE FUNCTION guard_solar_document_link();
