-- Preserve document lineage while allowing staff to replace a quarantined contract original.
...
ALTER TABLE documents DROP CONSTRAINT documents_ready_content;
--> statement-breakpoint
ALTER TABLE documents ADD CONSTRAINT documents_ready_content CHECK (
  state IN ('Uploading','PendingScan','Quarantined','Removed') OR
  (storage_key IS NOT NULL AND detected_mime IS NOT NULL AND checksum IS NOT NULL AND
    (scan_state='Available' OR (state='Superseded' AND scan_state='Quarantined')))
);
