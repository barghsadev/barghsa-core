...
function request(path,options={}) {
  return new Promise((resolve,reject)=>{
    ...
    const req=transport.request({host:'127.0.0.1',port:options.plain?80:443,path,rejectUnauthorized:false,
      servername:'barghsa.example.com',method:options.body?'POST':'GET',headers:options.headers,...options.tls},res=>{
      let body='';const chunks=[]
      res.on('data',chunk=>{body+=chunk;chunks.push(String(chunk))})
...
for(const version of ['TLSv1.2','TLSv1.3']) {
  assert.equal((await request('/api/ping',{tls:{minVersion:version,maxVersion:version}})).status,200)
}
const api=await request('/api/ping',{headers:{'X-Forwarded-For':'203.0.113.99','X-Real-IP':'203.0.113.99'}})
...
assert.equal((await request('/api/large',{body:Buffer.alloc(10*1024*1024+1)})).status,413)
...
await new Promise((resolve,reject)=>{
  const req=https.request({host:'127.0.0.1',port:443,path:'/api/ai/socket',rejectUnauthorized:false,
  ...
  req.on('upgrade',(res,socket)=>{try{assert.equal(res.statusCode,101);socket.destroy();resolve()}catch(error){reject(error)}})
...
for(const path of ['/api/auth/login','/api/upload/file','/api/ai/stream']) {
  const responses=await Promise.all(Array.from({length:60},()=>request(path,{headers:{'Accept-Language':'fa'}})))
...
console.log('PASS TLS, routing, forwarded-address overwrite, headers, size limit, SSE and WebSocket upgrade')
