...
test('common dependencies count once, an oversized common chunk fails, and missing routes fail closed', async () => {
  ...
  try {
    ...
    const manifest = { 'index.html': { file: 'entry.js', imports: ['common'] }, login: { file: 'login.js', imports: ['common', 'index.html'] }, common: { file: 'common.js' } }
    const contents = { 'entry.js': 'export const entry=1;', 'login.js': 'export const login=1;', 'common.js': 'export const common=1;' }
    for (const [file, content] of Object.entries(contents)) await writeFile(join(dist, file), content)
    ...
    assert.equal(measured.bytes, Object.values(contents).reduce((sum, content) => sum + gzipSync(content).length, 0))
