...
### Block TOS writes when history cannot be trusted

The editor previously offered draft creation after a failed history request and crashed on malformed records. History now validates record shape, dates, identity uniqueness and single draft/active invariants. Writes stay disabled until a valid reload. A localized retry control recovers the page, and request sequencing prevents an older response from replacing a newer load. Empty-history messaging appears only after a successful read.

Both new browser failures reproduced before repair. Eight focused production Chromium checks pass, including outage/malformed recovery, detail keyboard focus and English/Persian error dismissal. Web build/types, targeted lint, formatting, bundle budgets and diff review pass. The complete API regression for the preceding TOS repairs also passes: 3,080 tests across 233 files. Rich-text editing, preview/comparison and complete editor localization remain open.

### Add bilingual rich-text TOS editing without rewriting unsupported content

The administrator editor now offers headings, bold/italic, lists, quotations and undo/redo through pinned Tiptap 3.31.3 packages. Persian and English inputs have explicit direction, language and accessible names. The editor loads only when opened and preserves Markdown storage. Save freezes content and version controls and prevents duplicate submissions; failures retain the draft. Documents containing unsupported table/image/HTML/task-list/footnote constructs retain source editing to avoid dropping content. Disabling the editor does not serialize or rewrite its document.

Six focused production Chromium checks pass, covering both languages, formatting persistence, duplicate submission, failed-save recovery, history recovery and dialog focus. The source-preservation check passes again after broadening the fallback detector. Web build/types, targeted lint, formatting, bundle budgets and diff review pass. Preview, comparison, publication revision binding and full page localization remain open. Integration follows the [Tiptap React guide](https://tiptap.dev/docs/editor/getting-started/install/react) and [Markdown API](https://tiptap.dev/docs/editor/markdown/getting-started/basic-usage).

### Bind TOS publication to a rendered draft preview

TOS responses now include a content revision. Publication requires that revision and compares it with the locked draft before changing the active version or writing the audit. A stale preview returns 409; a missing revision returns 400. The editor displays current and proposed content side by side, with bounded line-diff highlighting and an explicit material-change checkbox. Publish waits for the preview to render. A conflict disables the old preview until it is closed and history reloaded. Markdown previews escape HTML, retain safe URL handling and do not fetch embedded images.

The stale-preview HTTP regression published changed content before repair. All 26 focused API checks pass afterward. Eleven production Chromium checks pass, including a complete create/reload/preview/publish/public-read flow against the migrated API, unsafe-markup handling and stale-preview recovery. The captured desktop preview was visually reviewed. API/web types, targeted lint, formatting, bundle budgets, generated OpenAPI comparison and diff review pass. Full editor localization, response confirmation, edit-conflict handling and published-content rendering remain open; this step does not close overall TOS acceptance.

### Reject stale TOS edits and discard, and confirm write responses

Draft edit and discard now require the revision originally opened by the editor and compare it under the same transaction lock. A newer draft cannot be overwritten or deleted by a stale request. The browser captures that revision, retains local content after an edit conflict and provides an explicit reload of the saved draft. Discard uses the captured revision and blocks parallel actions. Create/edit/publication responses must match the requested identity, content and state before the interface treats them as confirmed. Malformed success responses retain the draft and block further writes until history is reloaded.

The stale-edit HTTP overwrite reproduced before repair. All 27 focused API checks and 13 production Chromium checks pass, including the migrated API workflow, explicit conflict reload and malformed creation response. API/web types, targeted lint, formatting, bundle budgets, generated contract comparison and diff review pass. Full page localization, published Markdown rendering and broader acceptance review remain open.

### Render published TOS consistently and keep route payloads within budget

Public terms, registration consent, re-acceptance, administrator preview and version history now share a Markdown renderer. It preserves headings, emphasis, lists and tables, escapes embedded HTML/image markup and sanitizes generated HTML with an explicit element/attribute allowlist. Re-acceptance waits until the document renderer has loaded. Integration uses pinned Marked 18.0.11 and DOMPurify 3.4.15, following [Marked extension guidance](https://github.com/markedjs/marked/blob/master/docs/USING_PRO.md) and [DOMPurify configuration](https://github.com/cure53/DOMPurify).

The first shared renderer raised ordering payload to 297.60 KB against the unchanged 250 KB gate. The compact parser reduced it to 274.59 KB. Administrator and CRM dictionaries now load through their own consumers, removing 767 administrator and 160 CRM entries per language from customer routes. All 1,832 original translations per language were compared before/after and preserved exactly. Final ordering payload is 247.78 KB and every route budget passes.

All 291 production Chromium checks passed after rendering and dictionary extraction. Nine focused consent/public-page checks passed after correcting the loading label and adding delayed-renderer coverage. The full API regression passes 3,082 tests across 233 files; all 31 i18n checks pass, including namespace parity, placeholders and prototype-safe fallback. Root types, lint, formatting and diff review pass. Current browser inventory is 292 checks; no new combined coverage or production-image claim is made. TOS safe-entry exemptions and complete administrator page localization remain open.

### Keep TOS automatic review off support and legal-record entry points

The re-acceptance banner no longer automatically opens a modal on ticket/support and invoice/contract record routes, including the implemented staff ticket/invoice screens. Manual review remains available. Moving into an exempt route closes an automatic review, and request sequencing prevents an older response from replacing a newer review. Missing identity or malformed document content never enables acceptance.

Four browser regressions reproduced automatic modal interference on customer tickets, invoice list/detail and staff tickets. All ten focused production Chromium checks pass afterward, including manual review on those routes, normal automatic review, delayed rendering, account-local dates and malformed-document rejection. Web build/types, targeted lint, formatting, bundle limits and diff review pass. This does not certify future contract routes or all recovery workflows.

### Localize the complete TOS editor and preserve readable failure states
...
### Apply published numeral preferences to wallet and invoice displays

Wallet balances, dashboard balance/toman previews, customer invoices, receipt review, approval amounts and the wallet-limit editor now use a shared formatting hook driven by published appearance settings. IRR currency labels follow page language while digits follow the selected preference. Invalid or already-lossy money is displayed as unavailable instead of a rounded balance. Inputs retain their canonical numeric submission values when display digits change.

Both new browser cases reproduced ignored preferences before the consumer changes. All 17 focused production Chromium checks pass afterward, covering amounts above the safe-number range, currency labels, both languages, live preference refresh, localized input normalization, exact submissions and unavailable display for unsafe numeric balances. Both migrated-API dashboard flows also pass. All 149 web unit checks, web types, targeted lint, formatting, route budgets and diff review pass. Other administrator numeric consumers remain the next step.

### Apply numeral preferences to catalogue, VAT and gift-code administration
...
### Apply numeral preferences to administrator counts, sizes and versions
...
### Recover onboarding geography failures without losing identity fields

Both onboarding forms now validate province, city and company-type lists before rendering. Failed HTTP responses, malformed entries, duplicate IDs and cities from another province produce retryable errors. Required geography failures/loading block submission; selected IDs must belong to the loaded options. Typed fields and legal drafts remain intact. Requests are cancelled and their results ignored after a province change or clear, preventing obsolete city lists from returning.

All four new recovery cases failed against the preceding production bundle, including the reproduced legal-form crash. All 19 focused browser checks now pass, covering both locales, successful individual/legal submissions, autosave and restored geography, company-type recovery, malformed options and numeral counters. Six older browser fixtures omitted the real API's provinceId field; those fixtures were corrected before the clean rerun. All 151 web unit checks pass, including two late-response races. Web types, targeted lint, formatting, route budgets and diff review pass.

### Preserve general rate quotas across Redis loss and recovery

Every general rate-limit request now records its PostgreSQL counter before Redis is checked. Redis can further restrict admission; failed Redis reads use the already-recorded result without incrementing PostgreSQL twice. Missing/deleted Redis counters, new clients and mixed connected/degraded instances cannot grant another durable quota. A failed PostgreSQL write refuses admission even with healthy Redis. Retry timing follows whichever counter denies the request.

Both new unit regressions failed before repair. All 934 shared tests pass, including 83 rate-limit checks. Seven tests against real Redis and a production-migrated PostgreSQL database pass, covering concurrent admission, expiry, legacy counters, loss and recovery. The focused login/proxy/contact HTTP checks also pass. Shared types, targeted lint, formatting and diff review pass. Each general request now adds a PostgreSQL write; load capacity needs measurement. Fixed-window boundaries and traffic already lost before rollout remain limitations, so this does not close sliding-window acceptance or all F23 work.
...
### Repair PostgreSQL 17 monitoring queries and failed-read reporting

The database collector now reads checkpoint counters from pg_stat_checkpointer and uses PostgreSQL 17's shared block timing columns in pg_stat_statements. Query rankings are scoped to the current database. Removed backend-buffer fields remain unavailable instead of fabricated zeros. Required database/activity/query-state/settings reads must succeed; pool absence, invalid bounds or missing required data returns structured failure. An absent optional query extension returns null, distinct from a successfully empty query list.

Eight initial regressions failed before repair. A further enabled-extension test reproduced the separate renamed query-timing columns. All 617 database checks across 82 files pass after both fixes, including ten monitoring cases against PostgreSQL 17 with the extension both disabled and enabled. Root types passed after the interface change; final database types, targeted lint, formatting and diff review pass. References: [PostgreSQL 17 monitoring changes](https://www.postgresql.org/docs/17/release-17.html) and [query-statistics columns](https://www.postgresql.org/docs/17/pgstatstatements.html). Prometheus stale-value clearing and collection-failure visibility are the next step; production monitoring and the full infrastructure acceptance task remain unverified.

### Clear stale Prometheus samples and expose collection failures

Prometheus now publishes a core collection-success gauge and explicit optional-view availability. Failed or thrown collection clears the last snapshot and removes old database samples. Unavailable WAL, checkpoint and replication measurements are removed instead of reset to zero. Concurrent timer/scrape polls share one in-flight collection, and shutdown waits for it. An alert rule now detects sustained collection failure.

All four service regressions failed before repair. The first implementation exposed prom-client's reset behavior for unlabeled gauges, which creates zero samples; those samples are now removed explicitly. All five final service/HTTP checks pass. The HTTP test uses the actual compiled application and a migrated disposable database: it disallows new connections, terminates only the application's test connections, verifies a failed-collection scrape without stale values, restores connections, then verifies recovery. Initial fixture corrections used the actual /metrics route and a separate control database for ALTER DATABASE. Root types passed after the service changes; final API types, targeted lint, formatting and diff review pass. The alert rule is defined locally; no monitoring deployment or real alert-delivery exercise is claimed.

### Report worst streaming-replica lag without a false healthy zero
...
### Wire aggregate database metrics through OpenTelemetry

The API now creates a private OpenTelemetry MeterProvider when an explicit metrics endpoint is configured. It exports aggregate connection, cache, query-call, scan, transaction, tuple, deadlock and replica-lag measurements over OTLP/HTTP JSON. It does not export SQL text or customer identifiers. Intervals/timeouts are validated, shutdown is bounded, and ordinary HTTP fixtures explicitly disable external export unless supplied a local test receiver.

The initial live collector test failed because no export existed. A subsequent failure/recovery test exposed retained cumulative SDK observations; the exporter now filters instruments against the current collection, preventing stale values from being emitted after a failed snapshot. All sixteen final monitoring/OTLP checks pass, including a collector that rejects its first request, actual API delivery, current-value assertions, failed and throwing snapshots, recovery, invalid configuration and optional export. Root types, targeted lint, formatting, frozen-lockfile installation and diff review pass. Pinned SDK dependencies are recorded in the lockfile. Operational setup and limitations are documented in docs/operations/postgresql-monitoring.md. No production collector or alert destination was changed.
...
### Preserve localized unread counts across title changes
...
### Rebuild and exercise current production images
...
### Repair verification-settings contrast in both themes

The verification page now uses theme-aware text, controls and its own background, preserving readability despite the older admin shell background. Its selected option previously measured 4.44:1 for small text in light mode, below 4.5:1; dark-theme text also conflicted with the shell’s forced light background. All four initial language/theme contrast cases failed before repair.

All six verification browser checks pass after repair, including failed-read protection, unavailable automatic verification, save failure/mismatch recovery and measured contrast through initial/error/success states. The four final contrast cases also require zero unresolved axe measurements. English light and Persian dark screenshots were visually reviewed. Build, web types, targeted lint, formatting, frozen-lockfile install and diff review pass. @axe-core/playwright 4.13.0 is pinned as a web development dependency. This covers the verification page under the tested theme configuration; shared admin-shell and other-page theme work remains open. Production image and full-suite checkpoints predate this change.

### Restore administrator navigation contrast
...
### Review eight build and test infrastructure tasks
...
### Record five database foundation acceptance reviews
...
### Close localized DatePicker task acceptance

Reviewed the exact canonical T-06.03.04 requirements against the shared picker, calendar, timestamp helpers and all five product consumers. Added Persian half-open range checks in both themes: selecting the second Farvardin day retains the first included Tehran midnight and the next excluded midnight, announces that exclusion and highlights only included days. Ten scoped dialog accessibility scans across both themes and five browser profiles found zero violations. The generated Persian mobile calendar screenshot was inspected; the calendar remains within its 390px viewport.
