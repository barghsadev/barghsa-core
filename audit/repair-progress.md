...
The verification page now uses theme-aware text, controls and its own background, preserving readability despite the older admin shell background. Its selected option previously measured 4.44:1 for small text in light mode, below 4.5:1; dark mode also rendered dark text against dark surfaces. All four initial language/theme contrast cases failed before repair.
