...
Final review also reproduced COMMIT inheriting the last read's deadline. Transaction commands now receive their write deadline as well, with a real-server regression and continued cancellation/rollback checks. The focused timeout/cancellation suite passes all 10 tests; affected package coverage is being refreshed.

The broader refresh exposed a duplicate-cancellation race: a client CancelRequest sent at the automatic server deadline could cancel the following statement after PostgreSQL had already timed out the first. Automatic mode now leaves active-query cancellation to its installed server timeout, retaining the client guard for queued work. The policy regression checks the server timeout reason and successful following write. Explicit numeric client-cancellation overrides keep their existing behavior; live cancellation transport/proxy behavior remains outside this local evidence.
