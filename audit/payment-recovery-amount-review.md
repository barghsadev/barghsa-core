...
Validation correction: the package-level API lint command in the initial amount-repair step had no matching script and did not run lint. Direct ESLint subsequently passed for the gateway, its tests and the entire API source during the terms-dependency review. The recorded 99 runtime tests and API typecheck remain valid.
