...
Current through product/test `69b9e1a`, 2026-09-09. This is the only active plan. [progress.json](progress.json) tracks the next action and all 23 original F01–F23 groups. [acceptance-closure.json](acceptance-closure.json) owns historical task status. Archived plans are evidence, not instructions.

## Current position

| Population | Recorded verified | Partial | Pending | Total |
| --- | ---: | ---: | ---: | ---: |
| Tasks with merged PR evidence | 126 | 32 | 105 | 263 |
| Other historical claims | 3 | 0 | 56 | 59 |
| All claims | 129 | 32 | 161 | 322 |
...
Active R01 batch: staff permission audit timeline, PR219, `02-auth-users-admin.md#T-10.01.02`. Exact requirement and saved PR body read. Review role additions/removals, user/date filters and current API authorization; resolve both historical timeline/UI deferrals. Dashboard invitation PR137/task verified at69b9e1a:2 production browser cases, scoped lint and unchanged invitation source/evidence reuse pass. [Widget review](evidence/step-reviews.json#R01-invitation-dashboard).

Verification batch consolidated at709d28b:3 task acceptances partial/3 saved PR reviews open. Mode Draft→Active at1858caf passes90 backend/18 browser cases. Named actionable notices/current-owner dashboard/inbox atd74c8f8 pass68 backend/6 browser cases. Circuit recovery at709d28b passes29 distinct provider cases across focused runs. API/web types, lint/format, build, OpenAPI and42 unchanged budgets pass.2154 logs indexed;22 further current bindings refreshed. [Review](evidence/step-reviews.json#R01-verification-policy). Current saved PR totals106 closed/23 open/172 unreviewed.
...
| Current mapping | PRs |
| --- | ---: |
| At least one unresolved mapped task | 145 |
| Only recorded verified tasks | 152 |
...
Review the combined implementation once per qualified task, then associate every contributing PR. The 138 unresolved PR-backed tasks comprise infrastructure 50, auth/admin 45, core business 4, finance 7, notifications 29 and UI foundations 3. Another 56 unresolved claims have no direct PR mapping.
...
Task dependencies override batch order. These55 overlap the193 unresolved reviews, so do not add the counts. Review incidental implementation before scheduling a build.
