...
Refreshed 2026-09-08 through product commit `af68316`. This replaces the previous active plan. Read [progress.json](progress.json) for the next action and original F01–F23 group records. [Task acceptance](acceptance-closure.json) remains the only task-status authority.

## Current numbers

| Population | Verified | Partial | Pending review | Total |
| --- | ---: | ---: | ---: | ---: |
| Tasks with merged PR evidence | 39 | 18 | 206 | 263 |
| Other historical claims | 3 | 0 | 56 | 59 |
| Combined | 42 | 18 | 262 | 322 |

There are **280 unresolved task reviews**, including the 18 partial reviews. Unresolved review does not establish that a task is unbuilt. The records do not support a reliable percentage of implementation effort remaining.
...
- CRM: finish T-05.01.01 CRM list requirements; T-05.02.01 full profile view is verified. T-05.02.06 local archive guards, checklist, acknowledgements, blocker details, ownership and retained records pass. Contract integration and approved retention policy remain explicit prerequisites. T-05.02.02/.03/.04/.05 are verified locally.
...
| Disposition | Count |
| --- | ---: |
| PRs with unresolved mapped tasks | 257 |
| PRs mapping only to verified tasks | 40 |
| PRs with no current task mapping | 4 |
| Historical deferral statements | 170 across 101 PRs |

Review current combined implementation once per qualified task and associate all contributing PRs. Verified mappings do not automatically resolve separate PR deferrals. The 224 unresolved PR-backed tasks comprise infrastructure 50, auth/admin 86, core business 4, finance 52, notifications 29 and UI foundations 3. Another 56 unresolved claims have no direct PR mapping.
...
These are review batches. Build only the unmet remainder after checking dependencies. The 55 pending skips overlap the 280 unresolved reviews; do not count twice. [Earlier gaps](archive/queue-gaps.json) and [unrecorded backlog](archive/unstarted-backlog.csv) remain historical inputs, not a dispatch queue.
