...
The affected-package refresh exposed one real cancellation race in the new automatic mode: a duplicate client CancelRequest could arrive after server timeout and cancel the following write. Automatic mode now relies on its installed server deadline for active queries and retains the client guard for queued queries. Explicit numeric cancellation behavior remains unchanged. All 14 focused timeout/cancellation/health tests pass; the policy test also asserts the server's statement-timeout error. Affected coverage will be refreshed again. This was a runtime correction, not a test-timing relaxation.
