...
Branch codex/audit-fixes. Product/test HEAD **b27f4258**. Active batch **R02-notification-outbox-delivery**, saved PR163–170 and203. Seven confirmed defects are repaired locally. Latest work: scoped staff delivery history at8349c015, tested accumulation rules/runbook ate8e30195, and accepted-receipt preservation after transient persistence rollback atb27f4258. Earlier session/CSRF/audit, atomic inbox delivery/counters and exact staff acknowledgments remain verified.

**56 focused API cases,80 worker cases,12 distinct FA/EN browser cases and50 i18n cases pass.** Prior70 triage/8 chargeback/81 worker runs remain recorded with overlaps and superseded assertions identified. Applicable types/lint/format/build and42 unchanged budgets pass. Promtool validates2 rules and15 assertions across6 scenarios. No deployment or receiver delivery is claimed. [Current checkpoint](evidence/step-reviews.json#R02-outbox-delivery-checkpoint) preserves failures and reruns. 2437 logs indexed.

Critical next gap: uncertain external requests/process crashes can resend without proven deduplication. SMS snapshots contain a key that never reaches the provider; SMTP only has a stable Message-ID. Current receipt recovery handles a transient recording failure within one live worker, not a crash before durable acknowledgment. Fix durable ambiguous-send recovery/reconciliation before closing T-05.01.04. General log lookup beyond dead-letter entries and notification-specific graceful claim recovery also remain.
...
Repair uncertain external-delivery/crash recovery before claiming at-most-once delivery; inspect provider-supported idempotency and add durable reconciliation where needed. SMS currently drops the snapshot key; SMTP Message-ID is not deduplication proof. Then add general admin delivery-log lookup, verify notification-specific shutdown/claim recovery, and finish individual dispositions for the nine-task batch.
