...
Workspace `/Users/majid/www/barghsa/barghsa-core`, branch `codex/audit-fixes`. Product/test HEAD **214c8a1**. Staff audit task/PR219 verified/closed: fixed dark-theme contrast;49 API cases and4 production desktop/mobile browser cases pass. Types/lint/format/build/42 unchanged budgets pass. [Staff audit review](evidence/step-reviews.json#R01-staff-permission-audit). Dashboard invitation task and PR137 are verified/closed; two production browser checks pass with unchanged invitation workflow evidence reused. [Widget review](evidence/step-reviews.json#R01-invitation-dashboard). Verification batch consolidated: mode Draft→Active at1858caf, named actionable manual notices/dashboard atd74c8f8, provider circuit recovery at709d28b. **3 tasks partial /3 saved PR reviews open** because real-provider and external-delivery criteria remain unavailable. Do not count those dependencies as passed. [Batch review](evidence/step-reviews.json#R01-verification-policy).

Mode evidence remains90 backend/18 browser cases. Notice repair adds68 backend/6 browser cases. Provider evidence is29 distinct cases across focused runs; earlier27 overlap. API/web types, lint/format, build, OpenAPI and42 unchanged budgets pass. **2167 logs indexed.**22 further current source bindings refreshed;38 retained/identified stale evidence records remain. These identify older evidence gaps, not38 new coding defects. The interrupted unfiltered API run's unrelated wallet-controller mock failures remainV02; no full-regression evidence claimed.
...
Counts: **130 verified /32 partial /160 pending =322 claims**.192 unresolved task reviews are not an effort estimate. Saved PR reviews: **107 closed /23 open /171 unreviewed** of301.58 skips:3 verified/55 pending. GitHub inventory ends September3; no live refresh claimed. Full regression and coverage remainV02.

## Next action

Active **R01-staff-role-catalogue**, PR162, `02-auth-users-admin.md#T-09.05.01`. Exact requirement, saved PR body and both deferrals already read. AdminRolesPage and API list/effective methods read. Read role-permission-recovery.spec.ts and relevant earlier evidence. Repair missing grouped permission checkboxes; verify FA/EN/theme presentation and effective lookup. Predefined roles are explicitly read-only; custom-role editing is future scope.
