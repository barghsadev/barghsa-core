...
## Contact and settings transaction checkpoint

At39317d4, contact issuance/completion binds the current account/session through writes and correlated audit;74 affected API cases pass. At90f6415, profile updates preserve staff self-edit authority and address history under account/session/profile locks; timezone saves validate the full input and persist a correlated audit.60 distinct affected API cases pass across recorded runs. Two old concurrent-default fixtures now inspect the actual authentication wait chain, including tuple waiters; exact success/default/audit assertions remain.

AuthController, ProfilesController and UserSettingsController source hashes are refreshed. Route metadata and OpenAPI remain unchanged:347 routes,205 unsafe registrations,122 step-up registrations. Current contract/types/lint/format pass. Original review/hash provenance and83 unchanged class members are supported by the bounded comparison in the account-settings batch. Coverage and other-domain acceptance are not renewed.
