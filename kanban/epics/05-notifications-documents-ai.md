...
### Story N1: Notification Transport Interface & Core Infrastructure
...
|---|---|---|
| **N1.1 — Notification module scaffold** | M | P0 |
...
| **N1.2 — Durable outbox table & write pipeline** | L | P0 |
...
| **N1.3 — Job queue with retry schedule** | L | P0 |
...
| **N1.4 — Idempotency for delivery** | M | P0 |
...
| **N1.5 — Status tracking & delivery logs** | M | P1 |
...
| **N1.6 — Dead-letter queue & admin UI** | M | P1 |
...
| **N1.7 — Metrics & observability** | S | P1 |
...
### Story N2: In-App Notification Center
...
| Task | Complexity | Priority |
|---|---|---|
| **N2.1 — Notification entity & in-app transport** | M | P0 |
...
| **N2.2 — Notification center API** | M | P0 |
...
| **N2.3 — Notification center UI** | L | P0 |
...
| **N2.4 — New-notification polling / SSE** | M | P1 |
...
### Story N3: Notification Classification & Delivery Window
...
| Task | Complexity | Priority |
|---|---|---|
| **N3.1 — Notification type registry & classification** | M | P0 |
...
| **N3.2 — Delivery window logic** | S | P1 |
...
| **N3.3 — Admin delivery-window configuration UI** | S | P1 |
...
### Story N4: Template Management (Persian & English)
...
| Task | Complexity | Priority |
|---|---|---|
| **N4.1 — Template entity & CRUD API** | L | P0 |
...
| **N4.2 — Variable interpolation & escaping** | M | P0 |
...
### Story N5: Mandatory Notifications & Marketing Consent
...
| Task | Complexity | Priority |
|---|---|---|
| **N5.1 — Notification category model** | M | P0 |
...
| **N5.2 — Channel availability rules** | M | P1 |
...
| **N5.3 — Consent UI in profile settings** | S | P1 |
...
### Story N6: Email Provider Administration
...
| Task | Complexity | Priority |
|---|---|---|
| **N6.1 — Provider config entity & lifecycle** | L | P0 |
...
| **N6.3 — Resend configuration** | M | P0 |
...
| **N6.5 — Secrets encryption & masking** | M | P0 |
...
| **N6.6 — Circuit breaker for email** | M | P1 |
...
| **N6.7 — Email delivery callback handling** | S | P1 |
...
### Story N7: SMS.ir Provider Administration
...
| Task | Complexity | Priority |
|---|---|---|
| **N7.1 — SMS.ir config entity** | M | P0 |
| `sms_provider_configs` table: `id`, `status` (draft/active/superseded/disabled), `api_key` (encrypted), `default_sender` (line number), `request_timeout` (default 10s), `throughput_rps` (default 10), `low_credit_threshold`, `low_credit_balance` (last observed), `last_test_at`, `last_test_status`, `created_by`, `created_at`. Same lifecycle as email provider (N6.1). | | |
| **N7.2 — Template mapping** | M | P0 |
...
### Story N8: Provider Failure & Circuit Breaker
...
| Task | Complexity | Priority |
|---|---|---|
| **N8.1 — Error classification utility** | S | P1 |
...
| **N8.2 — Circuit breaker implementation** | M | P1 |
| Reusable `CircuitBreaker` decorator/service: state (closed/open/half-open), failure count, threshold (configurable), cooldown (configurable). On open: reject immediately with `ProviderUnavailableError`. On half-open: allow probe; success → reset; fail → reopen. Integrate into both email and SMS transport adapters. | | |
| **N8.3 — Provider health dashboard** | M | P1 |
...
### Story D1: S3-Compatible Object Storage Provider
...
| Task | Complexity | Priority |
|---|---|---|
| **D1.1 — File storage abstraction interface** | M | P0 |
...
| **D1.3 — Storage config entity & admin UI** | M | P1 |
...
| Task | Complexity | Priority |
|---|---|---|
| **D2.1 — Document template entity** | M | P0 |
...
| **D2.2 — Template file upload & placeholder extraction** | L | P0 |
...
### Story D3: Document Lifecycle Management
...
| Task | Complexity | Priority |
|---|---|---|
| **D3.1 — Document state machine** | L | P0 |
...
| **D3.3 — Document scanning integration** | M | P0 |
...
| **D3.4 — Document review workflow** | L | P1 |
...
| **D3.5 — Document supersession & immutability** | M | P1 |
...
| **D3.6 — Soft delete & hard delete** | L | P1 |
...
| **D3.7 — Document admin/staff UI** | L | P1 |
...
### Story D4: File Upload Validation & Categories
...
| Task | Complexity | Priority |
|---|---|---|
| **D4.1 — File validation service** | M | P0 |
...
| **D4.2 — Category & limit configuration** | M | P1 |
...
### Story D5: Authorized Access URLs & Security
...
| Task | Complexity | Priority |
|---|---|---|
| **D5.1 — Signed URL generation API** | M | P0 |
...
| **D5.2 — Access control middleware** | M | P0 |
...
| **D5.3 — Download access logging** | M | P1 |
...
### Story D6: Retention Policies & Legal Hold
...
| Task | Complexity | Priority |
|---|---|---|
| **D6.1 — Retention policy configuration** | M | P1 |
...
### Story D7: Multipart Upload & Orphan Cleanup
...
| Task | Complexity | Priority |
|---|---|---|
| **D7.1 — Multipart upload API** | M | P0 |
...
| **D7.2 — Orphan detection & cleanup** | M | P1 |
...
| Task | Complexity | Priority |
|---|---|---|
| **A1.1 — AI model entity & CRUD** | M | P0 |
...
### Story A2: Knowledge Bases & KB Groups
...
| Task | Complexity | Priority |
|---|---|---|
| **A2.1 — Knowledge base entity & CRUD** | M | P0 |
...
| **A2.2 — KB processing pipeline** | M | P0 |
...
| Task | Complexity | Priority |
|---|---|---|
| **A5.1 — Agent slot entity & configuration** | M | P0 |
...
| **A5.2 — Slot assignment admin UI** | S | P1 |
...
| Task | Complexity | Priority |
|---|---|---|
| **A7.1 — AuthZ for AI actions** | L | P0 |
...
| **A7.2 — Trusted-UI confirmation for writes** | L | P0 |
...
| **A7.4 — Data isolation per slot** | M | P1 |
...
| **A7.5 — Sensitive value redaction** | M | P1 |
...
| **A7.6 — Source attribution in answers** | L | P1 |
...
|---|---|---|
| Notification template engine (simple string interpolation vs mustache/handlebars) | N4.2 — template rendering | Domain Agent #5 |
| Vector embedding strategy for KBs (pgvector vs separate vector DB) | A2.2 — KB processing | Domain Agent #5 |
| Chunking strategy for KB documents | A2.2 — KB processing | Domain Agent #5 |
| AI model provider abstraction (single OpenAI-compatible vs multi-provider) | A1.1 — model management | Domain Agent #5 |
| Malware scanning service (ClamAV sidecar vs managed API) | D3.3 — scanning | Domain Agent #5 |
| SSE vs polling for in-app notification badge | N2.4 — notification polling | Domain Agent #5 |
...
|---|---|---|
| Provider secrets exposure | Encrypt at rest, mask in UI, exclude from logs/analytics, write-only after entry | N6.5, D1.3, A1.1 |
| Unauthorized file access | Signed URL with backend authorization, least-privilege download, profile-scoped | D5.1, D5.2 |
| Malware in uploaded files | Validate MIME + extension, malware scan, quarantine state | D3.3, D4.1 |
| AI prompt injection | Data isolation per slot, backend authZ for tools, trusted-UI confirmation for writes | A7.1–A7.6 |
| AI data leak across profiles | Each chatbot slot has profile-scoped context | A7.4 |
| Sensitive data in AI context | Redact secrets before prompt construction | A7.5 |
| Notification spam / abuse | Rate limits per profile, dead-letter controls, marketing consent required | N5.1, N5.2 |
| Immutable financial documents | Signed contracts cannot be replaced or deleted | D3.5 |
| Provider SSRF via SMTP config | Block private/internal network SMTP targets unless explicit deployment allow-list | N6.2 |
| SMS / email delivery failure | In-app mandatory delivery always succeeds; external failure does not hide in-app | N1.6 |
...
- AI inference concurrency: per-model concurrency limit, queue for excess

### Testing
- Unit: transport interface, error classification, template interpolation, state machines, permission checks
- Integration: outbox + worker against real PostgreSQL, S3 fakes (MinIO test container), Resend/SMS.ir adapter contract tests with deterministic fakes
...
| AI model cost escalation | Unpredictable operating cost | Per-model token/cost budget, admin-configurable concurrency limits, user quotas |
| Malware scanning latency | Upload flow slowed | Async scan with fallback to pending state; user notified to check back |
| Document storage cost for signed contracts | Long-term immutable storage growth | Lifecycle policies, tiered storage (S3 Glacier for aged contracts) |
| Notification delivery to invalid email/SMS | Provider reputation damage | Hard bounce suppression for email, configurable retry limits, alert on sustained failures |
...
### G-05.01 — AI isolation from core API capacity
- **Source:** README.md §Backend (line 245)
- **Gap:** Epic 05 has tasks for AI circuit breakers and rate limits but lacks tasks for: (a) ensuring core ordering/payment APIs remain available when AI is unavailable (separate process, bulkhead pattern, or concurrency isolation), (b) per-model budget controls (token/month or cost/month limits), (c) AI request queues with concurrency limits that prevent AI from consuming all API worker capacity.
- **Suggested Task:** Add story "AI Capacity Isolation": create dedicated AI worker process or concurrency-limited AI handler that runs independently from core API workers. Implement per-model token budget (resets monthly, admin-configurable). Implement AI request queue with max concurrency (e.g. 10 concurrent requests across all models). Ensure that when AI is saturated, the `/api/health/ready` endpoint stays healthy for core services.

### G-05.02 — Async job progress tracking (job ID + visible progress)
- **Source:** README.md §Performance engineering (lines 369–370)
- **Gap:** While presigned URL workflow is covered, there is no task for ensuring large async operations (document generation, exports, media processing) return a job ID with a progress endpoint instead of holding an HTTP request open.
- **Suggested Task:** Add story "Async Job Framework": Create `jobs` table with `id`, `type`, `status (queued/processing/completed/failed)`, `progress_pct`, `result_url`, `error`. Create `JobService.submit(type, payload)` returning job ID. Create `GET /api/jobs/:id/status` endpoint. Create `<JobProgress>` component that polls for status. Document the pattern: any operation taking > 5s must use the async job framework.

### G-05.03 — Document template: placeholder extraction during file changes
- **Source:** README.md §Documents templates (lines 770–773)
- **Gap:** D2.2 covers placeholder extraction at initial upload, but there is no task for re-extracting placeholders when files are added/replaced/deleted in a template version update, or for detecting placeholder conflicts between files in the same template.
- **Suggested Task:** Add task to D2.2: on template version update (new file added, old file replaced), re-run placeholder extraction, merge the placeholder set from all files in the new version, detect conflicts (same placeholder name in different files with different context). Store merged placeholder list. Alert if required system placeholders (`{{date}}`, `{{customerName}}`) are missing.