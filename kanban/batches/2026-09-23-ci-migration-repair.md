...
Normal `main` pushes now run tests for changed workspaces against the push's previous commit. Build and typecheck still cover their dependents. Lint, security scans, schema and contract checks, and the route budget checker's unit tests remain enabled. Browser coverage, combined coverage, and production route budgets are temporarily opt-in through the `CI_FULL_CHECKS=true` repository variable; the measured auth and electricity bundles still exceed their existing limits and need a separate optimization batch.
