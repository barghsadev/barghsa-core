...
Full main after #321 (35543648547) also passed all functional/security/integrity jobs, but combined dictionary coverage found the custom-field label fallback unexecuted by browser tests. The contract flow now includes an unmapped `deliveryZone` label and asserts its literal label and value in both languages. This covers the real imported-snapshot fallback without changing dictionary or coverage behavior. Final combined validation and review must include this addition.
