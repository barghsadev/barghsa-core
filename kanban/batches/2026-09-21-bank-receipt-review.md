...
PR333's implementation and translation follow-up received independent approval at `d2c71e4674b7f600c3b6e58b20aa232734308428`; the combined changed-source coverage gate passes at that revision. CI35583722403 then failed after all 466 worker assertions passed because the auth-delivery fixture force-dropped its database while a pooled connection was still closing. The fixture now waits for zero database connections before dropping without FORCE. Its 38 focused tests pass, including cleanup. The test-only repair needs a bounded follow-up review and a new successful CI run before merge.
