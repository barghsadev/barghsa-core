...
describe('ReminderOffsetTogglePanel (T-04.1.04.05)', () => {
  ...
  it('rejects the write until step-up, then retries the PUT after verification', async () => {
    ...
    await act(async () => {
      (
        container.querySelector('[data-testid="reminder-step-up-submit"]') as HTMLButtonElement
      ).click();
    });

    const stepUp = fetchMock.mock.calls.find((call) =>
      String(call[0]).endsWith('/api/auth/step-up')
    );
  ...
  it('reverts the toggle when the step-up challenge is cancelled', async () => {
    ...
    await act(async () => {
      (
        container.querySelector('[data-testid="reminder-step-up-cancel"]') as HTMLButtonElement
      ).click();
    });
    ...
    fetchMock.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/api/auth/step-up') && init?.method === 'POST') {
        return {
          ok: false,
          status: 422,
          json: async () => ({ error: { code: 'AUTH:LOGIN_INVALID_CREDENTIALS' } }),
        };
      }
      return { ok: false, status: 404, json: async () => ({ message: 'not found' }) };
    });
