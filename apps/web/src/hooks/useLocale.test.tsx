...
function Language(){return <output>{useLocale()}</output>}
it('updates mounted consumers on a language change and normalizes regional locales',async()=>{
  ...
  try {
    ...
    await act(async()=>{root.render(<Language />)})
    expect(container.textContent).toBe('fa')
    await act(async()=>{document.documentElement.lang='en-US'})
    expect(container.textContent).toBe('en')
    await act(async()=>{document.documentElement.lang='unsupported'})
    expect(container.textContent).toBe('fa')
  } finally {
