...
function ActivatePage() {
  ...
  useEffect(() => {
    ...
    const receiveLink = () => {
      ...
      if (incoming !== null) { setToken(incoming); setComplete(false); setError(null) }
  ...
  return (
    <AuthLayout locale={locale} footer={<Link to="/login" className="text-sm text-primary underline">{t('auth.forgotPassword.backToLogin', locale)}</Link>}>
      <div className="space-y-6" dir={locale === 'fa' ? 'rtl' : 'ltr'}>
        <h1 className="text-xl font-semibold">{t(complete ? 'auth.activate.success' : 'auth.activate.title', locale)}</h1>
        {complete ? <p role="status">{t('auth.resetPassword.signIn', locale)}</p> : !validToken ? (
          <Alert variant="destructive" role="alert"><AlertDescription>{t('auth.activate.invalid', locale)}</AlertDescription></Alert>
        ) : (
          <form onSubmit={activate} className="space-y-4">
            {error && <Alert variant="destructive" role="alert"><AlertDescription>{error}</AlertDescription></Alert>}
            <PasswordField id="activation-password" autoFocus label={t('auth.resetPassword.newPassword', locale)} locale={locale} value={password} onChange={setPassword} disabled={busy} />
            <PasswordField id="activation-confirmation" label={t('auth.resetPassword.confirmPassword', locale)} locale={locale} value={confirmation} onChange={setConfirmation} disabled={busy} showStrength={false} />
            <Button type="submit" className="w-full" disabled={busy || !validPassword || !confirmation}>{t(busy ? 'auth.resetPassword.submitting' : 'auth.activate.submit', locale)}</Button>
          </form>
        )}
      </div>
    </AuthLayout>
