  ...
  const load = useCallback(async () => {
    ...
    try {
      const responses = await Promise.all(
        ['/api/admin/staff-teams', '/api/admin/config/assignment-rules'].map((path) =>
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (!controller.signal.aborted) setMemberError(true);
      } finally {
          ...
          <form onSubmit={saveTeam} className="space-y-4 rounded-lg border bg-white p-4">
            ...
            <fieldset disabled={disabled} className="space-y-3">
              ...
              {memberLoading ? (
                ...
                <p role="alert">{label('memberError')}</p>
              ) : (
                <>
                    ...
                    {members.map((member) => (
                      <label key={member.id} className="flex items-center gap-2">
                        <input
                          ...
                          disabled={
                            !draft.members.includes(member.id) && draft.members.length >= 200
                          }
                ...
                <fieldset key={type} className="flex flex-wrap items-end gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {rules[type].teamId &&
                        !teams.some((team) => team.id === rules[type].teamId && team.isActive) && (
