...
function Corrections({ profileId }: { profileId: string | undefined }) {
  ...
  async function select(item: Case) {
    ...
    setNotes('');
    try {
      const response = await fetch(`/api/crm/verification-cases/${item.id}`, {
        credentials: 'include',
      });
      if (!response.ok) throw new Error();
      const data = await response.json();
      ...
      if (current === detailGeneration.current) setError(true);
    } finally {
      ...
      {profileId && profileType && queue?.viewer.canCreate && (
        ...
        >
          ...
          <fieldset disabled={busy || !!action} className="space-y-3">
            ...
            <div>
              <Label htmlFor="correction-files">{t('crm.corrections.files', locale)}</Label>
              <Input
                key={fileVersion}
                id="correction-files"
                type="file"
      ...
      {action && (
        <TeamActionDialog
          ...
          onSuccess={async () => {
            ...
            setReason('');
            setFiles([]);
            setFileVersion((previous) => previous + 1);
            await load();
          }}
        />
      )}
