...
describe('InvoiceDetailsPage (T-04.1.05.04)', () => {
  ...
  it('shows the original invoice and the replacement with its explanation', async () => {
    ...
    expect(container.querySelector(`[data-testid="invoice-card-${ORIGINAL_ID}"]`)).toBeTruthy();
    expect(container.querySelector(`[data-testid="invoice-card-${REPLACEMENT_ID}"]`)).toBeTruthy();
    expect(
      container.querySelector(`[data-testid="invoice-explanation-${REPLACEMENT_ID}"]`)?.textContent
  ...
  it('shows linked post-payment adjustments with explanations', async () => {
    ...
    expect(
      container.querySelector(`[data-testid="invoice-explanation-${ADJUSTMENT_ID}"]`)?.textContent
  ...
  it.each(['Unpaid', 'Overdue'] as const)(
    'shows the bank-receipt upload form for a %s invoice',
    async (state) => {
      const payload = replacementPayload();
