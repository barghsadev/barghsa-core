...
export function rateLimitMessage(response: Pick<Response, 'status' | 'headers'>, locale: Locale): string | null {
  if (response.status !== 429) return null
  const seconds = retryAfterSeconds(response)
  return seconds === null ? t('error.rate_limit.exceeded', locale)
    : t('error.rate_limit.retry_after', locale).replace('{seconds}',
