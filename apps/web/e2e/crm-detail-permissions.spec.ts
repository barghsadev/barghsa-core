...
test('session expiry preserves customer and reason through password confirmation', async ({
  page,
}) => {
  await page.addInitScript(() => {
    if (document.documentElement) document.documentElement.lang = 'en';
  ...
  await page.route('**/api/crm/users/customer/expire-sessions', (route) => {
    ...
    return route.fulfill(
      verified ? { json: { success: true } } : { status: 403, json: { requiresStepUp: true } }
    );
  });
  ...
  await page
    .getByRole('dialog')
    .getByRole('button', { name: 'Expire Sessions', exact: true })
    .click();
...
test('verification requires a reason when removing approval and archive blockers remain visible', async ({
  page,
}) => {
  await page.addInitScript(() => {
    if (document.documentElement) document.documentElement.lang = 'en';
  ...
  await page.route(`**/api/crm/profiles/${id}`, (route) =>
    route.request().method() === 'DELETE'
      ? route.fulfill({ status: 409, json: { error: { code: 'CRM:PROFILE:DELETION_BLOCKED' } } })
  ...
  await page
    .getByLabel('Reason (required to remove or renew verification)')
    .fill('Evidence expired');
