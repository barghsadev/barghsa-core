...
for (const locale of ['fa','en'] as const) {
  test(`staff activation consumes the fragment token without persisting it (${locale})`, async ({ page }) => {
    const token = 'a'.repeat(64)
    await page.route('**/api/auth/activate-staff', route => route.fulfill({ json: { activated: true } }))
    ...
    expect((await submitted).postDataJSON()).toEqual({ token,newPassword:'Activated-browser-password-123!' })
...
test('missing or rejected activation links never show successful activation', async ({ page }) => {
  ...
  await page.route('**/api/auth/activate-staff', route => route.fulfill({ status:401,json:{error:{code:'AUTH:TOKEN_INVALID'}} }))
