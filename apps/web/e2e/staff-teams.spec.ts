...
for(const locale of ['en','fa'])test(`staff team and routing changes survive confirmation and failure (${locale})`,async({page})=>{
  ...
  const text=locale==='fa'?{name:'نام تیم',search:'جستجوی کارکنان',save:'ذخیره تیم',confirm:'تأیید',password:'رمز عبور خود را تأیید کنید',edit:'ویرایش تیم',remove:'حذف تیم',rules:'ذخیره قوانین تخصیص',team:'تیم مسئول',strategy:'روش انتخاب عضو'}:{name:'Team name',search:'Search staff',save:'Save team',confirm:'Confirm',password:'Confirm your password',edit:'Edit team',remove:'Delete team',rules:'Save assignment rules',team:'Assigned team',strategy:'Member selection'}
  let teams:Array<{id:string;name:string;memberUserIds:string[];isActive:boolean}>=[],verified=false,fail=false
  let rules={ticket:{teamId:null as string|null,strategy:'round_robin'},verification_case:{teamId:null as string|null,strategy:'round_robin'}}
  const bodies:unknown[]=[]
  await page.route('**/api/admin/staff-teams/members?*',route=>route.fulfill({json:{items:[{id:'staff-a',name:'Support Alice'}],selected:[],hasMore:false}}))
  ...
  await expect(page.locator('section').first()).toHaveAttribute('dir',locale==='fa'?'rtl':'ltr')
