...
test('Persian picker uses Jalali month boundaries and sends Gregorian API dates',async({page})=>{
  await page.clock.install({time:new Date('2026-03-21T12:00:00Z')})
  await page.addInitScript(()=>{new MutationObserver(()=>{if(document.documentElement)document.documentElement.lang='fa'}).observe(document,{childList:true})})
  await page.route('**/api/**',route=>route.fulfill({status:404,json:{}}))
  const requests:URL[]=[]
  await page.route('**/api/crm/users?*',route=>{requests.push(new URL(route.request().url()));return route.fulfill({json:{users:[],cursor:null,hasMore:false}})})
  await page.goto('/admin/crm')
  await page.getByRole('combobox',{name:'ثبت‌نام از',exact:true}).click()
  const calendar=page.locator('[data-slot="calendar"]')
  await expect(calendar).toContainText('فروردین')
  const firstDay=calendar.getByRole('button',{name:/فروردین.*۱.*۱۴۰۵|۱.*فروردین.*۱۴۰۵/}).first()
  await firstDay.click()
  await expect.poll(()=>requests.at(-1)?.searchParams.get('dateFrom')).toBe('2026-03-21')
  await expect(page.getByRole('combobox',{name:'ثبت‌نام از',exact:true})).toContainText('1405/01/01')
})
test('Jalali leap-day selection and keyboard dismissal preserve the date',async({page})=>{
  ...
  await expect.poll(()=>requests.at(-1)?.searchParams.get('dateTo')).toBe('2025-03-20T23:59:59.999999Z')
  await trigger.click()
