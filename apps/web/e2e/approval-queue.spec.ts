...
const request = { id, actionType: 'bank_payment_confirmation', amountIrR: '10000000000000001', initiatorId: 'initiator', initiatorUsername: 'finance@example.test', reason: 'Bank evidence reviewed', status: 'pending', reviewerId: null, reviewerUsername: null, reviewReason: null, details: { entityType: 'wallet_bank_receipt' } }
...
for (const locale of ['fa', 'en']) {
  test(`approval retains exact amount and request across step-up (${locale})`, async ({ page }) => {
    ...
    let verified = false, resolved = false
    const actions: string[] = []
    await page.route('**/api/admin/approval-requests?*', route => route.fulfill({ json: resolved ? [] : [request] }))
    ...
    await expect(page.locator('dd').filter({ hasText: new Intl.NumberFormat(locale === 'fa' ? 'fa-IR' : 'en-US').format(10000000000000001n) })).toHaveCount(1)
    await page.getByRole('button', { name: locale === 'fa' ? 'تأیید' : 'Approve', exact: true }).click()
    ...
    await expect(page.getByRole('status')).toContainText(locale === 'fa' ? 'باید در روند مربوط به خود تکمیل شود' : 'must still be completed')
  })
}
test('rejection requires a reason and conflict never reports success', async ({ page }) => {
  await shell(page)
  await page.route('**/api/admin/approval-requests?*', route => route.fulfill({ json: [request] }))
...
test('pagination and history expose no decision controls', async ({ page }) => {
  ...
  await page.route('**/api/admin/approval-requests?*', route => {
    ...
    if (status === 'approved') return route.fulfill({ json: [{ ...request, status, reviewerId: 'reviewer', reviewerUsername: 'reviewer@example.test' }] })
    return route.fulfill({ json: url.searchParams.get('offset') === '0' ? Array.from({ length: 26 }, (_, n) => ({ ...request, id: `${id}-${n}` })) : [] })
  })
  await page.goto('/admin/approval-requests')
...
test('an obsolete pending response cannot replace selected history', async ({ page }) => {
  ...
  await page.route('**/api/admin/approval-requests?*', async route => {
    if (new URL(route.request().url()).searchParams.get('status') === 'pending') { started(); await pending; await route.fulfill({ json: [request] }) }
