...
test('target validation and confirmation preserve settings through a failed save',async({page})=>{
  ...
  let values={ticket:24,verification_case:null},verified=false,fail=true
  ...
  await page.route('**/api/admin/config/service-response-targets',route=>{
    ...
    values=route.request().postDataJSON();return route.fulfill({json:values})
  })
