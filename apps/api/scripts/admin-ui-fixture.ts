...
async function main() {
  ...
  await http.pool.query(`INSERT INTO sessions(session_id,user_id,csrf_token,family_id,expires_at,idle_deadline,step_up_verified_at)
    VALUES ($1,'team-ui-admin',$2,$3,NOW()+INTERVAL '1 day',NOW()+INTERVAL '1 hour',NOW())`,[session,csrf,randomUUID()])
  let closing=false
  const close=async()=>{if(closing)return;closing=true;await http.close();await teardown();process.exit(0)}
  process.once('message',()=>void close())
  process.once('disconnect',()=>void close())
  process.once('SIGTERM',()=>void close())
  process.send?.({base:http.base,session,csrf})
}
main().catch(async error=>{console.error(error);await teardown();process.exit(1)})
