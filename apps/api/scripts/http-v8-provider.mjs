...
export class HttpV8CoverageProvider extends V8CoverageProvider {
  ...
  async generateCoverage(options) {
    ...
    for (const file of files) {
      ...
      data.result = data.result.filter(entry => entry.url.startsWith('file:') && fileURLToPath(entry.url).startsWith(compiledRoot))
    ...
    this.ctx.logger.log(`Merged ${files.length} HTTP-process coverage files (${merged.result.length} compiled modules)`)
    return coverage
  }
}
