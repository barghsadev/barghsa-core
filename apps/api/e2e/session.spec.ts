...
test('browser cookies enforce CSRF across login, refresh, step-up, another tab, and logout', async ({
  page,
  context,
  http,
}) => {
  ...
  await context.addCookies([
    { name: 'barghsa_device', value: fingerprint, url: http.base, httpOnly: true, sameSite: 'Lax' },
  ]);
  await page.goto(`${http.base}/api/docs`);
  const helper = ts
    .transpile(readFileSync(resolve(__dirname, '../../web/src/lib/csrf.ts'), 'utf8'), {
  ...
  expect(cookies.find((cookie) => cookie.name === 'barghsa_refresh')?.path).toBe(
    '/api/auth/refresh'
  );
  ...
  const afterChange = await page.evaluate(
    async ({ password, oldToken }) => {
      // withCsrf is loaded from the production client module above.
      const withCsrf = (globalThis as unknown as { withCsrf: (headers?: HeadersInit) => Headers })
        .withCsrf;
      const stale = await fetch('/api/auth/step-up', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': oldToken },
        body: JSON.stringify({ password }),
      });
      const valid = await fetch('/api/auth/step-up', {
        method: 'POST',
