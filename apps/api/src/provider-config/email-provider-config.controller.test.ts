...
import { EmailProviderConfigController } from './email-provider-config.controller.js';
...
describe('EmailProviderConfigController', () => {
  ...
  describe('PUT /api/admin/email-providers/:id', () => {
    it('updates a draft provider config and returns the updated row', async () => {
      ...
      const result = await controller.update(adminReq, 'cfg-1', {
        label: 'Renamed',
    ...
    it('rejects an invalid body with 400', async () => {
      await expect(
        controller.update(adminReq, 'cfg-1', { label: '' } as never)
  ...
  describe('permission + step-up metadata (T-09.06.01)', () => {
    it('requires step-up on create (a provider-config mutation)', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.create
      );
    ...
    it('requires step-up on update', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.update
      );
      expect(meta).toBe(true);
    });
    ...
    it('requires step-up on activate', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.activate
      );
      expect(meta).toBe(true);
    });
    ...
    it('requires step-up on disable', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.disable
      );
      expect(meta).toBe(true);
    });
    ...
    it('requires step-up on rollback', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.rollback
      );
      expect(meta).toBe(true);
    });
    ...
    it('requires step-up on recordTest', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.recordTest
      );
      expect(meta).toBe(true);
    });
    ...
    it('does not require step-up for the read-only list', () => {
      const meta = Reflect.getMetadata(
        'requiresStepUp',
        EmailProviderConfigController.prototype.list
      );
