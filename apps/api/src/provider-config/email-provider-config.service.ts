...
import { ProviderSecretsService, type ProviderMaskedConfig } from './provider-secrets.service';
...
export class EmailProviderConfigService {
  ...
  private async findById(
    ...
    lock = false
  ): Promise<EmailProviderConfigResult | null> {
    const result = await query.query(
      `SELECT ${SELECT_COLUMNS} FROM email_provider_configs WHERE id = $1${lock ? ' FOR UPDATE' : ''}`,
      [id]
    );
    const row = result.rows[0] as
      (EmailProviderConfigResult & { config?: ProviderConfigBody }) | undefined;
    if (!row) return null;
    ...
    return mutateProvider(this.db, input.createdBy, 'email', 'created', async (client) => {
      ...
      await client.query(
       ...
       VALUES ($1, $2, $3, 'draft', $4, $5, $6)`,
        [id, input.transport, input.label, config, input.createdBy, input.supersedesId ?? null]
      );
  ...
  private async testSmtpConnection(id: string): Promise<{
    ...
    if (!this.smtpTester) {
      throw new HttpException(
        errBody(
          500,
          ErrorCodes.INTERNAL_UNEXPECTED.code,
          'SMTP connection tester is not available'
        ),
        500
      );
    }
    ...
    if (!this.resendTester) {
      throw new HttpException(
        errBody(
          500,
          ErrorCodes.INTERNAL_UNEXPECTED.code,
          'Resend connection tester is not available'
        ),
        500
      );
    }
  ...
  private async readConfig(
    ...
    const row = result.rows[0] as
      { config: ProviderConfigBody; transport: EmailProviderTransport } | undefined;
    if (!row) return {};
