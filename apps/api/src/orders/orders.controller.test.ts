...
describe('OrdersController', () => {
  ...
  describe('createOrder', () => {
    it('delegates to service and returns order', async () => {
      ...
      expect(service.createOrder).toHaveBeenCalledWith(req.session, body, 'unknown');
    });
  ...
  describe('getOrder', () => {
    ...
    it('throws 404 when order not found', async () => {
      ...
      await expect(controller.getOrder('ord-001', req)).rejects.toThrow();
    });
  });
  ...
  describe('cancelOrder', () => {
    it('delegates to service and returns the cancelled order', async () => {
      ...
      expect(service.cancelOrder).toHaveBeenCalledWith(req.session, 'ord-001', 'unknown');
    });
