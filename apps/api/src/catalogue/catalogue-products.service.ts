...
export type ProductCategory =
  (typeof CONSULTATION_CATEGORIES)[number] | (typeof ELECTRICITY_CATEGORIES)[number];
...
export class CatalogueProductsService {
  ...
  update(id: string, input: UpdateProductInput): Promise<ProductDetailDto> {
    return this.withTransaction(input.actorUserId, input.session, async (q, verifiedAt) => {
      ...
      if (input.description !== undefined) {
        push('description', input.description === null ? null : JSON.stringify(input.description));
      }
          ...
          throw new HttpException(
            {
              ...
              message: 'The requested effective window overlaps an existing price version',
            },
            409
          );
        }
        ...
        throw new HttpException(
          {
            ...
            message:
              ...
              'currently active version (active since ' +
              open.effective_from +
              ')',
          },
          400
  ...
  private async findOpenVersion(q: DbExecutor, productId: string): Promise<PriceVersionRow | null> {
    const result = await q.query<PriceVersionRow>(
      `SELECT id, price, effective_from, effective_until, created_by, created_at
         FROM product_price_versions
