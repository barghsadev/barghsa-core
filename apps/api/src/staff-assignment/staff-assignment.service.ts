...
export class StaffAssignmentService {
  async choose(
    ...
    if (
      !rule.teamId ||
      !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(rule.teamId)
    )
      return null;
    // Serialize selection against team edits and other selections in this team.
    const team = (
      await client.query(
        'SELECT id,skill_tags FROM staff_teams WHERE id=$1 AND is_active FOR UPDATE',
        [rule.teamId]
      )
    ).rows[0];
    if (!team) return null;
    if (rule.strategy === 'expertise') {
      const tags = Array.isArray(team.skill_tags)
        ? team.skill_tags
    ...
    const rows = (
      ...
      ORDER BY u.user_id FOR NO KEY UPDATE OF u`,
        [rule.teamId, workType === 'verification_case' ? actorId : null]
      )
    ).rows;
