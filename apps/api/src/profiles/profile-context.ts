import { hasPermission, type AgentRole, type AgentPermission } from '@barghsa/shared/agent-permissions'
...
export function activeProfileSql(permission: AgentPermission = 'profile:view') {
  ...
  const permitted = roles.filter(role=>hasPermission(role,permission)).map(role=>`'${role}'`).join(',') || "''"
