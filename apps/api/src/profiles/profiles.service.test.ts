...
describe('ProfilesService', () => {
  ...
  describe('getVerificationStatus', () => {
    ...
    it('keeps automatic verification unavailable when API mode requires verification', async () => {
      vi.mocked(configCache.get).mockResolvedValueOnce('API');
  ...
  describe('verifyProfileApi', () => {
    it('rejects when verification method is manual (default)', async () => {
      ...
      await expect(service.verifyProfileApi('user-1', 'prof-1')).rejects.toThrow(
  ...
  describe('completeOnboarding', () => {
    ...
    it('rejects when profile does not belong to the user', async () => {
      ...
      await expect(service.completeOnboarding('user-1', 'prof-1')).rejects.toThrow(
        'Profile not found'
      );
    });
