...
beforeEach(async()=>{
  ...
  profileId=(await http.pool.query("INSERT INTO profiles(user_id,profile_type,status,is_default) VALUES ('owner','LEGAL','ACTIVE',true) RETURNING id")).rows[0].id
  await http.pool.query("INSERT INTO profile_agents(profile_id,user_id,role) VALUES ($1,'target','Manager')",[profileId])
},40000)
afterEach(async()=>{await http?.close()},15000)
function post(action:string,user='owner',body:unknown={}){
  return fetch(`${http.base}/api/profiles/${profileId}/${action}`,{method:'POST',headers:headers[user]!,body:JSON.stringify(body)})
}
...
it('requires step-up and keeps the old owner until exact-transfer acceptance',async()=>{
  ...
  expect((await post('transfer-ownership','owner',{newOwnerUserId:'target'})).status).toBe(403)
  await http.pool.query("UPDATE sessions SET step_up_verified_at=NOW() WHERE user_id='owner'")
  const ownerTeam=await fetch(`${http.base}/api/profiles/${profileId}/agents`,{headers:headers.owner!})
  expect(await ownerTeam.json(),http.logs()).toMatchObject({canTransferOwnership:true})
  const managerTeam=await fetch(`${http.base}/api/profiles/${profileId}/agents`,{headers:headers.target!})
  expect(await managerTeam.json()).toMatchObject({canTransferOwnership:false})
  const id=await initiate()
  const outgoing=await fetch(`${http.base}/api/profiles/ownership-transfers`,{headers:headers.owner!})
  expect(await outgoing.json()).toMatchObject({transfers:[{id,direction:'outgoing'}]})
  expect((await http.pool.query('SELECT user_id FROM profiles WHERE id=$1',[profileId])).rows[0].user_id).toBe('owner')
  const list=await fetch(`${http.base}/api/profiles/ownership-transfers`,{headers:headers.target!})
  expect(await list.json()).toMatchObject({transfers:[{id,profileId,direction:'incoming'}]})
  ...
  expect((await http.pool.query('SELECT user_id,is_default FROM profiles WHERE id=$1',[profileId])).rows[0]).toEqual({user_id:'target',is_default:false})
  expect((await http.pool.query("SELECT count(*)::int AS count FROM sessions WHERE user_id IN ('owner','target') AND revoked_at IS NULL")).rows[0].count).toBe(0)
  expect((await post('ownership-accept','target',{transferId:id})).status).toBe(401)
  expect((await http.pool.query("SELECT count(*)::int AS count FROM audit_log WHERE event='ownership_transfer_completed'")).rows[0].count).toBe(1)
})
it.each(['decline','cancel'])('races acceptance with %s and preserves one owner',async(opponent)=>{
  ...
  try{
    ...
    attempts=[post('ownership-accept','target',{transferId:id}),post(`ownership-${opponent}`,opponent==='cancel'?'owner':'target',{transferId:id})]
    await expect.poll(async()=>(await http.pool.query(`SELECT count(*)::int AS count FROM pg_stat_activity WHERE datname=current_database()
      AND wait_event_type='Lock' AND query LIKE 'SELECT user_id,profile_type,archived FROM profiles%'`)).rows[0].count).toBe(2)
    await client.query('COMMIT')
    const results=await Promise.all(attempts)
    expect(results.map(r=>r.status).sort()).toEqual([200,409])
    const status=(await http.pool.query('SELECT status FROM profile_ownership_transfers WHERE id=$1',[id])).rows[0].status
    expect((await http.pool.query('SELECT user_id FROM profiles WHERE id=$1',[profileId])).rows[0].user_id).toBe(status==='Completed'?'target':'owner')
    expect((await http.pool.query("SELECT count(*)::int AS count FROM audit_log WHERE event IN ('ownership_transfer_completed','ownership_transfer_declined','ownership_transfer_cancelled')")).rows[0].count).toBe(1)
...
it('rejects expired or removed targets and allows a replacement after expiry',async()=>{
  const id=await initiate()
  await http.pool.query("UPDATE profile_ownership_transfers SET expires_at=NOW()-INTERVAL '1 minute' WHERE id=$1",[id])
  expect((await post('ownership-accept','target',{transferId:id})).status).toBe(409)
  expect((await http.pool.query('SELECT status FROM profile_ownership_transfers WHERE id=$1',[id])).rows[0].status).toBe('Expired')
  const replacement=await initiate()
  await http.pool.query("DELETE FROM profile_agents WHERE profile_id=$1 AND user_id='target'",[profileId])
...
it('rolls back ownership, sessions and transfer state when audit fails',async()=>{
  const id=await initiate()
  await http.pool.query(`CREATE FUNCTION reject_transfer_audit() RETURNS trigger LANGUAGE plpgsql AS $$
  ...
  expect((await http.pool.query('SELECT user_id,is_default FROM profiles WHERE id=$1',[profileId])).rows[0]).toEqual({user_id:'owner',is_default:true})
  expect((await http.pool.query('SELECT status FROM profile_ownership_transfers WHERE id=$1',[id])).rows[0].status).toBe('Pending')
  expect((await http.pool.query("SELECT count(*)::int AS count FROM sessions WHERE user_id IN ('owner','target') AND revoked_at IS NULL")).rows[0].count).toBe(2)
})
it('checks expiry again after waiting for target membership',async()=>{
  const id=await initiate(),client=await http.pool.connect()
  let attempt:Promise<Response>|undefined
  await http.pool.query("UPDATE profile_ownership_transfers SET expires_at=clock_timestamp()+INTERVAL '1 second' WHERE id=$1",[id])
  try{
    await client.query('BEGIN')
    await client.query("SELECT id FROM profile_agents WHERE profile_id=$1 AND user_id='target' FOR UPDATE",[profileId])
    attempt=post('ownership-accept','target',{transferId:id})
    await expect.poll(async()=>(await http.pool.query(`SELECT count(*)::int AS count FROM pg_stat_activity WHERE datname=current_database()
      AND wait_event_type='Lock' AND query LIKE 'SELECT pa.id FROM profile_agents%'`)).rows[0].count).toBe(1)
    await expect.poll(async()=>(await http.pool.query('SELECT expires_at<clock_timestamp() AS expired FROM profile_ownership_transfers WHERE id=$1',[id])).rows[0].expired).toBe(true)
    await client.query('COMMIT')
...
it('allows only one concurrent initiation',async()=>{
  const responses=await Promise.all([1,2].map(()=>post('transfer-ownership','owner',{newOwnerUserId:'target'})))
