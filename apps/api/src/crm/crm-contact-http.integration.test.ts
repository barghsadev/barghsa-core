...
beforeEach(async () => {
  ...
  await http.pool
    .query(`INSERT INTO users(user_id,username,email,mobile,password_hash,is_staff) VALUES
  ...
  await http.pool.query(
    "INSERT INTO user_roles(user_id,role_id) VALUES ('contact-staff','role-crm-verification')"
  );
  const session = randomUUID(),
    csrf = randomUUID();
  await http.pool.query(
    `INSERT INTO sessions(session_id,user_id,csrf_token,family_id,expires_at,idle_deadline,step_up_verified_at)
    VALUES ($1,'contact-staff',$2,$3,NOW()+INTERVAL '1 day',NOW()+INTERVAL '30 minutes',NOW())`,
  ...
  profileId = (
    await http.pool.query(`INSERT INTO profiles(user_id,title,status,contact_email,contact_mobile)
    VALUES ('contact-owner','Original','ACTIVE','original@example.test','+989122222222') RETURNING id`)
  ).rows[0].id;
}, 40000);
afterEach(async () => {
  await http?.close();
}, 15000);
const edit = (body: unknown, id = profileId) =>
...
const read = async () =>
  (
    await http.pool.query('SELECT title,contact_email,contact_mobile FROM profiles WHERE id=$1', [
      profileId,
    ])
  ).rows[0];

it('edits only profile contacts, normalizes values, preserves account credentials and isolates sibling profiles', async () => {
  const accountBefore = (await http.pool.query("SELECT * FROM users WHERE user_id='contact-owner'"))
    .rows[0];
  const sibling = (
    await http.pool.query(
      "INSERT INTO profiles(user_id,contact_email) VALUES ('contact-owner','sibling@example.test') RETURNING id"
    )
  ).rows[0].id;
  ...
  expect(await response.json()).toMatchObject({
    profile: {
      ...
      contactMobile: '+989123334444',
    },
    user: { username: 'owner@example.test', email: 'owner@example.test', mobile: '+989121111111' },
  });
  expect(
    (await http.pool.query("SELECT * FROM users WHERE user_id='contact-owner'")).rows[0]
  ).toEqual(accountBefore);
  expect(
    (await http.pool.query('SELECT contact_email FROM profiles WHERE id=$1', [sibling])).rows[0]
      .contact_email
  ).toBe('sibling@example.test');
  const detail = await fetch(`${http.base}/api/crm/profiles/${profileId}`, { headers });
  expect(detail.status).toBe(200);
  expect(await detail.json()).toMatchObject({
    profile: { contactEmail: 'contact@example.test' },
...
it('validates contact and identity fields and treats no-op edits as successful without an audit', async () => {
  ...
  expect(
    (await edit({ title: 'Original', email: 'original@example.test', mobile: '+989122222222' }))
      .status
  ).toBe(200);
  expect(
    (
      await http.pool.query(
        "SELECT count(*)::int AS count FROM audit_log WHERE event='profile_updated'"
      )
    ).rows[0].count
  ).toBe(0);
...
it('rolls back profile edits when their audit write fails', async () => {
  await http.pool
    .query(`CREATE FUNCTION reject_contact_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
...
for (const archive of [false, true])
  it(`locks the current profile before editing or auditing (archive=${archive})`, async () => {
    ...
    try {
      ...
      WHERE datname=current_database() AND wait_event_type='Lock' AND query LIKE '%FROM profiles WHERE id=$1 FOR UPDATE%'`)
        ...
        .toBe(1);
      await client.query('UPDATE profiles SET contact_email=$1,archived=$2 WHERE id=$3', [
      ...
      const audits = (
        await http.pool.query(
          "SELECT metadata::jsonb AS metadata FROM audit_log WHERE event='profile_updated'"
        )
      ).rows;
      if (archive) {
        expect(audits).toHaveLength(0);
        expect((await read()).contact_email).toBe('intervening@example.test');
      } else {
        expect(audits).toHaveLength(1);
        expect(audits[0].metadata).toMatchObject({
          scope: 'profile_contact',
          before: { email: 'intervening@example.test' },
          after: { email: 'latest@example.test' },
        });
      }
    } finally {
