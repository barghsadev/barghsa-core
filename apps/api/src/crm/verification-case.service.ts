...
export type GetVerificationCaseResult = VerificationCaseDetail | { error: string } | null;

export type ReviewVerificationCaseResult =
  { success: true; id: string; status: string; profileId: string } | { error: string } | null;
...
export class VerificationCaseService {
  constructor(
    private readonly evidence: VerificationEvidenceService = new VerificationEvidenceService(),
  ...
  async createCase(
    ...
    try {
      ...
      if (!allowed.includes(dto.fieldName))
        throw new BadRequestException('Invalid identity field for this profile type');
      const source =
        profile.profile_type === 'LEGAL'
          ? (await client.query('SELECT * FROM legal_profiles WHERE id=$1 FOR UPDATE', [profileId]))
              .rows[0]
          : profile;
      ...
      const pending = await client.query(
        "SELECT id FROM verification_cases WHERE profile_id=$1 AND field_name=$2 AND status IN ('Open','Under Review')",
        [profileId, dto.fieldName]
      );
      if (pending.rows.length)
        throw new ConflictException('An unresolved correction already exists for this field');
      const evidenceKeys = await this.evidence.seal(
        client,
        dto.evidenceUrls ?? [],
        actorUserId,
        profileId
      ...
      await client.query(
        `INSERT INTO audit_log(id,user_id,event,metadata,correlation_id,ip,created_at) VALUES ($1,$2,'verification_case_created',$3::jsonb,$4,$5,$6)`,
        [
          uuidv7(),
          actorUserId,
          JSON.stringify({
      ...
      if (assignment) {
        const localizedContent = {
          fa: {
            title: t('crm.corrections.assignedNotice', 'fa'),
            body: t('crm.corrections.assignedBody', 'fa'),
          },
          en: {
            title: t('crm.corrections.assignedNotice', 'en'),
            body: t('crm.corrections.assignedBody', 'en'),
          },
        };
        await this.notifications.create(
          {
            ...
            localizedContent,
            link: `/admin/crm/corrections?profileId=${profileId}`,
          },
          client
        );
      }
   ...
   */
  async getCase(caseId: string): Promise<GetVerificationCaseResult> {
    const pool = getDbPool();
  ...
  async reviewCase(
    ...
    try {
      ...
      if (!row || row.profile_id !== profile.id)
        throw new ConflictException('Correction target changed');
      if (profile.archived && dto.decision !== 'Rejected')
        throw new ConflictException(
          'Archived profiles only allow rejection of outstanding corrections'
        );
      if (row.created_by === reviewerUserId)
        throw new ForbiddenException('A different staff member must review the correction');
      if (!(ALLOWED_TRANSITIONS[row.status] ?? []).includes(dto.decision))
        throw new ConflictException('Invalid correction state transition');
      const field = String(row.field_name);
      const allowed =
        profile.profile_type === 'LEGAL' ? IDENTITY_FIELDS_LEGAL : IDENTITY_FIELDS_INDIVIDUAL;
      if (dto.decision !== 'Rejected' && !allowed.includes(field))
        throw new BadRequestException('Invalid identity field for this profile type');
      if (dto.decision === 'Approved') {
        const keys =
          typeof row.evidence_urls === 'string' ? JSON.parse(row.evidence_urls) : row.evidence_urls;
        if (!Array.isArray(keys) || !keys.every((key) => typeof key === 'string'))
          throw new ConflictException('Correction evidence is invalid');
        ...
        const source =
          table === 'profiles'
            ? profile
            : (
                await client.query('SELECT * FROM legal_profiles WHERE id=$1 FOR UPDATE', [
                  profile.id,
                ])
              ).rows[0];
        if (!source || (source[field] ?? null) !== row.current_value)
          throw new ConflictException('Identity changed after this correction was requested');
        const updated = await client.query(
          `UPDATE ${table} SET ${field}=$1,updated_at=NOW() WHERE id=$2 RETURNING id`,
          [row.requested_value, profile.id]
        );
        if (updated.rows.length !== 1) throw new ConflictException('Identity record is missing');
      }
      await client.query(
        `UPDATE verification_cases SET status=$1,reviewed_by=$2,reviewed_at=NOW(),reviewer_notes=$3,updated_at=NOW() WHERE id=$4`,
        [dto.decision, reviewerUserId, dto.reviewerNotes?.trim() ?? null, caseId]
      );
      await client.query(
        `INSERT INTO audit_log(id,user_id,event,metadata,correlation_id,ip) VALUES ($1,$2,'verification_case_reviewed',$3::jsonb,$4,$5)`,
        [
          uuidv7(),
          reviewerUserId,
          JSON.stringify({
            ...
            newValue: dto.decision === 'Approved' ? row.requested_value : null,
          }),
          uuidv7(),
          ip,
        ]
...
function validateIdentityValue(field: string, value: unknown): void {
  if (typeof value !== 'string' || !value.trim() || value.trim().length > 512)
    throw new BadRequestException('Invalid identity value');
  if (field === 'national_id' && !validateNationalId(value.trim()))
    throw new BadRequestException('Invalid national ID');
  if (field === 'national_identifier' && !validateLegalNationalIdentifier(value.trim()))
    throw new BadRequestException('Invalid legal national identifier');
}
