...
async function profile(status: string) {
  return (
    await db.pool.query(
      "INSERT INTO profiles(user_id,status) VALUES ($1,$2) RETURNING id",
      ["verify-owner", status],
    )
  ).rows[0].id as string;
}
async function state(id: string) {
  return (await db.pool.query("SELECT status FROM profiles WHERE id=$1", [id]))
    .rows[0].status;
}
...
it.each(["unverify", "reverify"])(
  "rejects %s without a reason",
  async (action) => {
    const id = await profile("VERIFIED");
    expect(
      await service.verifyProfile(id, { action }, "verify-staff", ""),
...
it("rejects invalid actions/transitions and absent or archived profiles", async () => {
  ...
  expect(
    await service.verifyProfile(id, { action: "invalid" }, "verify-staff", ""),
  ).toHaveProperty("error");
  expect(
    await service.verifyProfile(
      await profile("DRAFT"),
...
it("resolves the owner after acquiring the profile lock", async () => {
  ...
  await client.query(
    "UPDATE profiles SET user_id='verify-new-owner' WHERE id=$1",
    [id],
  );
  const verifying = service.verifyProfile(
    id,
    { action: "verify" },
    "verify-staff",
    "",
