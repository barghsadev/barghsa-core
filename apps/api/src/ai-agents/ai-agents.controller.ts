...
export const CreateAgentSchema = z
  .object({
    ...
    maxTokens: z.number().int().min(1).max(8192).nullable().optional(),
    linkMode: z.enum(['any_kb', 'all_kbs']).optional(),
...
export class AgentsController {
  ...
  async create(
    ...
    return this.service.create({
      ...
      ...(parsed.data.linkMode !== undefined ? { linkMode: parsed.data.linkMode } : {}),
