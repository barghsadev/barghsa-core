...
describe('AgentsController (T-09.11.04)', () => {
  ...
  describe('permission gate (admin:ai:agents)', () => {
    it('rejects non-admin sessions on every route with 403', async () => {
      ...
      await expect(
        controller.get(nonAdminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')
      ...
      await expect(
        controller.update(nonAdminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
          title: 'y',
        } as never)
      ).rejects.toMatchObject({ status: 403 });
      await expect(
        controller.remove(nonAdminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')
      ).rejects.toMatchObject({ status: 403 });
      await expect(
        controller.addKb(nonAdminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
      ...
      await expect(
        controller.addPolicy(nonAdminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
  ...
  describe('PUT /api/admin/agents/:id', () => {
    it('rejects an empty update body', async () => {
      await expect(
        controller.update(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {} as never)
      ).rejects.toMatchObject({
        status: 400,
      });
    ...
    it('rejects an oversized link list (>200)', async () => {
      await expect(
        controller.update(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
          kbIds: new Array(201).fill('kb-x'),
        } as never)
    ...
    it('forwards only provided fields', async () => {
      ...
      const result = await controller.update(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
        enabled: false,
      } as never);
  ...
  describe('DELETE /api/admin/agents/:id', () => {
    it('deletes the agent', async () => {
      ...
      await expect(
        controller.remove(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')
  ...
  describe('KB links', () => {
    it('links a KB (POST /:id/kbs)', async () => {
      ...
      await expect(
        controller.addKb(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
          kbId: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc',
        } as never)
      ).resolves.toBeUndefined();
    ...
    it('rejects a missing kbId', async () => {
      await expect(
        controller.addKb(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {} as never)
      ).rejects.toMatchObject({
        status: 400,
      });
      expect(mockAddKb).not.toHaveBeenCalled();
  ...
  describe('policy links', () => {
    it('links a policy (POST /:id/policies)', async () => {
      ...
      await expect(
        controller.addPolicy(adminReq, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', {
          policyId: 'dddddddd-dddd-4ddd-8ddd-dddddddddddd',
        } as never)
      ).resolves.toBeUndefined();
