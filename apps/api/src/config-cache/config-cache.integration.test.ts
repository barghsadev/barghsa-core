...
afterAll(async () => {
  ...
  if (management) {
    try {
      // Pool shutdown may finish before PostgreSQL observes the socket closing.
      // Wait instead of force-terminating a client whose error listener is draining.
      await vi.waitFor(
        async () => {
          const result = await management.query<{ count: number }>(
