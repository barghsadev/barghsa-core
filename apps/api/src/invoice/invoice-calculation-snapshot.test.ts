...
describe('buildManualInvoiceCalculationSnapshot', () => {
  ...
  it('JSON round-trips without bigint or Number coercion', () => {
    const inputs = [{ description: 'x', quantity: 1, unitPrice: 1_000_000n, vatRate: 900 }];
    const snapshot = buildManualInvoiceCalculationSnapshot(inputs, calculateManualInvoice(inputs));
