...
export function assertValidAutoLine(line: AutoInvoiceLineInput): void {
  ...
  if (!Number.isInteger(line.vatRate) || line.vatRate < 0 || line.vatRate > 10_000) {
 ...
 */
export function calculateAutoLine(line: AutoInvoiceLineInput, discount = 0n): CalculatedAutoLine {
  ...
  const vatAmount = isTaxable ? roundHalfUpDiv(lineTotal * BigInt(line.vatRate), 10_000n) : 0n;
  return {
...
export function calculateAutoInvoice(
  ...
  const totalGross = lines.reduce((sum, l) => sum + BigInt(l.quantity) * l.unitPrice, 0n);
  ...
  if (remaining > 0n) {
    throw new RangeError(AUTO_INVOICE_ERRORS.DISCOUNT_EXCEEDS_INVOICE(orderDiscount, totalGross));
  }

  const totalAmount = calculated.reduce((sum, line) => sum + line.lineTotal + line.vatAmount, 0n);
