...
describe('resolveAmountError — numeric guards', () => {
  ...
  describe('PartiallyFunded target', () => {
    ...
    it('accepts when 0 < paid < total', () => {
      expect(resolveAmountError('PartiallyFunded', { ...base, paidAmount: 500_000n })).toBeNull();
    });
  ...
  describe('negative / zero invariants', () => {
    ...
    it('rejects negative incomingPaidAmount', () => {
      expect(resolveAmountError('Unpaid', { ...base, incomingPaidAmount: -1n })).toContain(
        'negative'
      );
    });
...
describe('cumulative refund completion', () => {
  it('requires the exact paid amount for the final refund and keeps partial refunds strictly partial', () => {
    ...
    expect(() =>
      validateTransition('PartiallyRefunded', 'Refunded', { ...amounts, refundedAmount: 99n })
    ).toThrow();
    expect(() =>
      validateTransition('PartiallyRefunded', 'Refunded', { ...amounts, refundedAmount: 101n })
    ).toThrow();
    for (const refundedAmount of [0n, 100n, 101n])
      expect(() =>
        validateTransition('Paid', 'PartiallyRefunded', { ...amounts, refundedAmount })
      ).toThrow();
    expect(() =>
      validateTransition('Paid', 'PartiallyRefunded', { ...amounts, refundedAmount: 40n })
    ).not.toThrow();
  });
});
