...
export class InvoiceBankReceiptConfirmationService {
  ...
  async get(receiptId: string): Promise<InvoiceBankReceiptConfirmDto> {
    const pool = getDbPool();
    const result = await pool.query(`SELECT ${RECEIPT_SELECT} FROM bank_receipts WHERE id = $1`, [
      receiptId,
    ]);
  ...
  async confirm(input: ConfirmInvoiceBankReceiptInput): Promise<InvoiceBankReceiptConfirmDto> {
    ...
    try {
      ...
      try {
        ...
        if (latestRequest) {
          ...
          if (latestRequest.status !== 'rejected') {
            ...
            if (latestRequest.status === 'approved') {
              if (
                !latestRequest.reviewerId ||
                latestRequest.reviewerId === latestRequest.initiatorId
              ) {
                httpError(
                  ErrorCodes.CONFLICT_STATE.code,
                  'Receipt has no distinct approval reviewer',
                  409
                );
              }
        ...
        const updated = await this.markConfirmed(client, receipt.id, input.actorUserId, now);
  ...
  private async parkForDualApproval(
    ...
    await notifyApprovalRequested(client, {
      requestId,
      amountIrR: input.receipt.amount.toString(),
  ...
  private async assertApprovalBinding(
    client: WalletQueryClient,
    receipt: BankReceiptRow,
    request: DualApprovalRequestSummary
  ): Promise<void> {
    ...
    ).rows[0] as { metadata?: Record<string, unknown> } | undefined;
    if (
      source?.metadata?.fingerprint !== invoiceReceiptFingerprint(receipt) ||
      String(request.amountIrR) !== String(receipt.amount) ||
      request.actionType !== INVOICE_BANK_RECEIPT_DUAL_APPROVAL_ACTION_TYPE
    ) {
      httpError(
        ErrorCodes.CONFLICT_STATE.code,
        'Receipt approval is unbound or its evidence changed; manual reconciliation is required',
        409
      );
    }
  }
  ...
  private async loadDualApprovalThreshold(
    ...
    const result = await queryable.query(`SELECT value FROM app_config WHERE key = $1`, [
      DUAL_APPROVAL_THRESHOLD_CONFIG_KEY,
    ]);
    const raw = result.rows[0] as { value?: unknown } | undefined;
    return readInvoiceBankReceiptDualApprovalThreshold(raw === undefined ? undefined : raw.value);
  }
  ...
  private async resolvePendingDualApprovalOnReject(
    ...
    const latestRequest = await this.lockLatestDualApprovalRequest(client, input.receiptId);
    if (latestRequest?.status !== 'pending') {
  ...
  private async ensureAndLockWallet(client: WalletQueryClient, profileId: string): Promise<void> {
    await client.query(
  ...
  private async confirmInvoiceAfterAllocation(
    ...
    if (from !== 'PaymentUnderReview') {
      await this.invoiceStateMachine.transition(input.invoice.id, from, 'PaymentUnderReview', {
        ...transitionOpts,
  ...
  private async enqueueCustomerRejectionNotice(
    ...
    const priority =
      classifyNotificationType(INVOICE_BANK_RECEIPT_REJECTED_NOTIFICATION_EVENT_KEY) === 'immediate'
        ? 'urgent'
        : 'normal';
  ...
  private async toDto(
    ...
    return {
      ...
      ...(extra.notificationOutboxId ? { notificationOutboxId: extra.notificationOutboxId } : {}),
    };
  }
...
export function invoiceBankReceiptConfirmationLockKeys(receiptId: string): [number, number] {
  const digest = createHash('sha256').update(`invoice-bank-receipt-confirm:${receiptId}`).digest();
  return [digest.readInt32BE(0), digest.readInt32BE(4)];
}

function invoiceStateAfterConfirm(invoice: InvoiceRow, invoiceAllocation: bigint): string {
