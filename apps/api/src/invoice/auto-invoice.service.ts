...
export class AutoInvoiceService {
   ...
   */
  async createInvoiceForOrder(cmd: CreateAutoInvoiceCommand): Promise<AutoInvoiceResult> {
    ...
    try {
      ...
      const giftDiscount =
        order.gift_discount_amount !== null ? BigInt(order.gift_discount_amount) : 0n;
