...
describe('AutoInvoiceService — real PostgreSQL integration (T-04.1.02.03)', () => {
  ...
  async function insertOrder(id: string, overrides: Record<string, unknown> = {}): Promise<void> {
    await ctx.db.execute(
      `INSERT INTO orders
         (id, user_id, profile_id, product_id, order_type, status,
          snapshot_province_id, snapshot_city_id, snapshot_full_address, snapshot_postal_code,
  ...
  it('applies the gift-code discount before VAT', async () => {
    ...
    const meta = (
      await ctx.db.execute<{ metadata: Record<string, unknown> | null }>(
  ...
  it('rolls back every row when the audit insert fails mid-transaction', async () => {
    ...
    const before = {
      ...
      lines: (await ctx.db.execute<{ n: number }>(`SELECT COUNT(*)::int AS n FROM invoice_lines`))
        .rows[0]!.n,
      items: (await ctx.db.execute<{ n: number }>(`SELECT COUNT(*)::int AS n FROM invoice_items`))
        .rows[0]!.n,
  ...
  it('computes dueAt as issuedAt + electricity service_due_periods days', async () => {
    ...
    expect(new Date(result.dueAt!).getTime() - issuedAt.getTime()).toBe(21 * 24 * 60 * 60 * 1000);
