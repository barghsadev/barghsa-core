...
export class InvoiceStateMachineService {
  ...
  async transition(
    ...
    try {
      ...
      if (opts.reason) {
        ...
        this.logger.debug(`Transition ${from}→${to} on ${invoiceId}: ${opts.reason}`);
      }
      ...
      await client.query(`UPDATE invoices SET ${setClauses.join(', ')} WHERE id = $1`, values);
      ...
      this.logger.log(`Invoice ${invoiceId} ${from} → ${to} (${label}) by ${opts.actorUserId}`);
      ...
      throw new InternalServerErrorException(`Invoice transition failed: ${String(error)}`);
    } finally {
  ...
  canSubmitBankReceipt(from: InvoiceState, adjustmentKind?: string | null): boolean {
  ...
  canPayFromWallet(from: InvoiceState, adjustmentKind?: string | null): boolean {
    if (adjustmentKind === 'credit') return false;
    return from === 'Unpaid' || from === 'PartiallyFunded' || from === 'Overdue';
  }

  canMarkOverdue(from: InvoiceState, adjustmentKind?: string | null): boolean {
    if (adjustmentKind === 'credit') return false;
