...
describe('calculateManualInvoice', () => {
  ...
  it('rejects non-positive or non-integer quantity', () => {
    const base: ManualInvoiceLineInput = {
      description: 'x',
      quantity: 1,
      unitPrice: 100n,
      vatRate: 0,
    };
    expect(() => calculateManualInvoice([{ ...base, quantity: 0 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_QUANTITY()
    );
    expect(() => calculateManualInvoice([{ ...base, quantity: -1 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_QUANTITY()
    );
    expect(() => calculateManualInvoice([{ ...base, quantity: 1.5 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_QUANTITY()
    );
  });
  ...
  it('rejects VAT rates outside 0..10000 basis points', () => {
    ...
    expect(() => calculateManualInvoice([{ ...base, vatRate: -1 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_VAT_RATE()
    );
    expect(() => calculateManualInvoice([{ ...base, vatRate: 10_001 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_VAT_RATE()
    );
    expect(() => calculateManualInvoice([{ ...base, vatRate: 1.5 }])).toThrow(
      MANUAL_LINE_ERRORS.BAD_VAT_RATE()
    );
  });
});
