...
describe('InvoiceBankReceiptUploadService — real PostgreSQL (T-04.3.01.02)', () => {
  ...
  it('rejects a zero amount before insert', async () => {
    ...
    await insertReceiptFile(attachment);
    await expect(
      service.submit(payload({ amount: 0, attachmentKey: attachment }))
  ...
  it('rejects an oversize stored PDF', async () => {
    ...
    const rejection = await service
      .submit(payload({ attachmentKey: attachment }))
      .catch((e: unknown) => e);
  ...
  it('keeps the sealed copy when the original upload is overwritten after submit', async () => {
    ...
    expect(objects.get(sealed)?.subarray(0, 5)).toEqual(new TextEncoder().encode('%PDF-'));
    expect(objects.get(attachment)?.subarray(0, 4)).toEqual(
  ...
  it('inserts exactly one row under concurrent same-attachment retries', async () => {
    ...
    const count = await ctx.pool.query(`SELECT count(*)::int AS n FROM bank_receipts`);
    expect(count.rows[0]!.n).toBe(1);
    const claims = await ctx.pool.query(
      `SELECT count(*)::int AS n FROM bank_receipt_attachment_claims`
    );
