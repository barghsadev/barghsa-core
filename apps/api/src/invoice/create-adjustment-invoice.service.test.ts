...
describe('CreateAdjustmentInvoiceService', () => {
  ...
  it('issues a linked additional-charge invoice atomically', async () => {
    ...
    const originalUpdates = mockClient.query.mock.calls.filter((c) =>
      (c[0] as string).startsWith('UPDATE invoices')
    );
    const stateUpdates = originalUpdates.filter((c) => (c[0] as string).includes('SET state'));
    expect(stateUpdates).toHaveLength(1);
    expect((stateUpdates[0]![1] as unknown[])[0]).toBe(adjustmentId);

    const metadataUpdate = originalUpdates.find((c) => (c[0] as string).includes('metadata'));
  ...
  it('stores a credit as a non-negative total with null due_at', async () => {
    ...
    const payableClear = mockClient.query.mock.calls.filter((c) =>
      (c[0] as string).includes('payable_from = NULL')
    );
    expect(payableClear).toHaveLength(1);

    const dueLookups = mockClient.query.mock.calls.filter((c) =>
      (c[0] as string).includes('FROM service_due_periods')
    );
  ...
  it('rejects an empty reason without opening a connection', async () => {
    await expect(service.createAdjustmentInvoice(command({ reason: '   ' }))).rejects.toThrow(
      BadRequestException
    );
  ...
  it('rejects a zero amount without opening a connection', async () => {
    await expect(service.createAdjustmentInvoice(command({ amount: 0n }))).rejects.toThrow(
      CREATE_ADJUSTMENT_ERRORS.AMOUNT_ZERO()
    );
    expect(mockPool.connect).not.toHaveBeenCalled();
  });
  ...
  it('throws NotFoundException when the original is missing and rolls back', async () => {
    ...
    await expect(service.createAdjustmentInvoice(command())).rejects.toThrow(NotFoundException);
  ...
  it('rejects an unpaid invoice with ConflictException', async () => {
    ...
    await expect(rejection).rejects.toThrow(CREATE_ADJUSTMENT_ERRORS.NO_PAYMENT('inv-original'));
    const calls = mockClient.query.mock.calls.map((c) =>
      (c[0] as string).trim().split(/\s+/)[0]!.toUpperCase()
    );
    expect(calls).toContain('ROLLBACK');
