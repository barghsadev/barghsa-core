...
function script(opts: ScriptOptions = {}) {
  mockClient.query.mockImplementation(async (sql: string) => {
    ...
    if (sql.includes('SELECT metadata::jsonb AS metadata FROM audit_log'))
      return {
        rows: [
          {
            metadata: {
              fingerprint: invoiceReceiptFingerprint(
                (opts.locked ?? makeReceiptRow()) as Parameters<typeof invoiceReceiptFingerprint>[0]
              ),
            },
          },
        ],
    ...
    if (sql.includes('FROM approval_requests')) {
      return {
        rows: opts.pendingApproval
          ? [
              {
                action_type: INVOICE_BANK_RECEIPT_DUAL_APPROVAL_ACTION_TYPE,
                amount_irr: opts.locked?.amount ?? makeReceiptRow().amount,
...
describe('InvoiceBankReceiptConfirmationService dual-approval (T-04.3.01.05)', () => {
  ...
  it('lets a second finance staff member complete the parked confirmation', async () => {
    ...
    expect(
      mockClient.query.mock.calls.some(([sql]) => String(sql).includes('UPDATE approval_requests'))
    ).toBe(true);
