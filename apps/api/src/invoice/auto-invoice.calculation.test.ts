...
describe('calculateAutoLine', () => {
  ...
  it('throws when the discount exceeds the gross subtotal', () => {
    expect(() => calculateAutoLine(line({ unitPrice: 100_000n }), 100_001n)).toThrow(RangeError);
  });
...
describe('calculateAutoInvoice', () => {
  it('computes the invoice total as Σ(lineTotal + vatAmount)', () => {
    const result = calculateAutoInvoice([line({ unitPrice: 1_000_000n, vatRate: 900 })]);
  ...
  it('applies the order gift discount before VAT', () => {
    ...
    const result = calculateAutoInvoice([line({ unitPrice: 2_000_000n, vatRate: 900 })], 250_000n);
  ...
  it('throws when the order discount exceeds the taxable subtotal', () => {
    expect(() => calculateAutoInvoice([line({ unitPrice: 100_000n })], 100_001n)).toThrow(
      RangeError
    );
  });
  ...
  it('allows a 100%-coverage gift discount (zero total)', () => {
    ...
    const result = calculateAutoInvoice([line({ unitPrice: 100_000n, vatRate: 900 })], 100_000n);
  ...
  it('allocates the discount across lines in order', () => {
    const result = calculateAutoInvoice(
      [line({ unitPrice: 1_000_000n, vatRate: 900 }), line({ unitPrice: 500_000n, vatRate: 900 })],
      800_000n
    );
  ...
  it('spills the discount to the next line when the first is fully covered', () => {
    const result = calculateAutoInvoice(
      [line({ unitPrice: 500_000n, vatRate: 900 }), line({ unitPrice: 500_000n, vatRate: 900 })],
      800_000n
    );
  ...
  it('rejects a zero total that no discount produced', () => {
    ...
    expect(() => calculateAutoInvoice([line({ unitPrice: 0n })])).toThrow(
      AUTO_INVOICE_ERRORS.ZERO_TOTAL_WITHOUT_DISCOUNT()
    );
  });
});
...
describe('autoLineDescription', () => {
  it('prefers the fa title, falls back to en, then to the product type', () => {
    ...
    expect(autoLineDescription(line({ productTitle: { fa: null, en: 'Thermal' } }))).toBe(
      'Thermal'
    );
