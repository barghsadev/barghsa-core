...
describe('AutoInvoiceService', () => {
  ...
  describe('createInvoiceForOrder (standalone, owns its transaction)', () => {
    it('creates and issues the invoice atomically on one client', async () => {
      ...
      const readIdx = mockClient.query.mock.calls.findIndex((c) =>
        (c[0] as string).startsWith('SELECT id, order_id, profile_id')
      );
      const commitIdx = mockClient.query.mock.calls.findIndex((c) => (c[0] as string) === 'COMMIT');
      expect(readIdx).toBeGreaterThanOrEqual(0);
      expect(commitIdx).toBeGreaterThan(readIdx);

      const insertCall = mockClient.query.mock.calls.find((c) =>
        (c[0] as string).startsWith('INSERT INTO invoices')
      );
    ...
    it('applies an explicit VAT rate and the gift discount before VAT', async () => {
      ...
      const result = await service.createInvoiceForOrder(command({ vatRateBasisPoints: 900 }));
    ...
    it('throws NotFoundException for a missing order', async () => {
      ...
      await expect(service.createInvoiceForOrder(command())).rejects.toThrow(NotFoundException);
    ...
    it('throws BadRequestException for a cancelled order', async () => {
      ...
      await expect(service.createInvoiceForOrder(command())).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws ConflictException when the order already has an auto invoice', async () => {
      ...
      await expect(service.createInvoiceForOrder(command())).rejects.toThrow(ConflictException);
    });

    it('throws BadRequestException for a dueAt in the past', async () => {
      await expect(
        service.createInvoiceForOrder(command({ dueAt: new Date('2026-07-01T00:00:00.000Z') }))
    ...
    it('rolls back when the Issue transition fails', async () => {
      ...
      await expect(service.createInvoiceForOrder(command())).rejects.toThrow('DB down');
  ...
  describe('createInvoiceForOrder (joins a caller-owned transaction)', () => {
    it('never opens its own connection or BEGIN/COMMIT/ROLLBACK when a client is passed', async () => {
      ...
      mockClient.query.mockImplementation(async (sql: string, params?: unknown[]) => {
        ...
        if (s.startsWith('SELECT rate') && s.includes('vat_configurations'))
          return { rows: [{ rate: 900 }] };
        if (s.startsWith('INSERT INTO invoices')) {
      ...
      const result = await service.createInvoiceForOrder(command({ client: incomingClient }));
