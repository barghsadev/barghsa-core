...
function command(overrides: Record<string, unknown> = {}) {
  return {
    ...
    lines: [{ description: 'برق مصرفی', quantity: 1, unitPrice: 1_000_000n, vatRate: 900 }],
  ...
  describe('createManualInvoice', () => {
    it('creates and issues the invoice atomically on one client', async () => {
      ...
      mockQuery((sql) => {
        ...
        if (sql.startsWith('SELECT id, profile_id'))
          return {
            rows: [
              {
    ...
    it('rejects an empty line list lifecycle', async () => {
      await expect(service.createManualInvoice(command({ lines: [] }))).rejects.toThrow(
        BadRequestException
      );
    });
    ...
    it('throws NotFoundException for a missing profile and rolls back', async () => {
      ...
      await expect(service.createManualInvoice(command())).rejects.toThrow(NotFoundException);
    ...
    it('replays the existing invoice when the idempotency key is reused', async () => {
            ...
            rows: [
              {
                ...
                metadata: { fingerprint: fingerprintForTest() },
              },
            ],
          };
        if (sql.startsWith('SELECT id FROM audit_log'))
          return {
      ...
      const inserts = mockClient.query.mock.calls.filter((c) =>
        (c[0] as string).startsWith('INSERT INTO invoices')
      );
      expect(inserts).toHaveLength(0);
      // The state machine was not invoked for the replay
      const lockCalls = mockClient.query.mock.calls.filter((c) =>
    ...
    it('rejects an idempotency key reused with a different payload', async () => {
      ...
      await expect(
        service.createManualInvoice(command({ idempotencyKey: 'manual-inv-123' }))
    ...
    it('rolls back when the Issue transition fails', async () => {
      ...
      await expect(service.createManualInvoice(command())).rejects.toThrow('DB down');
