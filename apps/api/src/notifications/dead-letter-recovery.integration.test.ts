...
beforeAll(async () => {
  ...
  await db.pool.query("INSERT INTO users(user_id,username,password_hash) VALUES ('triage-staff','triage@example.test','test-only')")
  profileId = (await db.pool.query("INSERT INTO profiles(user_id) VALUES ('triage-staff') RETURNING id")).rows[0].id
}, 40000)
afterAll(async () => { await fixture?.close() })
async function seed() {
  const outbox = randomUUID(), job = randomUUID(), dead = randomUUID()
  await db.pool.query(`INSERT INTO notification_outbox(id,profile_id,event_key,payload,channels,idempotency_key,status)
    VALUES ($1::uuid,$2,'invoice.created','{}',ARRAY['email'],$1::text,'failed')`, [outbox, profileId])
  await db.pool.query(`INSERT INTO notification_job(id,outbox_id,channel,status,attempts,delivery_payload)
    VALUES ($1,$2,'email','dead_letter',5,'{"preserved":"snapshot"}')`, [job, outbox])
  await db.pool.query(`INSERT INTO notification_dead_letter(id,outbox_id,job_id,channel,event_key,profile_id,attempts,idempotency_key)
    VALUES ($1::uuid,$2,$3,'email','invoice.created',$4,5,$1::text)`, [dead, outbox, job, profileId])
  return { outbox, job, dead }
}
it('requeues once under concurrent actions and keeps the snapshot and audit evidence', async () => {
  const row = await seed()
  await Promise.all(Array.from({ length: 8 }, () => service.deadLetterAction(row.dead, 'retry', 'triage-staff')))
  expect((await db.pool.query('SELECT status,attempts,delivery_payload FROM notification_job WHERE id=$1', [row.job])).rows[0])
    .toEqual({ status: 'queued', attempts: 0, delivery_payload: { preserved: 'snapshot' } })
  expect((await db.pool.query("SELECT count(*)::int AS count FROM audit_log WHERE event='notification_dead_letter_action' AND metadata::jsonb->>'deadLetterId'=$1", [row.dead])).rows[0].count).toBe(1)
  await db.pool.query("UPDATE notification_job SET status='done',attempts=1,provider_ref='real-receipt' WHERE id=$1", [row.job])
  await db.pool.query("UPDATE notification_outbox SET status='delivered' WHERE id=$1", [row.outbox])
  await service.deadLetterAction(row.dead, 'retry', 'triage-staff')
  expect((await db.pool.query('SELECT status,attempts,provider_ref FROM notification_job WHERE id=$1', [row.job])).rows[0])
...
it('cannot steal an active worker claim or revive a cancelled event', async () => {
  const row = await seed()
  await db.pool.query("UPDATE notification_outbox SET status='sending',lease_token='worker-claim',locked_until=NOW()+INTERVAL '1 minute' WHERE id=$1", [row.outbox])
  await expect(service.deadLetterAction(row.dead, 'retry', 'triage-staff')).rejects.toMatchObject({ status: 409 })
  expect((await db.pool.query('SELECT lease_token FROM notification_outbox WHERE id=$1', [row.outbox])).rows[0].lease_token).toBe('worker-claim')
  await db.pool.query("UPDATE notification_outbox SET status='cancelled',locked_until=NULL WHERE id=$1", [row.outbox])
  await expect(service.deadLetterAction(row.dead, 'retry', 'triage-staff')).rejects.toMatchObject({ status: 409 })
})
it('never retries a resolved record or a job already delivered', async () => {
  ...
  expect(await service.deadLetterAction(row.dead, 'retry', 'triage-staff')).toMatchObject({ status: 'resolved' })
  expect((await db.pool.query('SELECT attempts FROM notification_job WHERE id=$1', [row.job])).rows[0].attempts).toBe(5)
