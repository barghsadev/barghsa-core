...
describe('truthful notification template tests', () => {
  it.each(['email', 'sms'] as const)('refuses to mark %s delivered through the inbox', async channel => {
    ...
    await expect(instance.testSend('template', 'staff', { destination: 'staff@example.test' })).rejects.toMatchObject({ status: 503 })
...
it('uses the email provider receipt and never inserts an inbox substitute', async () => {
  ...
  expect(await instance.testSend('template', 'staff', { destination: 'staff@example.test' })).toEqual({ ok: true, destination: 'email', lastTestStatus: 'delivered' })
  expect(send).toHaveBeenCalledWith(expect.objectContaining({ destination: 'staff@example.test', subject: 'Invoice', html: 'Test body' }))
  expect(create).not.toHaveBeenCalled()
  const audit = query.mock.calls.find(([sql]) => sql.includes('INSERT INTO audit_log'))
  expect(JSON.parse(audit![1][3])).toMatchObject({ deliveredTo: 'email', providerRef: 'email-receipt', status: 'delivered' })
})
