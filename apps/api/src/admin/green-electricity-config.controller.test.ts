...
describe('green-electricity-rules config permission gate (T-09.10.02)', () => {
  ...
  it('does not call the service when the permission gate rejects', async () => {
    ...
    await controller.setGreenElectricityRules(VALID_BODY, nonAdminReq).catch((e: unknown) => e);
  ...
  it('safety-status response shape matches the documented OpenAPI contract (camelCase)', async () => {
    ...
    expect(Object.keys(result).sort()).toEqual(['product', 'simpleOrder', 'advancedOrder'].sort());
