...
const configurations=[
  {path:'config/service-response-targets',key:'admin.service_response_targets',value:{ticket:24},invalid:{ticket:1.5}},
  {path:'config/escalation-policy',key:'admin.escalation_policy',value:{ticket:{level2:{delayHours:2,channels:['in_app','email']},level3:{delayHours:3,channels:['in_app']}}},invalid:{ticket:{level2:{delayHours:2,channels:['email']},level3:{delayHours:3,channels:['in_app']}}}},
]
for(const config of configurations){
  it(`${config.path} enforces permissions, confirmation, validation and disable`,async()=>{
    ...
    try { expect((await call(config.path,'PUT',config.value)).status).toBe(403) }
    ...
    expect(await (await call(config.path)).json()).toEqual({ticket:null,verification_case:null})
  })
  it(`${config.path} serializes first writes and rolls audit failures back`,async()=>{
    ...
    const before=(await http.pool.query("SELECT version FROM config_version WHERE id='global'")).rows[0].version
    const responses=await Promise.all(Array.from({length:5},()=>call(config.path,'PUT',config.value)))
    expect(responses.map(response=>response.status)).toEqual([200,200,200,200,200])
    const audits=(await http.pool.query("SELECT metadata::jsonb AS metadata FROM audit_log WHERE metadata::jsonb->>'key'=$1 ORDER BY (metadata::jsonb->>'version')::int",[config.key])).rows
    ...
    expect((await http.pool.query("SELECT version FROM config_version WHERE id='global'")).rows[0].version).toBe(before+5)
    await http.pool.query(`CREATE FUNCTION fail_target_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected audit failure'; END $$;
      CREATE TRIGGER fail_target_audit BEFORE INSERT ON audit_log FOR EACH ROW EXECUTE FUNCTION fail_target_audit()`)
    try { expect((await call(config.path,'PUT',{})).status).toBe(500) }
    finally {await http.pool.query('DROP TRIGGER fail_target_audit ON audit_log; DROP FUNCTION fail_target_audit()')}
    expect((await http.pool.query('SELECT version,value FROM app_config WHERE key=$1',[config.key])).rows[0]).toMatchObject({version:5,value:config.value})
    expect((await http.pool.query("SELECT version FROM config_version WHERE id='global'")).rows[0].version).toBe(before+5)
    expect((await call(config.path,'PUT',{})).status).toBe(200)
  })
}
