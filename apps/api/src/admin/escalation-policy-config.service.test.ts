...
import { DEFAULT_ESCALATION_POLICIES, ESCALATION_POLICY_CONFIG_KEY } from '@barghsa/shared/admin';
  ...
  it('rejects a level with missing in_app channel with a 400', async () => {
    ...
    await expect(
      service.setEscalationPolicy(
        {
          ticket: {
            level2: { delayHours: 24, channels: ['email'] },
            level3: { delayHours: 48, channels: ['in_app'] },
          },
        },
        'admin-1',
    ...
    for (const bad of [0, -1, 1.5, 8761, '24']) {
      await expect(
        service.setEscalationPolicy(
          {
            ticket: {
              level2: { delayHours: bad, channels: ['in_app'] },
              level3: { delayHours: 48, channels: ['in_app'] },
            },
          },
          'admin-1',
