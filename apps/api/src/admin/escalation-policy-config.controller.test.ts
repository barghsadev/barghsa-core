...
function makeController() {
  const getEscalationPolicy = vi.fn().mockResolvedValue(DEFAULT_ESCALATION_POLICIES);
...
describe('escalation-policy config permission gate (T-09.08.03)', () => {
  it('rejects non-admin on getEscalationPolicy with the AUTHZ_FORBIDDEN contract', async () => {
    ...
    const rejection = await controller.getEscalationPolicy(nonAdminReq).catch((e: unknown) => e);
  ...
  it('does not call the service when the permission gate rejects', async () => {
    ...
    await controller.setEscalationPolicy({ ticket: null }, nonAdminReq).catch((e: unknown) => e);
