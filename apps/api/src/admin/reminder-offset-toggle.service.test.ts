...
describe('ReminderOffsetToggleService.list (T-04.1.04.05)', () => {
  ...
  it('overlays stored disabled rows onto the defaults', async () => {
    ...
    const result = await service.list();
    expect(
      result.find((row) => row.serviceType === 'electricity' && row.offset === -7)?.enabled
    ).toBe(false);
    expect(
      result.find((row) => row.serviceType === 'electricity' && row.offset === -3)?.enabled
    ).toBe(true);
  });
});
...
describe('ReminderOffsetToggleService.set (T-04.1.04.05)', () => {
  it('rejects an invalid body with 400', async () => {
    ...
    const rejection = await service
      .set({
        raw: { serviceType: 'hardware', offset: -7, enabled: true },
        actorUserId: ACTOR,
        ip: '127.0.0.1',
      })
      .catch((e: unknown) => e);
  ...
  it('upserts the toggle, writes an audit event, and returns the merged matrix', async () => {
    ...
    expect(result.find((row) => row.serviceType === 'manual' && row.offset === 0)?.enabled).toBe(
      false
    );
    expect(
      result.find((row) => row.serviceType === 'electricity' && row.offset === 0)?.enabled
    ).toBe(true);
  ...
  it('locks the pair with an advisory lock before reading previousEnabled', async () => {
    ...
    const lock = router.queries('pg_advisory_xact_lock')[0];
    expect(lock?.values).toEqual(['barghsa.invoice_reminder_offset_toggles', 'electricity', -7]);
    const audit = router.queries('INSERT INTO audit_log')[0];
