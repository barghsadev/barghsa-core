...
const MOCK_ROLES = [
  {
    id: 'role-customer-support' as const,
    name: 'Customer Support',
    description: '',
    permissions: [],
  },
  {
    id: 'role-crm-verification' as const,
  ...
  {
    id: 'role-legal-contracts' as const,
  ...
  it('creates a staff user with temporary password and returns it', async () => {
    ...
    mockClientQuery
      ...
      .mockResolvedValueOnce({
        rows: [{ user_id: 'new-user-id', username: 'newstaff@example.com' }],
      })
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined);
...
describe('AdminService.createStaffUser (link)', () => {
  ...
  it('creates a profile with INDIVIDUAL type and VERIFIED status', async () => {
    ...
    const insertProfileCall = mockClientQuery.mock.calls.find((call) =>
...
describe('AdminService.updateStaffRoles', () => {
  it('updates roles for an existing user (replaces role set)', async () => {
    ...
    mockClientQuery
      ...
      .mockResolvedValueOnce({
        rows: [{ role_id: 'role-customer-support' }, { role_id: 'role-finance' }],
      }) // current roles
      ...
      rows: [
        {
          ...
          name: 'X',
          description: 'x',
          permissions: ['tickets:read', 'crm:read'],
          created_at: null,
          updated_at: null,
        },
      ],
        ...
        rows: [
          {
            role_id: 'role-crm',
            name: 'CRM & Verification',
            permissions: JSON.stringify(['crm:read', 'profiles:read']),
          },
          {
...
describe('AdminService.disableStaff', () => {
  it('disables the account, revokes sessions/tokens, and records an audit entry', async () => {
    ...
    mockClientQuery
      .mockResolvedValueOnce(undefined) // BEGIN
      .mockResolvedValueOnce({
        rows: [{ user_id: 'u-target', username: 'staff@example.com', disabled_at: null }],
      })
    ...
    expect(sqlCalls.some((s) => s.includes('UPDATE sessions') && s.includes('revoked_at'))).toBe(
      true
    );
    expect(
      sqlCalls.some((s) => s.includes('UPDATE refresh_tokens') && s.includes('consumed_at'))
    ).toBe(true);

    // Audit entry carries actor + target
    const auditCall = mockClientQuery.mock.calls.find((c) =>
  ...
  it('rejects disabling your own account', async () => {
    ...
    mockClientQuery
      .mockResolvedValueOnce(undefined) // BEGIN
      .mockResolvedValueOnce({
        rows: [{ user_id: 'u-admin', username: 'admin@example.com', disabled_at: null }],
      })
        ...
        rows: [
          {
            ...
            disabled_at: new Date('2026-08-01T00:00:00Z'),
          },
        ],
      })
      .mockResolvedValueOnce(undefined); // COMMIT
...
describe('AdminService.listStaffAudit', () => {
  ...
  it('computes removals and falls back to roleId when a role has no staff_roles row', async () => {
    ...
    expect(result.items[0]!.removedRoles).toEqual([{ roleId: 'role-ops', roleName: 'role-ops' }]);
  ...
  it('passes the userId and date-range filters into both SQL queries', async () => {
    ...
    pool.query.mockResolvedValueOnce({ rows: [{ total: 0 }] }).mockResolvedValue({ rows: [] });

    vi.doMock('@barghsa/db', () => mockDbModule(pool));
    const { AdminService: Svc } = await import('./admin.service.js');
    service = new Svc();

    await service.listStaffAudit({
