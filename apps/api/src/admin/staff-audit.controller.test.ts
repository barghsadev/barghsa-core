...
describe('staff permission audit endpoint (T-10.01.02)', () => {
  it('rejects non-admin with the AUTHZ_FORBIDDEN contract and does not call the service', async () => {
    ...
    const rejection = await controller
      .listStaffAudit(undefined, undefined, undefined, undefined, undefined, nonAdminReq)
      .catch((e: unknown) => e);
  ...
  it('returns the service result to the caller', async () => {
    ...
    await expect(
      controller.listStaffAudit(undefined, undefined, undefined, undefined, undefined, adminReq)
