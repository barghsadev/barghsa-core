...
function makeController() {
  ...
  const setWalletTopUpLimitConfig = vi.fn().mockResolvedValue({ limitIrR: 1_000_000_000 });
  const adminService = {
...
describe('wallet-top-up-limit config permission gate (T-09.10.01)', () => {
  it('rejects non-admin on getWalletTopUpLimit with the AUTHZ_FORBIDDEN contract', async () => {
    ...
    const rejection = await controller.getWalletTopUpLimit(nonAdminReq).catch((e: unknown) => e);
