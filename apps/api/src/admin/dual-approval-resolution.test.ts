...
describe('applyApprovalRequestResolutionOnClient', () => {
  it('approves a pending request and writes approval_request_approved', async () => {
    ...
    const notice = client.query.mock.calls.find((call) =>
      String(call[0]).includes('INSERT INTO in_app_notifications')
    );
    expect(notice?.[1]?.[1]).toBe('user-1');
    expect(String(client.query.mock.calls[0]![0])).toContain('UPDATE approval_requests');
    expect(client.query.mock.calls[0]![1]).toEqual(['approved', 'user-2', null, BASE.now, 'req-1']);
