...
export class VerificationProviderRegistry {
  ...
  private readonly providers = new Map<
    string,
    { adapter: VerificationProviderAdapter; breaker: CircuitBreaker }
  >();
  ...
  register(adapter: VerificationProviderAdapter): void {
    ...
    this.logger.log(
      `Verification provider registered: ${adapter.providerId} (${adapter.displayName})`
    );
  }
  ...
  async verify(
    ...
    const timeoutMs =
      rawTimeoutMs !== undefined && Number.isFinite(Number(rawTimeoutMs))
        ? Number(rawTimeoutMs)
        : 10_000;
    const rawMaxRetries = config?.settings?.['maxRetries'];
    const maxRetries =
      rawMaxRetries !== undefined && Number.isFinite(Number(rawMaxRetries))
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.warn(
            `Verification attempt ${attempt + 1} failed for provider "${providerId}": ${String(error)}. Retrying in ${delay}ms`
          );
    ...
    return {
      ...
      rawResponse: {
        error: isTimeout
          ...
          : VerificationErrorCodes.PROVIDER_RESPONSE_ERROR,
      },
    };
  }
}
