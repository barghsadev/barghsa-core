...
describe('PayInvoiceWithWalletService — concurrent PostgreSQL (T-04.2.03.04)', () => {
  ...
  async function seedInFlightClaim(key: string, entityId: string, expiresAt: Date): Promise<void> {
    await ctx.pool.query(
  ...
  function rejectedReasons(settled: PromiseSettledResult<PayInvoiceWithWalletResult>[]): unknown[] {
    return settled
  ...
  it('lets exactly one of three concurrent distinct-key payments succeed', async () => {
    ...
    const keys = [`pay-race-a-${invoiceId}`, `pay-race-b-${invoiceId}`, `pay-race-c-${invoiceId}`];
