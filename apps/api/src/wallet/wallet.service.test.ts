...
describe('WalletService', () => {
  ...
  describe('createWallet', () => {
    it('creates a new wallet', async () => {
      ...
      expect(result.profileId).toBe('profile-1');
      expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO wallets'), [
        'profile-1',
      ]);
    });
  ...
  describe('credit (T-04.2.01.03)', () => {
    ...
    it('inserts a Completed ledger row and updates postedBalance under version + postedBalance >= 0', async () => {
      ...
      const insertCall = mockClient.query.mock.calls.find(
        (c) =>
          typeof c[0] === 'string' && (c[0] as string).includes('INSERT INTO wallet_transactions')
      );
    ...
    it('returns the existing transaction on idempotent retry without mutating the wallet', async () => {
      ...
      const result = await service.credit('profile-1', 100000n, { type: 'topup' }, 'idem-001');
    ...
    it('rejects zero or negative credit without opening a transaction', async () => {
      await expect(service.credit('profile-1', 0n, { type: 'topup' }, 'k')).rejects.toThrow(
        'Credit amount must be positive'
      );

      await expect(service.credit('profile-1', -100n, { type: 'topup' }, 'k')).rejects.toThrow(
        'Credit amount must be positive'
      );
    ...
    it('rejects a blank idempotency key', async () => {
      await expect(service.credit('profile-1', 100n, { type: 'topup' }, '   ')).rejects.toThrow(
    ...
    it('rejects a non-credit ledger type', async () => {
      await expect(
        service.credit('profile-1', 100n, { type: 'payment' } as unknown as { type: 'topup' }, 'k')
    ...
    it('rejects reversal credits so reverseTransaction remains the only writer (T-04.2.04.01)', async () => {
      await expect(
        service.credit('profile-1', 100n, { type: 'reversal' } as unknown as { type: 'topup' }, 'k')
    ...
    it('throws NotFound when the wallet row is missing', async () => {
      ...
      await expect(service.credit('missing', 100n, { type: 'topup' }, 'k')).rejects.toThrow(
        'Wallet not found: missing'
      );
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });
    ...
    it('returns the committed ledger row when INSERT races on the unique idempotency index', async () => {
      ...
      expect(result.id).toBe('tx-001');
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
      expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('idempotency_key'), [
        'idem-001',
      ]);
    });
  ...
  describe('debit (T-04.2.01.04)', () => {
    ...
    it('rejects zero or negative debit without opening a transaction', async () => {
      await expect(service.debit('profile-1', 0n, { type: 'payment' }, 'k')).rejects.toThrow(
        'Debit amount must be positive'
      );

      await expect(service.debit('profile-1', -100n, { type: 'payment' }, 'k')).rejects.toThrow(
        'Debit amount must be positive'
      );
    ...
    it('rejects a blank idempotency key', async () => {
      await expect(service.debit('profile-1', 100n, { type: 'payment' }, '   ')).rejects.toThrow(
        'Idempotency key is required'
      );
      expect(mockPool.connect).not.toHaveBeenCalled();
    });
    ...
    it('rejects a non-debit ledger type', async () => {
      await expect(
        service.debit('profile-1', 100n, { type: 'topup' } as unknown as { type: 'payment' }, 'k')
    ...
    it('throws NotFound when the wallet row is missing', async () => {
      ...
      await expect(service.debit('missing', 100n, { type: 'payment' }, 'k')).rejects.toThrow(
        'Wallet not found: missing'
      );
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });
    ...
    it('rejects when the locked version differs from expectedVersion (T-04.2.03.02)', async () => {
      ...
      await expect(
        service.debit('profile-1', 100000n, { type: 'payment', expectedVersion: 99 }, 'idem-001')
  ...
  describe('reserve (T-04.2.01.05)', () => {
    ...
    it('binds the reserved_balance UPDATE and ledger insert to the wallet row canonical profile_id', async () => {
      ...
      mockClient.query
        ...
        .mockResolvedValueOnce({
          rows: [makeTxRow({ wallet_id: canonical, type: 'reservation', state: 'Reserved' })],
        });
    ...
    it('rejects a colliding idempotency key owned by another wallet', async () => {
      ...
      mockClient.query
        ...
        .mockResolvedValueOnce({ rows: [wallet] })
        .mockResolvedValueOnce({
          rows: [makeTxRow({ wallet_id: 'profile-other', type: 'reservation', state: 'Reserved' })],
        });
    ...
    it('throws NotFound when the wallet row is missing', async () => {
      ...
      await expect(service.reserve('missing', 100n, 'k')).rejects.toThrow(
        'Wallet not found: missing'
      );
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });
  ...
  describe('release (T-04.2.01.05)', () => {
    ...
    it('binds wallet and ledger UPDATEs to the locked row canonical ids', async () => {
      ...
      mockClient.query
        ...
        .mockResolvedValueOnce({
          rows: [
            makeWalletRow({ profile_id: canonicalWallet, reserved_balance: '150000', version: 2 }),
          ],
        })
        .mockResolvedValueOnce({ rows: [released] });
    ...
    it('throws NotFound when the reservation is missing and rolls back', async () => {
      ...
      await expect(service.release('missing-res')).rejects.toThrow(
        'Reservation not found: missing-res'
      );
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
  ...
  describe('applyPostedBalanceDelta (T-04.2.01.06)', () => {
    it('updates postedBalance by delta and increments version when expectedVersion matches', async () => {
      mockPool.query.mockResolvedValue({
        rows: [
          makeWalletRow({ version: 2, posted_balance: '1100000', available_balance: '900000' }),
        ],
      });
    ...
    it('adds available-balance floor when requireAvailableAtLeast is set (T-04.2.04.01)', async () => {
      mockPool.query.mockResolvedValue({
        rows: [
          makeWalletRow({ version: 2, posted_balance: '900000', available_balance: '700000' }),
        ],
      });
  ...
  describe('reverseTransaction (T-04.2.04.01)', () => {
    ...
    it('rejects reversing a reservation', async () => {
      ...
      await expect(service.reverseTransaction(ORIGINAL_ID, 'oops', 'rev-idem-001')).rejects.toThrow(
        "Ledger type 'reservation' cannot be reversed"
      );
      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    });
    ...
    it('rejects reversing a reversal', async () => {
      ...
      await expect(service.reverseTransaction(ORIGINAL_ID, 'oops', 'rev-idem-001')).rejects.toThrow(
        "Ledger type 'reversal' cannot be reversed"
      );
    });
    ...
    it('rejects a Pending original', async () => {
      ...
      await expect(service.reverseTransaction(ORIGINAL_ID, 'oops', 'rev-idem-001')).rejects.toThrow(
        "Ledger row in state 'Pending' cannot be reversed"
      );
    });
    ...
    it('rejects a blank reason and a non-UUID original without opening a transaction', async () => {
      await expect(service.reverseTransaction(ORIGINAL_ID, '   ', 'rev-idem-001')).rejects.toThrow(
        'Reversal reason is required'
      );
      await expect(
        service.reverseTransaction('not-a-uuid', 'reason', 'rev-idem-001')
      ).rejects.toThrow('Original transaction id must be a UUID');
      await expect(service.reverseTransaction(ORIGINAL_ID, 'reason', '   ')).rejects.toThrow(
        'Idempotency key is required'
      );
      expect(mockPool.connect).not.toHaveBeenCalled();
    });
  ...
  describe('validateOnlineTopUpAmount (T-04.2.02.06)', () => {
    ...
    it('uses the 2,000,000,000 IRR default at version 0 when no limit is persisted', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('app_config'), [
