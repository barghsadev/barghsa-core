...
describe('payment gateway URL allow-list (T-04.2.02.01)', () => {
  ...
  it('rejects credentials, unsupported schemes, and unexpected hosts', () => {
    ...
    expect(() => assertSafePaymentGatewayUrl('javascript:alert(1)', allowed, 'redirect')).toThrow(
      /unsupported URL scheme/
    );
    expect(() => assertSafePaymentGatewayUrl('data:text/html,phish', allowed, 'redirect')).toThrow(
      /unsupported URL scheme/
    );
    expect(() => assertSafePaymentGatewayUrl('http://psp.test/pay', allowed, 'redirect')).toThrow(
      /https URL/
    );
    expect(() =>
...
describe('ZarinPal payment gateway (T-04.2.02.01)', () => {
  ...
  it('recovers the created authority after a timeout without posting a second payment request', async () => {
    ...
    const request = {
      ...START_REQUEST,
      callbackUrl: paymentCallbackUrlForOrder(
        START_REQUEST.callbackUrl,
        START_REQUEST.merchantOrderId
      ),
    };
    ...
    expect(recovered).toEqual({
      authority: 'A00000000000000000000000000000000001',
      redirectUrl: 'https://payment.zarinpal.com/pg/StartPay/A00000000000000000000000000000000001',
    });
    expect(created).toHaveLength(1);
    expect(fetchImpl).toHaveBeenCalledWith(
  ...
  it('derives the unverified inquiry URL from the payment request URL', () => {
    expect(
      resolveZarinpalUnverifiedUrl('https://sandbox.zarinpal.com/pg/v4/payment/request.json')
    ).toBe('https://sandbox.zarinpal.com/pg/v4/payment/unVerified.json');
  });
});
