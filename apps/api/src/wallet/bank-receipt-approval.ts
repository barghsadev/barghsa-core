...
function conflict(message: string): never {
  throw new HttpException({ statusCode: 409, error: ErrorCodes.CONFLICT_STATE.code, message }, 409);
}
export function walletReceiptApproval(metadata: unknown): WalletReceiptApproval | null {
  if (!metadata || typeof metadata !== 'object') return null;
  const raw = (metadata as Record<string, unknown>).dualApproval;
  if (raw === undefined) return null;
  if (!raw || typeof raw !== 'object') return conflict('Invalid saved receipt approval binding');
...
export async function gateWalletReceiptApproval(
  ...
  if (!saved) {
    ...
    await notifyApprovalRequested(client, {
      requestId: binding.requestId,
      amountIrR: input.amount.toString(),
      initiatorUserId: input.actorUserId,
    });
    return binding;
  }
  if (saved.fingerprint !== fingerprint)
    conflict('Receipt evidence or intended invoice changed after approval was requested');
  const request = (
    await client.query('SELECT * FROM approval_requests WHERE id=$1 FOR UPDATE', [saved.requestId])
...
export async function rejectWalletReceiptApproval(
  client: WalletQueryClient,
  input: {
    id: string;
    metadata: unknown;
  ...
  const row = (
    await client.query('SELECT * FROM approval_requests WHERE id=$1 FOR UPDATE', [
      binding.requestId,
    ])
  ).rows[0] as Record<string, unknown> | undefined;
  ...
  if (
    !row ||
    row.initiator_id !== binding.initiatorId ||
    details?.receiptId !== input.id ||
    details?.fingerprint !== binding.fingerprint
  )
    conflict('Receipt approval binding is invalid');
  if (row.status === 'rejected') {
    if (typeof row.review_reason !== 'string' || !row.review_reason.trim())
      conflict('Approval rejection has no reason');
