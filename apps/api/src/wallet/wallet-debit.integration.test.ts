...
describe('WalletService.debit — real PostgreSQL (T-04.2.01.04)', () => {
  ...
  it('posts a first-time debit with an uppercase walletId against the canonical UUID', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'debit-uuid-case-first'
      )
    ).toEqual([
      expect.objectContaining({
        id: tx.id,
  ...
  it('lets concurrent retries of the same idempotency key debit only once', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'debit-concurrent-same'
      )
    ).toHaveLength(1);
  });
  ...
  it('rejects debit when reserved funds leave availableBalance below the amount', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_B)).filter(
        (row) => row.idempotency_key === 'debit-reserved-blocks'
      )
    ).toHaveLength(0);
  });
  ...
  it('rejects debit that reuses a credit idempotency key without changing the balance', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'debit-reuses-credit-key'
      )
    ).toHaveLength(1);
  });
  ...
  it('rejects debit that reuses a reservation idempotency key without completing a debit', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'debit-reuses-reserve-key'
      )
    ).toEqual([
      expect.objectContaining({
        type: 'reservation',
  ...
  it('rejects a same-key debit with a different amount without changing the balance', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'debit-mismatch-amount'
      )
    ).toHaveLength(1);
  });
