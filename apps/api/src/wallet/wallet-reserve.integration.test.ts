...
describe('WalletService.reserve / release — real PostgreSQL (T-04.2.01.05)', () => {
  ...
  it('reserves: Reserved ledger row, reservedBalance rises, postedBalance is unchanged', async () => {
    ...
    const tx = await service.reserve(PROFILE_A, 250_000n, 'reserve-happy-1', {
      refId: 'inv-1',
  ...
  it('returns the original ledger row on idempotent retry without double-reserving', async () => {
    const first = await service.reserve(PROFILE_A, 10_000n, 'reserve-idem-1', {
      refId: 'inv-idem',
    });
    const afterFirst = await fetchWallet(PROFILE_A);

    const second = await service.reserve(PROFILE_A, 10_000n, 'reserve-idem-1', {
      refId: 'inv-idem',
    });
  ...
  it('posts a first-time reserve with an uppercase walletId against the canonical UUID', async () => {
    const before = await fetchWallet(PROFILE_A);
    const tx = await service.reserve(PROFILE_A.toUpperCase(), 4_500n, 'reserve-uuid-case-first', {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-uuid-case-first'
      )
    ).toEqual([
      expect.objectContaining({
        id: tx.id,
  ...
  it('lets concurrent retries of the same idempotency key reserve only once', async () => {
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-concurrent-same'
      )
    ).toHaveLength(1);
  });
  ...
  it('rejects reserve when availableBalance is insufficient and writes no ledger row', async () => {
    ...
    await expect(
      service.reserve(PROFILE_B, 100_000n, 'reserve-insufficient')
  ...
  it('rejects reserve when reserved funds leave availableBalance below the amount', async () => {
    ...
    await expect(
      service.reserve(PROFILE_B, 60_000n, 'reserve-reserved-blocks')
    ).rejects.toBeInstanceOf(BadRequestException);
    ...
    expect(
      (await fetchLedger(PROFILE_B)).filter(
        (row) => row.idempotency_key === 'reserve-reserved-blocks'
      )
    ).toHaveLength(0);
  });
  ...
  it('rejects reserve that reuses a credit idempotency key without changing the balance', async () => {
    const creditTx = await service.credit(
      PROFILE_A,
      1_000n,
      { type: 'topup' },
      'reserve-reuses-credit-key'
    );
    const afterCredit = await fetchWallet(PROFILE_A);

    await expect(
      service.reserve(PROFILE_A, 1_000n, 'reserve-reuses-credit-key')
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-reuses-credit-key'
      )
    ).toHaveLength(1);
  });
  ...
  it('rejects reserve that reuses a debit idempotency key without completing a hold', async () => {
    const debitTx = await service.debit(
      ...
      1_000n,
      { type: 'payment' },
      'reserve-reuses-debit-key'
    );
    const afterDebit = await fetchWallet(PROFILE_A);

    await expect(
      service.reserve(PROFILE_A, 1_000n, 'reserve-reuses-debit-key')
    ).rejects.toBeInstanceOf(ConflictException);
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-reuses-debit-key'
      )
    ).toHaveLength(1);
  });

  it('rejects a same-key reserve with a different amount without changing the balance', async () => {
    const first = await service.reserve(PROFILE_A, 2_000n, 'reserve-mismatch-amount', {
      refId: 'inv-amt',
    });
    const afterFirst = await fetchWallet(PROFILE_A);
    ...
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-mismatch-amount'
      )
    ).toHaveLength(1);
  });

  it('rejects a same-key reserve with a different refId without changing the balance', async () => {
    const first = await service.reserve(PROFILE_A, 1_500n, 'reserve-mismatch-ref', {
      refId: 'inv-ref-a',
    });
    const afterFirst = await fetchWallet(PROFILE_A);
  ...
  it('release drops reservedBalance, leaves postedBalance unchanged, and marks the row Released', async () => {
    ...
    expect(BigInt(afterRelease.reserved_balance)).toBe(
      BigInt(afterReserve.reserved_balance) - 8_000n
    );
  ...
  it('releases with an uppercase reservation id against the canonical ledger UUID', async () => {
    ...
    expect(BigInt(afterRelease.posted_balance)).toBe(BigInt(afterReserve.posted_balance));
    expect(BigInt(afterRelease.reserved_balance)).toBe(
      BigInt(afterReserve.reserved_balance) - 3_500n
    );
    expect(afterRelease.version).toBe(afterReserve.version + 1);
    expect(
      (await fetchLedger(PROFILE_A)).filter(
        (row) => row.idempotency_key === 'reserve-release-uuid-case'
      )
    ).toEqual([
      expect.objectContaining({
        id: hold.id,
  ...
  it('rejects replaying a released reservation key as a new hold', async () => {
    ...
    await expect(
      service.reserve(PROFILE_A, 2_500n, 'reserve-replay-released')
    ).rejects.toBeInstanceOf(ConflictException);
