...
describe('TosService', () => {
  ...
  describe('requiresReAcceptance', () => {
    it.each([true, false])('returns the database consent decision %s', async (required) => {
      mockPool.query.mockResolvedValueOnce({ rows: [{ required }] });
      expect(await service.requiresReAcceptance('user-0001')).toBe(required);
      expect(mockPool.query).toHaveBeenCalledWith(expect.any(String), ['user-0001']);
    });
    it('returns false when the user or active publication does not exist', async () => {
      mockPool.query.mockResolvedValueOnce({ rows: [] });
      expect(await service.requiresReAcceptance('missing')).toBe(false);
    });
    it('propagates database errors', async () => {
      mockPool.query.mockRejectedValueOnce(new Error('DB error'));
      await expect(service.requiresReAcceptance('user-0001')).rejects.toThrow('DB error');
    });
  });
});
