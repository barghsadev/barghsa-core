...
beforeEach(async () => {
  ...
  await fixture.pool.query(`INSERT INTO tos_versions(id,version_id,content_fa,content_en,status,is_active,published_at)
    VALUES ($1,'consent-v1','قوانین اول','First terms','published',true,NOW()),
           ...
           ($3,'consent-draft','پیش نویس','Draft terms','draft',false,NULL)`, [oldTerms, newTerms, draftTerms])
}, 40000)

afterEach(async () => { await fixture?.close() }, 15000)
...
it('rejects placeholder, unknown and draft terms before creating a challenge', async () => {
  for (const tosVersionId of ['current', randomUUID(), draftTerms]) {
    const response = await post('auth/register', { username: 'invalid-consent@example.test', password, tosVersionId })
    expect(response.status, await response.text()).toBe(400)
  }
  expect((await fixture.pool.query("SELECT count(*)::int AS count FROM otp_challenges WHERE destination='invalid-consent@example.test'")).rows[0].count).toBe(0)
})
...
it('keeps consent bound to the displayed publication and supports later re-acceptance by ID', async () => {
  ...
  expect(await current.json()).toMatchObject({ id: oldTerms, versionId: 'consent-v1', content: 'First terms' })
  ...
  const verified = await post('auth/register/verify', { challengeId: challenge.challengeId, otp: '123456' })
  ...
  expect((await fixture.pool.query('SELECT version_id FROM tos_acceptances WHERE user_id=$1', [user.userId])).rows).toEqual([{ version_id: oldTerms }])
  expect((await fixture.pool.query('SELECT last_accepted_tos_version FROM users WHERE user_id=$1', [user.userId])).rows[0].last_accepted_tos_version).toBe(oldTerms)
  ...
  expect((await fixture.pool.query('SELECT version_id FROM tos_acceptances WHERE user_id=$1 ORDER BY accepted_at', [user.userId])).rows).toEqual([{ version_id: oldTerms }, { version_id: newTerms }])
  const duplicate = await post('auth/register', { username: 'consent@example.test', password, tosVersionId: newTerms })
...
it('preserves published content when draft edits race publication', async () => {
  await fixture.pool.query("INSERT INTO users(user_id,username,password_hash,is_admin) VALUES ('terms-admin','terms-admin@example.test','test-only',true)")
  ...
  await fixture.pool.query(`INSERT INTO sessions(session_id,user_id,csrf_token,family_id,expires_at,idle_deadline,step_up_verified_at)
    VALUES ($1,'terms-admin',$2,$3,NOW()+INTERVAL '1 day',NOW()+INTERVAL '30 minutes',NOW())`, [sessionId, token, randomUUID()])
  const headers = { Cookie: `barghsa_session=${sessionId}`, 'X-CSRF-Token': token, 'Content-Type': 'application/json' }
  ...
  try {
    ...
    edit = fetch(`${fixture.base}/api/admin/tos/versions/${draftTerms}`, {
      method: 'PUT', headers, body: JSON.stringify({ contentEn: 'Racing edit must not overwrite published text' }),
    })
    await expect.poll(async () => (await fixture.pool.query(`SELECT count(*)::int AS count FROM pg_stat_activity
      WHERE datname=current_database() AND wait_event_type='Lock' AND query LIKE 'UPDATE tos_versions%'`)).rows[0].count).toBe(1)
    await client.query("UPDATE tos_versions SET status='published',published_at=NOW() WHERE id=$1", [draftTerms])
    await client.query('COMMIT')
    const response = await edit
    expect(response.status, await response.text()).toBe(409)
    expect((await fixture.pool.query('SELECT content_en FROM tos_versions WHERE id=$1', [draftTerms])).rows[0].content_en).toBe('Draft terms')
  } finally {
  ...
  await fixture.pool.query(`INSERT INTO tos_versions(id,version_id,content_fa,content_en,status)
    VALUES ($1,'publish-once','قوانین','Publish once','draft')`, [anotherDraft])
  const results = await Promise.all([1, 2].map(() => post(`admin/tos/versions/${anotherDraft}/publish`, { changeType: 'major' }, headers)))
