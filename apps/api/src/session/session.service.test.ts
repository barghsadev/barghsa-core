...
describe('SessionService', () => {
  ...
  describe('createSession', () => {
    ...
    it('revokes oldest session when limit is reached (50)', async () => {
      ...
      mockClient.query.mockImplementation(async (sql: string) => {
        ...
        if (sql.includes('UPDATE sessions') && sql.includes('LIMIT 1')) return { rows: [] };
        // INSERT
        return { rows: [] };
      });
      ...
      const revokeCall = calls.find(
        (s: any) =>
          s.includes('UPDATE sessions') && s.includes('revoked_at') && s.includes('LIMIT $3')
      );
  ...
  describe('redeemRefreshToken', () => {
    ...
    it('revokes entire family when a consumed token is reused (theft detection)', async () => {
      ...
      await expect(service.redeemRefreshToken('reused-token')).rejects.toThrow(
      ...
      const familyTokenUpdate = calls.filter(
        (s: string) => s.includes('UPDATE refresh_tokens') && s.includes('consumed_at')
      );
      const familySessionUpdate = calls.filter(
        (s: string) => s.includes('UPDATE sessions') && s.includes('revoked_at')
      );
    ...
    it('rejects a token whose session is expired', async () => {
      ...
      await expect(service.redeemRefreshToken(validToken)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('rejects an unknown token hash', async () => {
      ...
      await expect(service.redeemRefreshToken('nonexistent')).rejects.toThrow(
        UnauthorizedException
      );
    });
    ...
    it('rejects a refresh token for a disabled account (T-10.01.01)', async () => {
      ...
      const inserts = mockClient.query.mock.calls.filter((c: any[]) =>
  ...
  describe('revokeSession', () => {
    it('revokes a session and consumes its family tokens', async () => {
      mockClient.query.mockImplementation(async (sql: string) => {
        ...
        if (sql.includes('refresh_tokens') && sql.includes('consumed_at')) return { rows: [] };
        return { rows: [] };
      });
  ...
  describe('revokeAllUserSessions', () => {
    ...
    it('excludes a specified session when excludeSessionId is provided', async () => {
      ...
      const excludeCallParams = mockClient.query.mock.calls.find((c: any[]) =>
  ...
  describe('rotateSession', () => {
    it('rotates to a new session and returns fresh credentials', async () => {
      ...
      mockClient.query.mockImplementation(async (sql: string) => {
        ...
        if (sql.includes('FOR UPDATE') && sql.includes('sessions')) return { rows: [oldSession] };
        // Revoke old session
        if (sql.includes('UPDATE sessions') && !sql.includes('INSERT')) return { rows: [] };
        // Insert new session
