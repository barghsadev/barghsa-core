...
beforeAll(async () => {
  ...
  await pool.query('INSERT INTO users(user_id, username, password_hash) VALUES ($1,$2,$3)',
    ['http-user', 'http@example.test', await argon2.hash(password)])
  await pool.query("INSERT INTO device_trusts(id,user_id,device_fingerprint,expires_at) VALUES ($1,$2,$3,NOW()+INTERVAL '1 day')",
    [randomUUID(), 'http-user', createHash('sha256').update(fingerprint).digest('hex')])
}, 40000)

afterAll(async () => { await fixture?.close() }, 15000)
...
async function login() {
  ...
  return { cookie: cookies.map((cookie) => cookie.split(';')[0]).join('; '),
    token: data.csrfToken as string, sessionId: data.sessionId as string }
}
...
it('authenticates parsed cookies and rejects unsafe requests before extending or revoking a session', async () => {
  ...
  const before = (await pool!.query('SELECT idle_deadline FROM sessions WHERE session_id=$1', [auth.sessionId])).rows[0]
  for (const token of [undefined, 'wrong-token']) {
    ...
    expect((await fetch(`${base}/api/auth/sessions/${randomUUID()}`, { method: 'DELETE', headers })).status).toBe(403)
  }
  const after = (await pool!.query('SELECT idle_deadline, revoked_at FROM sessions WHERE session_id=$1', [auth.sessionId])).rows[0]
  expect(after).toEqual({ ...before, revoked_at: null })
  expect((await fetch(`${base}/api/auth/step-up`, {
    method: 'POST', headers: { Cookie: auth.cookie, 'Content-Type': 'application/json' },
    body: JSON.stringify({ password }),
  })).status).toBe(403)
  expect((await fetch(`${base}/api/auth/step-up`, {
    method: 'POST', headers: { Cookie: auth.cookie, 'Content-Type': 'application/json', 'X-CSRF-Token': auth.token },
    body: JSON.stringify({ password }),
  })).status).toBe(200)
  const otherSession = await login()
  expect(otherSession.token).not.toBe(auth.token)
  expect((await fetch(`${base}/api/auth/step-up`, {
    method: 'POST', headers: { Cookie: otherSession.cookie, 'Content-Type': 'application/json', 'X-CSRF-Token': auth.token },
    body: JSON.stringify({ password }),
  })).status).toBe(403)
  expect((await fetch(`${base}/api/auth/sessions/${randomUUID()}`, {
    method: 'DELETE', headers: { Cookie: auth.cookie, 'X-CSRF-Token': auth.token },
  })).status).toBe(404)
  expect((await fetch(`${base}/api/auth/logout`, {
    method: 'POST', headers: { Cookie: auth.cookie, 'X-CSRF-Token': auth.token },
  })).status).toBe(200)
...
it('requires CSRF for refresh, recovers idle expiry, and retains refresh reuse revocation', async () => {
  ...
  expect((await pool!.query('SELECT consumed_at FROM refresh_tokens WHERE session_id=$1', [auth.sessionId])).rows)
...
it('invalidates old sessions and refresh tokens atomically on a forced password change', async () => {
  ...
  await pool.query("UPDATE users SET must_change_password=true, password_change_token=$1, password_change_token_expires_at=NOW()+INTERVAL '5 minutes' WHERE user_id='http-user'", [passwordChangeToken])
  await pool.query('INSERT INTO password_history(id,user_id,password_hash,version) VALUES ($1,$2,$3,1)',
  ...
  expect((await fetch(`${base}/api/auth/sessions`, { headers: { Cookie: auth.cookie } })).status).toBe(401)
  expect((await fetch(`${base}/api/auth/refresh`, {
    method: 'POST', headers: { Cookie: auth.cookie, 'X-CSRF-Token': auth.token },
  })).status).toBe(403)
  expect((await pool.query("SELECT count(*)::int AS active FROM sessions WHERE user_id='http-user' AND revoked_at IS NULL")).rows[0].active).toBe(0)
  expect((await pool.query("SELECT count(*)::int AS active FROM refresh_tokens WHERE user_id='http-user' AND consumed_at IS NULL")).rows[0].active).toBe(0)
  expect((await pool.query("SELECT version FROM password_history WHERE user_id='http-user' ORDER BY version")).rows).toEqual([{ version: 1 }, { version: 2 }])
}, 15000)
