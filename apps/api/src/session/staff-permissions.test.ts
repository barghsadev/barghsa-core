...
it('unions valid roles and rejects invalid role data', () => {
  expect(resolveStaffPermissions(['["tickets:read","tickets:write"]', '["tickets:read"]', 'invalid', '{"*":true}', '["*",1]']))
...
it('accepts exact capabilities or explicit wildcard authority without granting prefix matches', () => {
  const request = { session: { isAdmin: false, permissions: ['tickets:read'] } } as AuthenticatedRequest
