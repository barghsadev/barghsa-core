...
function defaultHandler(
  rows: Array<{ ledger_id: string; item_id: string; responsible_user_id: string | null }> = [
    { ledger_id: 'l1', item_id: 'ticket-1', responsible_user_id: 'staff-1' },
  ]
) {
...
describe('scanServiceEscalations (T-09.08.03)', () => {
  ...
  it('escalates an un-responded breached ticket to level 2 (team lead) with the level channels', async () => {
    ...
    expect(l2Call!.sql).toContain(
      "updated_at <= $4::timestamptz - (l.target_hours * INTERVAL '1 hour')"
    );
    expect(l2Call!.sql).toContain('l.alerted_at <= $3');
    expect(
      db.calls.some(
        (c) =>
          c.sql.includes('FROM profiles') &&
          Array.isArray(c.params[0]) &&
          (c.params[0] as string[]).includes('teamlead-1')
      )
    ).toBe(true);
  ...
  it('uses escalated_at as the level-3 base and escalates to admin only after its delay', async () => {
    ...
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets')) {
        ...
        return params[1] === 2
          ? { rows: [{ ledger_id: 'l2', item_id: 'ticket-2', responsible_user_id: 'staff-2' }] }
          : { rows: [] };
      }
      if (sql.includes('FROM users WHERE is_admin')) return { rows: [{ user_id: 'admin-1' }] };
      if (sql.includes('FROM profiles'))
        return { rows: [{ id: 'profile-admin', user_id: 'admin-1' }] };
  ...
  it('falls back to platform admins when the responsible user is in no team', async () => {
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets'))
        return params[1] === 1
          ? { rows: [{ ledger_id: 'l1', item_id: 't1', responsible_user_id: 'solo-1' }] }
          : { rows: [] };
  ...
  it('falls back to admins for an unassigned item (no responsible user)', async () => {
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets'))
        return params[1] === 1
          ? { rows: [{ ledger_id: 'l1', item_id: 't1', responsible_user_id: null }] }
          : { rows: [] };
      if (sql.includes('FROM users WHERE is_admin')) return { rows: [{ user_id: 'admin-9' }] };
      if (sql.includes('FROM profiles')) return { rows: [{ id: 'pa', user_id: 'admin-9' }] };
      if (sql.includes('RETURNING id')) return { rows: [{ id: 'l1' }], rowCount: 1 };
  ...
  it('does not claim and re-evaluates when no deliverable recipient exists', async () => {
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets'))
        return params[1] === 1
          ? { rows: [{ ledger_id: 'l1', item_id: 't1', responsible_user_id: 'ghost-1' }] }
          : { rows: [] };
      if (sql.includes('FROM staff_team_members')) return { rows: [] };
  ...
  it('skips an escalation whose claim was already won by a concurrent scan', async () => {
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets'))
        return params[1] === 1
          ? { rows: [{ ledger_id: 'l1', item_id: 't1', responsible_user_id: 'staff-1' }] }
          : { rows: [] };
    ...
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN tickets')) {
        return params[1] === 1
          ? {
              rows: [
                { ledger_id: 'l1', item_id: 't1', responsible_user_id: 'staff-1' },
                { ledger_id: 'l2', item_id: 't2', responsible_user_id: 'staff-2' },
              ],
            }
          : { rows: [] };
      }
  ...
  it('escalates a verification case creator to their team', async () => {
    const db = makeFakeDb((sql, params) => {
      ...
      if (sql.includes('JOIN verification_cases'))
        return params[1] === 1
          ? { rows: [{ ledger_id: 'lc', item_id: 'case-1', responsible_user_id: 'staff-c' }] }
          : { rows: [] };
    ...
    const vcCall = db.calls.find(
      (c) => c.sql.includes('JOIN verification_cases') && c.params[1] === 1
    );
