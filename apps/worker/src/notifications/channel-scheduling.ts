...
export async function refreshOutboxState(pool: QueryPool, id: string, now = new Date()): Promise<'delivered' | 'failed' | 'pending'> {
  const jobs: ChannelJob[] = (await pool.query('SELECT channel,status,attempts,max_attempts,run_after,last_error FROM notification_job WHERE outbox_id=$1', [id])).rows
...
export async function reconcileChannelWindows(pool: QueryPool, config?: DeliveryWindowConfig, now = new Date()): Promise<number> {
  ...
  const pending = await pool.query(`SELECT o.id,o.event_key,o.channels,o.status,o.scheduled_for,u.timezone,
    ...
    ORDER BY o.created_at,o.id LIMIT 500`, [now])
  let changed = 0
  for (const candidate of pending.rows) {
    ...
    try {
      ...
      if (!jobs.length) { if (client) await client.query('COMMIT'); continue }
      const recipientConfig = normalizeWindowConfig({ ...cfg, timezone: row.timezone ?? cfg.timezone })
      // Direct test overrides can intentionally use a full-day window.
      if (config) { recipientConfig.startHour = config.startHour; recipientConfig.endHour = config.endHour }
      let adjusted = false
      for (const job of jobs) {
        if (terminalJob(job) || (job.run_after && new Date(job.run_after) > now)) continue
        const previousBoundary = row.status === 'scheduled' && row.scheduled_for && new Date(row.scheduled_for) > now
          ? new Date(row.scheduled_for) : null
        const savedWindow = job.delivery_window ? normalizeWindowConfig(job.delivery_window) : recipientConfig
        const decision = decideDeliverySchedule(row.event_key, [job.channel], now, savedWindow)
        const next = job.channel !== 'in_app' && previousBoundary ? previousBoundary : decision.kind === 'schedule' ? decision.scheduledFor : null
        if (next) {
          await q.query(`UPDATE notification_job SET run_after=$3,delivery_window=COALESCE(delivery_window,$5::jsonb),updated_at=NOW()
            WHERE outbox_id=$1 AND channel=$2 AND status IN ('queued','retrying')
              AND (run_after IS NULL OR run_after<=$4)`, [row.id, job.channel, next, now, JSON.stringify(savedWindow)])
          adjusted = true
        }
      }
