...
describe('deriveChannelIdempotencyKey', () => {
  ...
  it('varies by channel and profile for pre-existing events, but ignores the outbox key', () => {
    const emailP1 = deriveChannelIdempotencyKey(
      'profile_verified',
      'email',
      'profile-1',
      'outbox-a'
    );
    const inAppP1 = deriveChannelIdempotencyKey(
      'profile_verified',
      'in_app',
      'profile-1',
      'outbox-a'
    );
    const emailP2 = deriveChannelIdempotencyKey(
      'profile_verified',
      'email',
      'profile-2',
      'outbox-a'
  ...
  it('matches the historical digest so a queued non-reminder retry cannot redeliver', () => {
    const legacy = createHash('sha256').update('profile_verified:email:profile-1').digest('hex');
    expect(
