  ...
  async send(payload: NotificationSendPayload): Promise<NotificationSendResult> {
    ...
    if (!snapshot) {
      const providers = await this.pool.query(
        "SELECT id FROM email_provider_configs WHERE status='active' AND last_test_status='passed'"
      );
      if (providers.rows.length !== 1 || typeof providers.rows[0]!.id !== 'string')
        throw new Error('Email provider unavailable');
      const templates = await this.pool.query(
        `SELECT id,version,subject,body_template,variables FROM notification_templates
        WHERE event_key=$1 AND channel='email' AND locale=$2 AND status='active' AND is_active=true`,
      ...
      const subject = renderTemplate(template.subject, names, {
    ...
    if (
      ...
      saved.destination !== recipient.email ||
    ...
    const providerRef = await createEmailSender(
      ...
      html: saved.html,
      idempotencyKey: payload.idempotencyKey,
      expectedProviderId: saved.providerId,
