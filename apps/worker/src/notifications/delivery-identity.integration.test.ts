...
beforeAll(async () => {
  ...
  await pool.query(
    "INSERT INTO users(user_id,username,password_hash) VALUES ('delivery-owner','delivery@example.test','test-only')"
  );
  profileId = (
    await pool.query("INSERT INTO profiles(user_id) VALUES ('delivery-owner') RETURNING id")
  ).rows[0].id;
  for (const [id, attempts] of [
    ...
    await pool.query(
      `INSERT INTO notification_outbox(id,profile_id,user_id,event_key,payload,channels,idempotency_key,attempts,status)
      VALUES ($1::uuid,$2,'delivery-owner','wallet.topup_completed','{}',ARRAY['in_app','email'],$1::text,$3,'queued')`,
      [id, profileId, attempts]
    );
    await pool.query(
      "INSERT INTO notification_job(outbox_id,channel) VALUES ($1,'in_app'),($1,'email')",
      [id]
    );
  }
  await pool.query(
    `INSERT INTO in_app_notifications(id,profile_id,type,title_i18n_key,body_i18n_key,is_read,read_at)
    VALUES ($1::uuid,$2,'wallet.topup_completed','title','body',true,NOW())`,
    [inbox, profileId]
  );
  await pool.query(
  ...
  await pool.query(
    readFileSync(resolve(folder, '0092_notification_delivery_identity.sql'), 'utf8')
  );
  for (const entry of journal.entries.filter(
    (entry) => entry.tag > '0092_notification_delivery_identity'
  )) {
    if (entry.tag === '0095_notification_message_snapshot') {
      await pool.query(
        ...
        VALUES ($1::uuid,$2,'wallet.topup_completed','{}',ARRAY['email'],$1::text,'queued')`,
        [legacyEmail, profileId]
      );
      await pool.query(
        "INSERT INTO notification_job(outbox_id,channel,status,attempts,last_error) VALUES ($1,'email','retrying',1,'historical-timeout')",
        [legacyEmail]
      );
    }
    if (entry.tag === '0096_unified_notification_inbox') {
      await pool.query(
        `INSERT INTO notifications(id,user_id,profile_id,type,title,body,link,read,read_at,created_at)
        VALUES ($1,'delivery-owner',$2,'profile_verified','Legacy verified','Original body','/app/settings/profile',true,'2026-01-02','2026-01-01')`,
        [legacyNotice, profileId]
      );
    }
    await pool.query(readFileSync(resolve(folder, `${entry.tag}.sql`), 'utf8'));
  }
...
it('preserves proven legacy delivery and read state while holding ambiguous retries', async () => {
  ...
  expect(
    (await pool.query('SELECT is_read,delivery_key FROM in_app_notifications WHERE id=$1', [inbox]))
      .rows[0]
  ).toEqual({ is_read: true, delivery_key: `outbox:${proven}` });
  const states = (
    await pool.query(
      'SELECT id,status,last_error,idempotency_version FROM notification_outbox WHERE id=ANY($1::uuid[])',
      [[proven, ambiguous, untouched]]
    )
  ).rows;
  expect(states.find((r) => r.id === proven)).toMatchObject({
    status: 'queued',
    idempotency_version: 1,
  });
  expect(states.find((r) => r.id === ambiguous)).toMatchObject({
    status: 'failed',
    last_error: 'legacy_delivery_requires_reconciliation',
    idempotency_version: 1,
  });
  expect(states.find((r) => r.id === untouched)).toMatchObject({
    status: 'queued',
...
it('retries one top-up without duplicate inbox rows and gives a second top-up a distinct provider key', async () => {
  const first = row(randomUUID()),
    second = row(randomUUID());
  const transport = new InAppNotificationTransport(pool);
  ...
  expect(
    (
      await pool.query(
        'SELECT count(*)::int AS count FROM in_app_notifications WHERE delivery_key=ANY($1::text[])',
        [[`outbox:${first.id}`, `outbox:${second.id}`]]
      )
    ).rows[0].count
  ).toBe(2);
});
...
it('concurrent retries insert once and cannot reset read state or overwrite the payload', async () => {
  ...
  const outcomes = await Promise.all(
    Array.from({ length: 8 }, () =>
      dispatchOutbox({ ...row(id), channels: ['in_app'] }, { in_app: transport })
    )
  );
  ...
  await dispatchOutbox(
    { ...row(id), channels: ['in_app'], payload: { amount: 'changed' } },
    { in_app: transport }
  );
  expect(
    (await pool.query('SELECT is_read,params FROM in_app_notifications WHERE id=$1', [ids[0]]))
      .rows[0]
...
it('enqueues two occurrences while a repeated business key inserts no second job set', async () => {
  ...
  try {
    ...
    const second = await enqueueOutbox(client, {
      ...input,
      channels: [...input.channels],
      idempotencyKey: 'topup:second',
    });
    expect(first.inserted).toBe(true);
    expect(duplicate.inserted).toBe(false);
    expect(second.inserted).toBe(true);
    const saved = await client.query(
      'SELECT id,idempotency_version FROM notification_outbox WHERE id=ANY($1::uuid[])',
    ...
    expect(
      (
        await client.query(
          'SELECT count(*)::int AS count FROM notification_job WHERE outbox_id=ANY($1::uuid[])',
          [[first.outboxId, second.outboxId]]
        )
      ).rows[0].count
    ).toBe(4);
...
it('the real runner reuses delivered channel outcomes after an email failure', async () => {
  ...
  const transports = {
    ...
    email: {
      ...
      async send() {
        emailCalls++;
        if (emailCalls === 1) throw new Error('email timeout');
        return { status: 'delivered' as const, providerRef: 'email-retry-ref' };
      },
    },
  };
  const options = {
  ...
  expect(
    (
      await pool.query(
        "SELECT status,provider_ref FROM notification_job WHERE outbox_id=$1 AND channel='in_app'",
        [id]
      )
    ).rows[0].status
  ).toBe('done');
  ...
  expect(
    (await pool.query('SELECT status FROM notification_outbox WHERE id=$1', [id])).rows[0].status
  ).toBe('delivered');
  expect(
    (
      await pool.query(
        "SELECT count(*)::int AS count FROM notification_delivery_log WHERE notification_id=$1 AND channel='in_app'",
        [id]
      )
    ).rows[0].count
  ).toBe(1);
...
it('delivers mixed-channel inboxes immediately in each recipient timezone and preserves scheduled window snapshots', async () => {
  ...
  await pool.query(
    "INSERT INTO users(user_id,username,password_hash,timezone) VALUES ('ny-owner','ny@example.test','test-only','America/New_York')"
  );
  const nyProfile = (
    await pool.query("INSERT INTO profiles(user_id) VALUES ('ny-owner') RETURNING id")
  ).rows[0].id;
  const client = await pool.connect();
  ...
  const options = {
    ...
    transports: {
      ...
      email: {
        channel: 'email' as const,
        async send(payload: { profileId: string | null }) {
          emailed.push(payload.profileId!);
          return { status: 'delivered' as const, providerRef: `email:${payload.profileId}` };
        },
      },
    },
    availability: () => ({
  ...
  try {
    ...
    expect(
      (
        await pool.query(
          "SELECT count(*)::int AS count FROM notification_job WHERE outbox_id=ANY($1::uuid[]) AND channel='in_app' AND status='done'",
          [[tehranId, nyId]]
        )
      ).rows[0].count
    ).toBe(2);
    const scheduled = (
      await pool.query(
        "SELECT run_after,delivery_window FROM notification_job WHERE outbox_id=$1 AND channel='email'",
        [tehranId]
      )
    ).rows[0];
    ...
    expect(
      (await pool.query('SELECT status FROM notification_outbox WHERE id=$1', [tehranId])).rows[0]
        .status
    ).toBe('delivered');
  } finally {
    vi.useRealTimers();
...
it('exhausts one channel without exhausting a different channel or repeating the terminal job', async () => {
  ...
  try {
    ...
    id = (
      await enqueueOutbox(client, {
        ...
        eventKey: 'wallet.topup_completed',
        channels: ['in_app', 'email'],
        idempotencyKey: 'runner:independent-budget',
      })
    ).outboxId;
    await client.query(
      "UPDATE notification_job SET attempts=4 WHERE outbox_id=$1 AND channel='in_app'",
      [id]
    );
    await client.query(
      "UPDATE notification_job SET max_attempts=2 WHERE outbox_id=$1 AND channel='email'",
      [id]
    );
    await client.query('COMMIT');
  } finally {
  ...
  const options = {
    ...
    transports: {
      in_app: {
        ...
        async send() {
          inAppCalls++;
          return { status: 'failed' as const, providerRef: '' };
        },
      },
      email: {
        channel: 'email' as const,
        async send() {
          emailCalls++;
          return emailCalls === 1
            ? { status: 'failed' as const, providerRef: '' }
            : { status: 'delivered' as const, providerRef: 'email-budget-ref' };
        },
      },
    },
    availability: () => ({
  ...
  expect(
    (
      await pool.query(
        'SELECT channel,status,attempts FROM notification_job WHERE outbox_id=$1 ORDER BY channel',
        [id]
      )
    ).rows
  ).toEqual([
    { channel: 'email', status: 'retrying', attempts: 1 },
    { channel: 'in_app', status: 'dead_letter', attempts: 5 },
  ]);
  await pool.query(
    "UPDATE notification_job SET run_after=NOW()-INTERVAL '1 second' WHERE outbox_id=$1 AND channel='email'",
    [id]
  );
  await pool.query(
    "UPDATE notification_outbox SET locked_until=NULL,scheduled_for=NOW()-INTERVAL '1 second' WHERE id=$1",
  ...
  expect(
    (
      await pool.query('SELECT channel,attempts FROM notification_dead_letter WHERE outbox_id=$1', [
        id,
      ])
    ).rows
  ).toEqual([{ channel: 'in_app', attempts: 5 }]);
});
  ...
  try {
    ...
    id = (
      await enqueueOutbox(client, {
        ...
        eventKey: 'wallet.topup_completed',
        channels: ['in_app', 'email'],
        idempotencyKey: 'runner:slow-renewal',
      })
    ).outboxId;
    await client.query('COMMIT');
  } finally {
    ...
    transports: {
      ...
      email: {
        ...
        async send() {
          started = true;
          await gate;
          return { status: 'delivered' as const, providerRef: 'slow-ref' };
        },
      },
    },
    availability: () => ({
...
it('a replaced claim cannot send the next channel or overwrite its successor', async () => {
  ...
  try {
    ...
    id = (
      await enqueueOutbox(client, {
        ...
        idempotencyKey: 'runner:takeover',
      })
    ).outboxId;
    await client.query('COMMIT');
  } finally {
    ...
    transports: {
      ...
      email: {
        ...
        async send(payload: { idempotencyKey: string }) {
          ...
          started = true;
          await gate;
          return { status: 'delivered' as const, providerRef: 'takeover-ref' };
        },
      },
      sms: {
        ...
        async send() {
          smsCalls++;
          return { status: 'delivered' as const, providerRef: 'sms-ref' };
        },
      },
    },
    availability: () => ({
  ...
  try {
    await expect.poll(() => started).toBe(true);
    await pool.query(
      "UPDATE notification_outbox SET lease_token='successor-claim',locked_until=NOW()+INTERVAL '1 day',last_error='successor-owned' WHERE id=$1",
      [id]
    );
  } finally {
    release();
  ...
  expect(
    (
      await pool.query(
        'SELECT lease_token,last_error,status FROM notification_outbox WHERE id=$1',
        [id]
      )
    ).rows[0]
  ...
  expect(
    (
      await pool.query(
        'SELECT count(*)::int AS count FROM notification_delivery_log WHERE notification_id=$1',
        [id]
      )
    ).rows[0].count
  ).toBe(0);
...
it('uses the explicit recipient, applies address suppression, and refuses disabled or unactivated recipients', async () => {
  ...
  await pool.query(
    "INSERT INTO email_suppressions(address,reason,profile_id,source_event_id) VALUES ('recipient@example.test','complaint',$1,NULL)",
    [profileId]
  );
  const availability = await loadChannelAvailabilityContext(pool, id);
  expect(
    resolveChannelAvailability('wallet.topup_completed', ['in_app', 'email'], availability)
  ...
  await pool.query(
    "UPDATE users SET disabled_at=NULL,activation_token='pending' WHERE user_id='delivery-recipient'"
  );
  expect(await loadNotificationRecipient(pool, id)).toBeNull();
...
it('delivers a queued email with the recipient locale, template and durable receipt through a controlled HTTP provider', async () => {
  ...
  await pool.query(
    "UPDATE users SET activation_token=NULL,disabled_at=NULL WHERE user_id='delivery-recipient'"
  );
  ...
  const email = new EmailNotificationTransport(pool, async (_url, options) =>
  ...
  try {
    ...
    id = (
      await enqueueOutbox(client, {
        ...
        eventKey: 'wallet.topup_completed',
        channels: ['in_app', 'email'],
        payload: { name: 'A&B <customer>', amount: '5000' },
        idempotencyKey: 'real-email:test',
      })
    ).outboxId;
    await client.query('COMMIT');
  } finally {
  ...
  try {
    ...
    await pool.query(
      'UPDATE notification_outbox SET payload=\'{"name":"Changed person","amount":"9000"}\' WHERE id=$1',
      [id]
    );
    await pool.query(
      "UPDATE notification_job SET run_after=NOW()-INTERVAL '1 second' WHERE outbox_id=$1 AND channel='email'",
    ...
    expect(
      (
        await pool.query(
          "SELECT status,provider_ref FROM notification_job WHERE outbox_id=$1 AND channel='email'",
          [id]
        )
      ).rows[0]
...
it('refuses to retarget a snapshotted email after the recipient changes', async () => {
  ...
  try {
    ...
    await pool.query("UPDATE users SET email=NULL WHERE user_id='delivery-recipient'");
  }
});
...
it('holds an attempted legacy email without deleting its job or retry evidence', async () => {
  expect(
    (
      await pool.query('SELECT status,last_error FROM notification_outbox WHERE id=$1', [
        legacyEmail,
      ])
    ).rows[0]
  ...
  expect(
    (
      await pool.query(
        'SELECT status,attempts,last_error,delivery_payload FROM notification_job WHERE outbox_id=$1',
        [legacyEmail]
      )
    ).rows[0]
  ).toEqual({
...
it('saves active bilingual inbox templates and keeps original content/read state on retry', async () => {
  ...
  expect(
    (
      await pool.query(
        'SELECT localized_content,recipient_user_id FROM in_app_notifications WHERE id=$1',
        [id]
      )
    ).rows[0]
  ).toEqual({
    recipient_user_id: 'delivery-owner',
    localized_content: {
      fa: { title: 'شارژ کیف پول', body: 'مبلغ 5000 ریال' },
      en: { title: 'Wallet credited', body: 'Amount 5000 IRR' },
    },
  });
  await pool.query(
    "UPDATE in_app_notifications SET is_read=true,read_at='2026-01-01' WHERE id=$1",
    [id]
  );
  await pool.query(
    "UPDATE notification_templates SET body_template='{{missing}}' WHERE event_key='wallet.topup_completed' AND channel='in_app'"
  );
  expect(
    (
      await dispatchOutbox(
        { ...event, channels: ['in_app'], payload: { amount: '9999' } },
        { in_app: transport }
      )
    )[0]!.result.providerRef
  ).toBe(id);
  expect(
...
it('delivers and escalates profileless staff alerts privately with retry and rollback safety', async () => {
  ...
  const ticket = (
    await pool.query(`INSERT INTO tickets(user_id,assigned_to,subject,body,status,updated_at)
    VALUES ('delivery-owner','alert-staff','Overdue','Private conversation','in_progress',NOW()-INTERVAL '4 hours') RETURNING id`)
  ).rows[0].id;
  ...
  expect(
    (await pool.query('SELECT id FROM service_breach_alerts WHERE item_id=$1', [ticket])).rows
  ).toEqual([]);
  expect(
    (await pool.query("SELECT id FROM notification_outbox WHERE user_id='alert-staff'")).rows
  ).toEqual([]);
  expect((await scanServiceBreaches({ pool, logger })).errors).toEqual([]);
  await scanServiceBreaches({ pool, logger });
  const alerts = (await pool.query("SELECT * FROM notification_outbox WHERE user_id='alert-staff'"))
    .rows;
  ...
  const outcomes = await Promise.all(
    Array.from({ length: 4 }, () =>
      dispatchOutbox({ ...notice, channels: [...notice.channels] }, { in_app: transport })
    )
  );
  ...
  expect(
    (
      await pool.query(
        'SELECT profile_id,recipient_user_id,localized_content FROM in_app_notifications WHERE delivery_key=$1',
        [`outbox:${alerts[0].id}`]
      )
    ).rows[0]
  ).toMatchObject({
    profile_id: null,
    recipient_user_id: 'alert-staff',
    localized_content: { fa: expect.any(Object), en: expect.any(Object) },
  });
  await pool.query(
    "UPDATE service_breach_alerts SET alerted_at=NOW()-INTERVAL '2 hours' WHERE item_id=$1",
    [ticket]
  );
  const escalationFailure = await scanServiceEscalations({
    pool,
    logger,
    enqueue: async (client, input) => {
      await enqueueOutbox(client, input);
      throw new Error('injected escalation queue failure');
    },
  });
  expect(escalationFailure.errors).toHaveLength(1);
  expect(
    (
      await pool.query('SELECT escalation_level FROM service_breach_alerts WHERE item_id=$1', [
        ticket,
      ])
    ).rows[0].escalation_level
  ).toBe(1);
  expect((await scanServiceEscalations({ pool, logger })).errors).toEqual([]);
  await scanServiceEscalations({ pool, logger });
  const escalations = (
    await pool.query(
      "SELECT * FROM notification_outbox WHERE event_key='admin.service_escalated' AND payload->>'item_id'=$1",
      [ticket]
    )
  ).rows;
  ...
  expect(await loadNotificationRecipient(pool, escalations[0].id)).toMatchObject({
    userId: 'alert-admin',
    profileId: null,
    email: 'admin-alert@example.test',
  });
  ...
  try {
    scheduledId = (
      await enqueueOutbox(queueClient, {
        profileId: null,
        userId: 'alert-admin',
        eventKey: 'staff.daytime_notice',
        channels: ['in_app', 'email'],
  ...
  await reconcileChannelWindows(
    pool,
    { startHour: 23, endHour: 24, timezone: 'UTC' },
    new Date('2026-09-06T12:00:00Z')
  );
  const jobs = (
    await pool.query('SELECT channel,run_after FROM notification_job WHERE outbox_id=$1', [
      scheduledId,
    ])
  ).rows;
  ...
  await expect(
    pool.query(`INSERT INTO notification_outbox(profile_id,event_key,channels,idempotency_key)
    VALUES (NULL,'invalid',ARRAY['in_app'],'missing-recipient')`)
...
it('escalates to the configured lead, excludes teammates, and falls back when the lead is disabled', async () => {
  ...
  const team = (
    await pool.query(
      "INSERT INTO staff_teams(name,lead_user_id) VALUES ('Lead routing','lead-target') RETURNING id"
    )
  ).rows[0].id;
  await pool.query(
    "INSERT INTO staff_team_members(team_id,user_id) VALUES ($1,'lead-owner'),($1,'lead-target'),($1,'lead-other')",
    [team]
  );
  const logger = { warn: vi.fn(), info: vi.fn() };
  async function createAndEscalate() {
    const ticket = (
      await pool.query(`INSERT INTO tickets(user_id,assigned_to,subject,body,status,updated_at)
      VALUES ('delivery-owner','lead-owner','Lead test','Private','in_progress',NOW()-INTERVAL '4 hours') RETURNING id`)
    ).rows[0].id;
    expect((await scanServiceBreaches({ pool, logger })).errors).toEqual([]);
    await pool.query(
      "UPDATE service_breach_alerts SET alerted_at=NOW()-INTERVAL '2 hours' WHERE item_id=$1",
    ...
    return (
      await pool.query(
        "SELECT user_id FROM notification_outbox WHERE event_key='admin.service_escalated' AND payload->>'item_id'=$1",
        [ticket]
      )
  ...
  await expect(
    pool.query("INSERT INTO staff_team_members(team_id,user_id) VALUES ($1,'lead-owner')", [team])
