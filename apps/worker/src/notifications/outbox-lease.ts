...
export class OutboxLeaseLost extends Error {
  constructor() { super('notification outbox claim is no longer owned') }
}
...
export async function assertOutboxClaim(pool: QueryPool, row: OutboxRow): Promise<void> {
  ...
  const owned = await pool.query(`SELECT id FROM notification_outbox
    WHERE id=$1 AND lease_token=$2 AND status='sending' AND locked_until>clock_timestamp()
    FOR UPDATE`, [row.id, row.leaseToken])
...
export async function startOutboxLease(pool: QueryPool, row: OutboxRow, durationMs: number): Promise<OutboxLease> {
  if (!row.leaseToken) throw new OutboxLeaseLost()
  ...
  async function renew() {
    try {
      const result = await pool.query(`UPDATE notification_outbox SET status = 'sending',
        ...
        WHERE id=$1 AND lease_token=$2 AND locked_until>clock_timestamp()
          AND status IN ('queued','scheduled','sending') RETURNING id`, [row.id, row.leaseToken, durationMs])
  ...
  const timer = setInterval(() => {
    ...
    pending = renew().catch(() => {}).finally(() => { pending = null })
