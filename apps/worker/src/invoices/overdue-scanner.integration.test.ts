...
import { FIND_OVERDUE_CANDIDATES_SQL, scanOverdueInvoices } from './overdue-scanner.js';
...
describe('overdue scanner — real PostgreSQL (T-04.1.03.04)', () => {
  ...
  beforeAll(async () => {
    ...
    await ctx.pool.query(`INSERT INTO profiles (id) VALUES ($1) ON CONFLICT (id) DO NOTHING`, [
      PROFILE_ID,
    ]);
  ...
  it('rejects text[] comparison against invoice_state (the production failure)', async () => {
    await expect(
      ctx.pool.query(BROKEN_TEXT_ARRAY_SQL, [[...OVERDUE_ELIGIBLE_STATES], NOW, 200])
