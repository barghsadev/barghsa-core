...
describe('cancel invoice reminders on stop state — real PostgreSQL (T-04.1.04.06)', () => {
  ...
  async function insertInvoice(opts: { state: string; paidAmount?: number }): Promise<string> {
    const id = randomUUID();
    const paid =
      opts.paidAmount ?? (opts.state === 'Paid' || opts.state === 'Refunded' ? 1_000_000 : 0);
    await ctx.pool.query(
  ...
  async function statuses(
    invoiceId: string
  ): Promise<Array<{ status: string; sent_at: Date | null }>> {
  ...
  it('does not cancel when the invoice becomes Overdue or PartiallyRefunded', async () => {
    ...
    await ctx.pool.query(`UPDATE invoices SET state = 'PartiallyRefunded' WHERE id = $1`, [
      partial,
    ]);
