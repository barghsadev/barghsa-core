...
import {
  decryptProviderSecret, SmsirConfigSchema, sendSmsirVerification,
...
${message.activationUrl}` : `کد تأیید برق‌آسا: ${message.code}\nBarghsa verification code: ${message.code}\nاین کد را با کسی به اشتراک نگذارید. Do not share this code.`
    if (email) return createEmailSender(pool, request)({ destination: message.destination, subject, text, idempotencyKey: message.id })
